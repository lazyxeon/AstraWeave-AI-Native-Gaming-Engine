...
fn test_transform_matrix_trs() {
    ...
    assert!(
        (origin_world - t.translation).length() < 0.001,
...
fn test_transform_forward() {
    ...
    assert!(
        (forward - Vec3::NEG_Z).length() < 0.001,
    ...
    assert!(
        (forward_rotated - Vec3::NEG_X).length() < 0.001,
        "90° Y rotation: forward = -X"
    );
}
...
fn test_transform_point_formula() {
    let t = Transform::new(Vec3::new(10.0, 0.0, 0.0), Quat::IDENTITY, Vec3::splat(2.0));
...
fn test_transform_lerp() {
    ...
    assert!(
        (mid.translation.x - 5.0).abs() < 0.001,
        "Translation lerped"
    );
...
fn test_transform_lerp_boundaries() {
    ...
    assert!(
        (at_0.translation - a.translation).length() < 0.001,
        "t=0 gives A"
    );
    assert!(
        (at_1.translation - b.translation).length() < 0.001,
        "t=1 gives B"
    );
}
...
fn test_aabb_from_center_half_extents() {
    ...
    assert!(
        (aabb.min - Vec3::new(5.0, 15.0, 25.0)).length() < 0.001,
        "min = center - half"
    );
    assert!(
        (aabb.max - Vec3::new(15.0, 25.0, 35.0)).length() < 0.001,
        "max = center + half"
    );
}
...
fn test_aabb_center() {
    ...
    assert!(
        (center - Vec3::new(5.0, 10.0, 15.0)).length() < 0.001,
        "center = (min + max) / 2"
    );
}
...
fn test_aabb_half_extents() {
    ...
    assert!(
        (half - Vec3::new(5.0, 10.0, 15.0)).length() < 0.001,
        "half = (max - min) / 2"
    );
}
...
fn test_frustum_plane_normalization() {
    ...
    assert!(
        ...
        "Normalized plane normal has length 1"
    );
}
...
fn test_cell_state_transitions() {
    ...
    assert!(
        is_valid_transition(CellState::Unloaded, CellState::Loading),
        "Unloaded->Loading valid"
    );
    assert!(
        is_valid_transition(CellState::Loading, CellState::Loaded),
        "Loading->Loaded valid"
    );
    assert!(
        is_valid_transition(CellState::Loaded, CellState::Unloading),
        "Loaded->Unloading valid"
    );
    assert!(
        !is_valid_transition(CellState::Unloaded, CellState::Loaded),
        "Unloaded->Loaded invalid (skip Loading)"
    );
}
...
fn test_streaming_config_defaults() {
    ...
    assert!(
        (streaming_radius - 500.0).abs() < 0.001,
        "Default 500m radius"
    );
