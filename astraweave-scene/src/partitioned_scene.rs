...
mod tests {
    ...
    #[test]
    fn test_on_cell_loaded() {
        let mut scene = PartitionedScene::new_default();
        let coord = GridCoord::new(0, 0, 0);

        // Create test cell data
        let mut cell_data = astraweave_asset::cell_loader::CellData::new([0, 0, 0]);
        cell_data.add_entity(
            astraweave_asset::cell_loader::EntityData::new([1.0, 0.0, 1.0])
        ...
        assert!(scene.query_entities_in_cell(coord).is_some());
        assert_eq!(scene.query_entities_in_cell(coord).unwrap().len(), 1);
    ...
    #[test]
    fn test_on_cell_unloaded() {
        let mut scene = PartitionedScene::new_default();
        let coord = GridCoord::new(0, 0, 0);

        // Setup: load cell with entities
        let mut cell_data = astraweave_asset::cell_loader::CellData::new([0, 0, 0]);
        cell_data.add_entity(astraweave_asset::cell_loader::EntityData::new([
            1.0, 0.0, 1.0,
        ]));
        scene.on_cell_loaded(coord, cell_data);
        scene.drain_events(); // Clear events
        ...
        assert!(events
            .iter()
            .any(|e| matches!(e, SceneEvent::CellUnloaded(_))));
        assert!(events
            .iter()
            .any(|e| matches!(e, SceneEvent::EntityDespawned(_, _))));
    }

    #[test]
    fn test_query_entities_in_multiple_cells() {
        let mut scene = PartitionedScene::new_default();

        // Add entities to multiple cells
        scene.move_entity_to_cell(1, GridCoord::new(0, 0, 0));
        scene.move_entity_to_cell(2, GridCoord::new(1, 0, 0));
        scene.move_entity_to_cell(3, GridCoord::new(0, 0, 1));

        let coords = vec![GridCoord::new(0, 0, 0), GridCoord::new(1, 0, 0)];

        let entities = scene.query_entities_in_cells(&coords);
