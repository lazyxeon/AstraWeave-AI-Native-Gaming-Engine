...
use astraweave_physics::{SpatialHash, AABB, PhysicsWorld, CharState};
use glam::Vec3;
...
fn test_spatial_hash_multi_cell_spanning() {
    ...
    assert!(grid.cell_count() >= 4, "Large object should span multiple cells");
}
...
fn test_control_character_vertical_movement() {
    ...
    assert!(transform.w_axis.y > 1.0, "Character should move upward, y={}", transform.w_axis.y);
}
