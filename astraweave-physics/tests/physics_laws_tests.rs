...
fn test_newtons_first_law_inertia() {
    ...
    assert!(
        (pos2 - expected_pos).length() < 0.001,
        "Object in motion drifted: {:?} vs {:?}",
        pos2,
        expected_pos
    );
}
...
fn test_newtons_third_law_reaction() {
    ...
    assert!(
        (delta_v1 + delta_v2).length() < 0.001,
        "Reaction forces not equal/opposite: dv1={:?}, dv2={:?}",
        delta_v1,
        delta_v2
    );
}
...
fn test_momentum_conservation() {
    ...
    assert!(
        (final_momentum - initial_momentum).length() < 0.001,
...
fn test_gravity_acceleration() {
    ...
    assert!(
        (vel - expected_vel).length() < 0.1,
        "Gravity acceleration incorrect: {:?} vs {:?}",
        vel,
        expected_vel
    );
}
