...
fn create_dynamic_box() {
    ...
    assert!((initial_pos.x - pos.x).abs() < 0.01, "X position should match");
    assert!((initial_pos.y - pos.y).abs() < 0.01, "Y position should match");
    assert!((initial_pos.z - pos.z).abs() < 0.01, "Z position should match");
}
...
fn body_transform_returns_valid_matrix() {
    ...
    assert!((position.x - pos.x).abs() < 0.01, "Transform X should match initial position");
    assert!((position.y - pos.y).abs() < 0.01, "Transform Y should match initial position");
    assert!((position.z - pos.z).abs() < 0.01, "Transform Z should match initial position");
}
...
fn dynamic_box_falls_with_gravity() {
    ...
    assert!(final_y < initial_y, "Box should fall due to gravity: initial_y={}, final_y={}", initial_y, final_y);
}
...
fn static_ground_does_not_move() {
    ...
    assert!((final_pos.x - initial_pos.x).abs() < 0.001, "Ground X should not move");
    assert!((final_pos.y - initial_pos.y).abs() < 0.001, "Ground Y should not move");
    assert!((final_pos.z - initial_pos.z).abs() < 0.001, "Ground Z should not move");
}
...
fn character_moves_horizontally() {
    ...
    assert!(final_x > initial_x + 0.5, "Character should move forward: initial_x={}, final_x={}", initial_x, final_x);
}
...
fn character_zero_velocity_does_not_move() {
    ...
    assert!(distance < 0.1, "Character should not move with zero velocity, moved distance={}", distance);
}
...
fn default_layer_bodies_collide() {
    ...
    assert!(final_y > 0.5 && final_y < 3.0, "Box should rest on ground at y={}", final_y);
}
...
fn large_position_values() {
    ...
    assert!((pos.x - large_pos.x).abs() < 1.0, "Large X position should be preserved");
    assert!((pos.z - large_pos.z).abs() < 1.0, "Large Z position should be preserved");
}
