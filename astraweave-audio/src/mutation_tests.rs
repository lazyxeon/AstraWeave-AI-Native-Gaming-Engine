...
mod boolean_return_path_tests {
    ...
    fn test_ears_facing_neg_z_exact_positions() {
        ...
        assert!(left[1].abs() < 1e-4, "Left ear Y must be 0, got {}", left[1]);
        assert!(left[2].abs() < 1e-4, "Left ear Z must be 0, got {}", left[2]);
        assert!(
            (right[0] - 0.1).abs() < 1e-4,
            "Right ear X must be 0.1, got {}",
            right[0]
        );
        assert!(right[1].abs() < 1e-4, "Right ear Y must be 0, got {}", right[1]);
        assert!(right[2].abs() < 1e-4, "Right ear Z must be 0, got {}", right[2]);
    }
