...
pub fn generate_test_beep(path: &str, frequency: f32, duration_secs: f32, sample_rate: u32) -> Result<()> {
...
pub fn setup_all_test_assets() -> Result<()> {
    ...
    generate_test_beep(&format!("{}/test_beep_440hz.wav", assets_dir), 440.0, 0.5, 22050)?;
    
    // Low frequency beep for bass testing (200 Hz, 0.5 seconds)
    generate_test_beep(&format!("{}/test_beep_200hz.wav", assets_dir), 200.0, 0.5, 22050)?;
    
    // High frequency beep (1000 Hz, 0.3 seconds)
    generate_test_beep(&format!("{}/test_beep_1000hz.wav", assets_dir), 1000.0, 0.3, 22050)?;
    
    // Very short beep for rapid-fire testing (0.1 seconds)
    generate_test_beep(&format!("{}/test_beep_short.wav", assets_dir), 440.0, 0.1, 22050)?;
    
    // Long beep for duration testing (3 seconds)
    generate_test_beep(&format!("{}/test_beep_long.wav", assets_dir), 440.0, 3.0, 22050)?;
...
mod tests {
    ...
    fn test_generate_beep() -> Result<()> {
        ...
        assert!((metadata.len() as f32 - expected_total).abs() < 100.0, "File size should be close to expected");
    ...
    fn test_generate_music() -> Result<()> {
        ...
        let metadata = fs::metadata(path)?;
        assert!(metadata.len() > 44, "Music file should be larger than header");
    ...
    fn test_generate_voice() -> Result<()> {
        ...
        let metadata = fs::metadata(path)?;
        assert!(metadata.len() > 44, "Voice file should be larger than header");
    ...
    fn test_cleanup_assets() -> Result<()> {
        ...
        assert!(!Path::new("tests/assets").exists(), "Assets directory should be removed");
