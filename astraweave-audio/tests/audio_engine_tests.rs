...
fn test_master_volume_control() {
    ...
    assert_eq!(
        ...
        "Master volume should be set to 0.5"
    );

    engine.set_master_volume(0.0);
    assert_eq!(
        engine.master_volume, 0.0,
        "Master volume should be set to 0.0"
    );

    engine.set_master_volume(1.0);
    assert_eq!(
        engine.master_volume, 1.0,
        "Master volume should be set to 1.0"
    );
    ...
    assert_eq!(
        engine.master_volume, 1.0,
        "Master volume should clamp to 1.0"
    );

    engine.set_master_volume(-0.5);
    assert_eq!(
        engine.master_volume, 0.0,
        "Master volume should clamp to 0.0"
    );
}
...
fn test_3d_sfx_beep_playback() {
    ...
    for (i, pos) in positions.iter().enumerate() {
        ...
        assert!(
            result.is_ok(),
            "3D beep at position {:?} should succeed",
            pos
        );
    }
}
...
fn test_spatial_audio_listener_movement() {
    ...
    // Play 3D sound
    engine
        .play_sfx_3d_beep(emitter_id, sound_pos, 440.0, 2.0, 0.5)
        .unwrap();
...
fn test_distance_attenuation() {
    ...
    let near_id = 1;
    engine
        .play_sfx_3d_beep(near_id, vec3(1.0, 0.0, 0.0), 440.0, 1.0, 0.8)
        .unwrap();

    // Far sound (should be quiet due to attenuation)
    let far_id = 2;
    engine
        .play_sfx_3d_beep(far_id, vec3(100.0, 0.0, 0.0), 440.0, 1.0, 0.8)
        .unwrap();

    // Very far sound (should be very quiet)
    let very_far_id = 3;
    engine
        .play_sfx_3d_beep(very_far_id, vec3(1000.0, 0.0, 0.0), 440.0, 1.0, 0.8)
        .unwrap();
...
fn test_listener_orientation() {
    ...
    let emitter_id = 1;
    engine
        .play_sfx_3d_beep(emitter_id, vec3(5.0, 0.0, 0.0), 440.0, 2.0, 0.5)
        .unwrap();
...
fn test_rapid_listener_changes() {
    ...
    let emitter_id = 1;
    engine
        .play_sfx_3d_beep(emitter_id, vec3(10.0, 0.0, 0.0), 440.0, 5.0, 0.5)
        .unwrap();
...
fn test_overlapping_emitters() {
    ...
    for i in 0..5 {
        engine
            .play_sfx_3d_beep(i, pos, 200.0 + (i as f32 * 100.0), 1.0, 0.5)
            .unwrap();
    }
...
fn test_extreme_listener_positions() {
    ...
    let emitter_id = 1;
    engine
        .play_sfx_3d_beep(emitter_id, vec3(0.0, 0.0, 0.0), 440.0, 2.0, 0.5)
        .unwrap();
...
fn test_emitter_reuse() {
    ...
    // Play at position 1
    engine
        .play_sfx_3d_beep(emitter_id, vec3(5.0, 0.0, 0.0), 440.0, 0.5, 0.5)
        .unwrap();
    engine.tick(0.1);

    // Reuse same emitter at position 2
    engine
        .play_sfx_3d_beep(emitter_id, vec3(-5.0, 0.0, 0.0), 880.0, 0.5, 0.5)
        .unwrap();
    engine.tick(0.1);

    // Reuse again at position 3
    engine
        .play_sfx_3d_beep(emitter_id, vec3(0.0, 5.0, 0.0), 220.0, 0.5, 0.5)
        .unwrap();
    engine.tick(0.1);
}
...
fn test_zero_duration_sounds() {
    ...
    engine
        .play_sfx_3d_beep(1, vec3(5.0, 0.0, 0.0), 440.0, 0.0, 0.5)
        .unwrap();
