...
fn test_play_voice_file_explicit_duration() -> Result<()> {
    ...
    assert!(result.is_ok(), "Should play voice file with explicit duration");
...
fn test_invalid_file_format() -> Result<()> {
    ...
    assert!(result.is_err(), "Should return error for invalid file format");
...
fn test_emitter_reuse_with_files() -> Result<()> {
    ...
    // Play at position 1
    engine.play_sfx_3d_file(emitter_id, "tests/assets/test_beep_200hz.wav", vec3(5.0, 0.0, 0.0))?;
    thread::sleep(Duration::from_millis(100));
    engine.tick(0.1);
    
    // Reuse same emitter at position 2
    engine.play_sfx_3d_file(emitter_id, "tests/assets/test_beep_440hz.wav", vec3(-5.0, 0.0, 0.0))?;
    thread::sleep(Duration::from_millis(100));
    engine.tick(0.1);
    
    // Reuse again at position 3
    engine.play_sfx_3d_file(emitter_id, "tests/assets/test_beep_1000hz.wav", vec3(0.0, 5.0, 0.0))?;
    thread::sleep(Duration::from_millis(100));
    engine.tick(0.1);
