...
fn test_ai_planning_latency_under_load() {
    ...
    println!("   Total time: {:.2}ms for {} agents", 
             elapsed.as_secs_f64() * 1000.0, NUM_AI_AGENTS);
}
...
fn test_frame_budget_never_exceeded() {
    ...
    println!("   Max frame time: {:.2}ms ({:.1}% of budget)",
             max_frame_time.as_secs_f64() * 1000.0,
...
fn test_memory_allocation_stability() {
    ...
    println!("   Min: {}, Max: {}, Initial: {}", min_count, max_count, NUM_ENTITIES);
}
...
fn test_stress_10k_entities_graceful_degradation() {
    ...
    println!("   {} entities simulated for {} frames", NUM_ENTITIES, NUM_FRAMES);
    println!("   Avg frame time: {:.2}ms", avg_frame_time.as_secs_f64() * 1000.0);
    println!("   Max frame time: {:.2}ms", max_frame_time.as_secs_f64() * 1000.0);
    println!("   Total time: {:.2}s", total_time.as_secs_f64());
    
    if avg_frame_time > FRAME_BUDGET_60FPS {
        println!("   ⚠️  Frame drops expected (avg {}ms > 16.67ms budget)", 
