...
fn test_capture_and_replay_minimal() {
    ...
    let path = "test.snap";
    capture_replay::capture_state(5, path, &world)
        .unwrap_or_else(|e| panic!("failed to capture state (seed: 5, path: {}): {e}", path));
    let cfg = SimConfig { dt: 0.016 };
    let w2 = capture_replay::replay_state(path, 3, &cfg)
        .unwrap_or_else(|e| panic!("failed to replay state (seed: 3, path: {}): {e}", path));
