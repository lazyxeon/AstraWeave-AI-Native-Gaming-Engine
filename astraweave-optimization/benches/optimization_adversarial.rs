...
impl LoadBalancer {
    fn select_worker(&self, _request_size: usize) -> Option<u32> {
        match self.strategy {
            LoadBalanceStrategy::RoundRobin => self.workers.first().map(|w| w.id),
            ...
            LoadBalanceStrategy::WeightedRandom => {
                ...
                let total_capacity: f32 = self
                    .workers
                    .iter()
                    .map(|w| w.capacity - w.current_load)
                    .sum();
                ...
                self.workers
                    .iter()
                    .find(|w| w.current_load < w.capacity)
                    .map(|w| w.id)
            }
            LoadBalanceStrategy::LatencyAware => self
                .workers
                .iter()
                .filter(|w| w.current_load < w.capacity)
...
fn bench_batch_inference(c: &mut Criterion) {
    ...
    group.bench_function("batch_padding_1000", |bencher| {
        let batches: Vec<Vec<usize>> = (0..1000)
            .map(|i| (0..32).map(|j| 50 + (i * j) % 200).collect())
            .collect();
        ...
        bencher.iter(|| {
            ...
            let avg_efficiency: f32 =
                padding_stats.iter().map(|(_, _, e)| e).sum::<f32>() / padding_stats.len() as f32;
    ...
    group.bench_function("dynamic_batch_sizing_2000", |bencher| {
        let request_sizes: Vec<usize> = (0..2000).map(|i| 20 + (i % 500)).collect();
...
fn bench_prompt_caching(c: &mut Criterion) {
    ...
    group.bench_function("cache_lookup_10000", |bencher| {
        ...
        let lookups: Vec<String> = (0..10000).map(|i| format!("prompt_{}", i % 1000)).collect();
...
fn bench_compression(c: &mut Criterion) {
    ...
    group.bench_function("token_compression_10000", |bencher| {
        let token_sequences: Vec<Vec<u32>> = (0..10000)
            .map(|i| (0..100).map(|j| ((i * 100 + j) % 50000) as u32).collect())
            .collect();
...
fn bench_load_balancing(c: &mut Criterion) {
    ...
    for strategy in [
        ...
        group.bench_with_input(
            ...
            |bencher, &strategy| {
                ...
                let request_sizes: Vec<usize> = (0..10000).map(|i| 100 + (i % 1000)).collect();
    ...
    group.bench_function("request_routing_10000", |bencher| {
        ...
        let requests: Vec<String> = (0..10000)
            .map(|i| ["text", "code", "image", "math"][i % 4].to_string())
            .collect();
...
fn bench_token_optimization(c: &mut Criterion) {
    ...
    group.bench_function("tokenization_5000", |bencher| {
        ...
        bencher.iter(|| {
            let tokenized: Vec<Vec<u32>> = texts.iter().map(|t| optimizer.tokenize(t)).collect();
    ...
    group.bench_function("token_merging_2000", |bencher| {
        ...
        let token_sequences: Vec<Vec<u32>> = (0..2000)
            .map(|i| (0..200).map(|j| ((i * 200 + j) % 50000) as u32).collect())
            .collect();
...
fn bench_model_optimization(c: &mut Criterion) {
    ...
    group.bench_function("activation_checkpointing_500", |bencher| {
        ...
        let activations: Vec<Vec<f32>> = (0..500)
            .map(|i| (0..1024).map(|j| (i + j) as f32 * 0.001).collect())
            .collect();
