...
pub fn compile_to_wgsl(g: &Graph) -> Result<(String, Vec<String>)> {
    fn expr_of(nodes: &std::collections::BTreeMap<String, Node>, id: &str, binds: &mut Vec<String>) -> Result<String> {
        let n = nodes.get(id).ok_or_else(|| anyhow!("missing node {id}"))?;
        Ok(match n {
            Node::Texture2D{ id, uv } => { binds.push(id.clone()); format!("textureSample(tex_{}, samp_{}, {}).rgb", id, id, uv) },
            Node::Constant3{ value } => format!("vec3<f32>({},{},{})", value[0], value[1], value[2]),
            Node::Constant1{ value } => format!("{value}"),
            Node::Multiply{ a, b } => format!("(({}) * ({}))", expr_of(nodes, a, binds)?, expr_of(nodes, b, binds)?),
            Node::Add{ a, b } => format!("(({}) + ({}))", expr_of(nodes, a, binds)?, expr_of(nodes, b, binds)?),
            Node::MetallicRoughness{ metallic, roughness } => format!("vec2<f32>({},{})", expr_of(nodes, metallic, binds)?, expr_of(nodes, roughness, binds)?),
            Node::Clearcoat{ weight, roughness } => format!("vec2<f32>({},{})", expr_of(nodes, weight, binds)?, expr_of(nodes, roughness, binds)?),
            Node::Anisotropy{ amount } => format!("{}", expr_of(nodes, amount, binds)?),
            Node::Transmission{ amount } => format!("{}", expr_of(nodes, amount, binds)?),
            Node::NormalMap{ id, uv, scale } => { binds.push(id.clone()); format!("normalize((textureSample(tex_{}, samp_{}, {}).xyz*2.0-1.0) * {:.6})", id, id, uv, scale) },
        })
    }

    let mut binds = Vec::new();
    let bc = expr_of(&g.nodes, &g.base_color, &mut binds)?;
    let mr = if let Some(m) = &g.mr { Some(expr_of(&g.nodes, m, &mut binds)?) } else { None };
    let nrm = if let Some(n) = &g.normal { Some(expr_of(&g.nodes, n, &mut binds)?) } else { None };
    let cc = if let Some(c) = &g.clearcoat { Some(expr_of(&g.nodes, c, &mut binds)?) } else { None };
    let aniso = if let Some(a) = &g.anisotropy { Some(expr_of(&g.nodes, a, &mut binds)?) } else { None };
    let trans = if let Some(t) = &g.transmission { Some(expr_of(&g.nodes, t, &mut binds)?) } else { None };
...
mod tests {
    ...
    fn compiles_minimal_graph() {
        ...
        nodes.insert("uv".into(), Node::Constant3{ value: [0.0,0.0,0.0]}); // unused
        nodes.insert("c".into(), Node::Constant3{ value: [1.0,0.5,0.1]});
    let g = Graph{ nodes, base_color: "c".into(), mr: None, normal: None, clearcoat: None, anisotropy: None, transmission: None };
    ...
    fn compiles_tex_and_mr_and_validates_with_naga() {
        ...
        nodes.insert("mr".into(), Node::MetallicRoughness{ metallic: "m".into(), roughness: "r".into()});
    let g = Graph{ nodes, base_color: "base_tex".into(), mr: Some("mr".into()), normal: None, clearcoat: None, anisotropy: None, transmission: None };
    ...
    fn snapshot_contains_clearcoat_aniso_transmission() {
        let mut nodes = std::collections::BTreeMap::new();
        nodes.insert("c".into(), Node::Constant3{ value: [0.8,0.6,0.2]});
        nodes.insert("m".into(), Node::Constant1{ value: 0.5});
        nodes.insert("r".into(), Node::Constant1{ value: 0.3});
        nodes.insert("mr".into(), Node::MetallicRoughness{ metallic: "m".into(), roughness: "r".into()});
        nodes.insert("cc".into(), Node::Clearcoat{ weight: "m".into(), roughness: "r".into()});
        nodes.insert("an".into(), Node::Anisotropy{ amount: "m".into()});
        nodes.insert("tr".into(), Node::Transmission{ amount: "r".into()});
        let g = Graph{ nodes, base_color: "c".into(), mr: Some("mr".into()), normal: None, clearcoat: Some("cc".into()), anisotropy: Some("an".into()), transmission: Some("tr".into()) };
        let (wgsl, _b) = compile_to_wgsl(&g).unwrap();
