...
fn test_save_load_roundtrip() -> Result<()> {
    ...
    assert_eq!(
        loaded_bundle.world.state_hash,
        original_bundle.world.state_hash
    );
    assert_eq!(
        loaded_bundle.inventory.credits,
        original_bundle.inventory.credits
    );
    assert_eq!(
        loaded_bundle.inventory.items.len(),
        original_bundle.inventory.items.len()
    );
    assert_eq!(
        loaded_bundle.companions.len(),
        original_bundle.companions.len()
    );
    assert_eq!(
        loaded_bundle.companions[0].name,
        original_bundle.companions[0].name
    );
