...
#[command(
    name = "aw_save_cli",
    version,
    about = "AstraWeave save/loader & migration utility"
)]
struct Cli {
    ...
    match cli.cmd {
        Cmd::List { player } => {
            ...
            for m in sm.list_saves(&player)? {
                println!(
                    "[slot {:02}] {}  id={}  schema={}",
                    m.slot, m.created_at, m.save_id, m.schema
                );
            }
        }
        Cmd::Verify { file } => {
            ...
            println!(
                "OK: schema={} id={} player={} slot={}",
                v2.schema, v2.save_id, v2.player_id, v2.slot
            );
        }
        Cmd::Migrate { file } => {
            let sm = SaveManager::new(cli.root);
        ...
        Cmd::DemoSave { player, slot } => {
            ...
            let bundle = SaveBundleV2 {
                ...
                world: WorldState {
                    tick: 123,
                    ecs_blob: vec![1, 2, 3, 4],
                    state_hash: 0xdead_beef_cafe_babe,
                },
