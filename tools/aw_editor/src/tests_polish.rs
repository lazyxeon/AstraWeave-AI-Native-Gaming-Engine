#[cfg(test)]
mod tests {
    use crate::polish::{LoadingScreen, LoadingStyle, SplashScreen, SplashSequence};
    ...
    fn test_sequence_total_duration() {
        let seq = SplashSequence::new().with_engine_logo(); // 2s + 0.4 fadein + 0.4 fadeout = 2.8s
    ...
    fn test_loading_screen_builder() {
        let mut screen =
            LoadingScreen::default().with_tips(vec!["Tip 1".to_string(), "Tip 2".to_string()]);
