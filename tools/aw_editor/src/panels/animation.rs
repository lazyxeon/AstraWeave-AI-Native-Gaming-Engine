...
impl AnimationClip {
    ...
    pub fn sample_bounce() -> Self {
        ...
        y_track.keyframes = vec![
            Keyframe { time: 0.0, value: 0.0, easing: EasingFunction::QuadOut },
            Keyframe { time: 0.5, value: 2.0, easing: EasingFunction::QuadIn },
            Keyframe { time: 1.0, value: 0.0, easing: EasingFunction::BounceOut },
        ];
    ...
    pub fn sample_spin() -> Self {
        ...
        rot_track.keyframes = vec![
            ...
            Keyframe { time: 2.0, value: std::f32::consts::TAU, easing: EasingFunction::Linear },
        ];
    ...
    pub fn sample_pulse() -> Self {
        ...
        scale_track.keyframes = vec![
            Keyframe { time: 0.0, value: 1.0, easing: EasingFunction::SineInOut },
            Keyframe { time: 0.5, value: 1.3, easing: EasingFunction::SineInOut },
            Keyframe { time: 1.0, value: 1.0, easing: EasingFunction::SineInOut },
        ];
...
impl Default for AnimationPanel {
    fn default() -> Self {
        ...
        let mut bounce_tween = Tween::new(0.0f32, 200.0f32, 2.0).with_easing(EasingFunction::ElasticOut);
        bounce_tween.play();

        let mut color_tween = Tween::new(Color32::RED, Color32::BLUE, 3.0).with_easing(EasingFunction::SineInOut);
        color_tween.play();
        ...
        Self {
            ...
            output: AnimationOutput { scale_multiplier: 1.0, ..Default::default() },
...
impl AnimationPanel {
    ...
    fn show_editor_ui(&mut self, ui: &mut egui::Ui) {
        ...
        ui.horizontal(|ui| {
            ...
            if let Some(entity_id) = self.selected_entity {
                ui.label(egui::RichText::new(format!("Entity #{}", entity_id)).color(Color32::GREEN).strong());
            } else {
                ui.label(egui::RichText::new("None selected").color(Color32::GRAY).italics());
            }
        });
        ...
        ui.horizontal(|ui| {
            ...
            if let Some(clip) = self.selected_clip_idx.and_then(|idx| self.clips.get_mut(idx)) {
                ui.checkbox(&mut clip.looping, "🔁");
            }
            
            ui.label("Speed:");
            ui.add(egui::DragValue::new(&mut self.playback_speed).speed(0.1).range(0.1..=3.0).suffix("x"));
        });
        ...
        if let Some(clip) = self.selected_clip_idx.and_then(|idx| self.clips.get(idx)) {
            ui.horizontal(|ui| {
                ...
                let response = ui.add(egui::Slider::new(&mut self.current_time, 0.0..=clip.duration).show_value(false));
                if response.dragged() {
            ...
            if let Some(clip) = self.clips.get_mut(clip_idx) {
                ui.horizontal(|ui| {
                    if ui.button("➕ Add Track").clicked() {
                        clip.tracks.push(AnimationTrack::new(AnimatedProperty::PositionY));
                    }
                    ui.add(egui::DragValue::new(&mut clip.duration).prefix("Dur: ").suffix("s").speed(0.1).range(0.1..=30.0));
                });
                ...
                for (track_idx, track) in clip.tracks.iter_mut().enumerate() {
                    ui.horizontal(|ui| {
                        egui::ComboBox::from_id_salt(format!("tr_{}", track_idx))
                            ...
                            .width(100.0)
                            .show_ui(ui, |ui| {
                                ui.selectable_value(&mut track.property, AnimatedProperty::PositionX, "Position X");
                                ui.selectable_value(&mut track.property, AnimatedProperty::PositionY, "Position Y");
                                ui.selectable_value(&mut track.property, AnimatedProperty::PositionZ, "Position Z");
                                ui.selectable_value(&mut track.property, AnimatedProperty::RotationY, "Rotation Y");
                                ui.selectable_value(&mut track.property, AnimatedProperty::Scale, "Scale");
                            });
                        ...
                        if ui.small_button("➕").on_hover_text("Add keyframe").clicked() {
                            ...
                            track.keyframes.push(Keyframe { time: self.current_time, value: val, easing: EasingFunction::Linear });
                    ...
                    if !track.keyframes.is_empty() {
                        ui.indent(format!("kf_{}", track_idx), |ui| {
                            ...
                            for (kf_idx, kf) in track.keyframes.iter_mut().enumerate() {
                                ui.horizontal(|ui| {
                                    ui.add(egui::DragValue::new(&mut kf.time).prefix("t:").suffix("s").speed(0.01).range(0.0..=clip.duration));
                                    ui.add(egui::DragValue::new(&mut kf.value).prefix("v:").speed(0.1));
                                    egui::ComboBox::from_id_salt(format!("e_{}_{}", track_idx, kf_idx))
                                        .selected_text(format!("{:?}", kf.easing).chars().take(6).collect::<String>())
                                        .width(60.0)
                                        .show_ui(ui, |ui| {
                                            ui.selectable_value(&mut kf.easing, EasingFunction::Linear, "Linear");
                                            ui.selectable_value(&mut kf.easing, EasingFunction::QuadOut, "QuadOut");
                                            ui.selectable_value(&mut kf.easing, EasingFunction::SineInOut, "SineInOut");
                                            ui.selectable_value(&mut kf.easing, EasingFunction::ElasticOut, "Elastic");
                                            ui.selectable_value(&mut kf.easing, EasingFunction::BounceOut, "Bounce");
                                        });
                                    if ui.small_button("×").clicked() { kf_remove = Some(kf_idx); }
                                });
                            }
                            if let Some(idx) = kf_remove { track.keyframes.remove(idx); }
                        });
                    }
                }
                
                if let Some(idx) = track_to_remove { clip.tracks.remove(idx); }
            }
        }
        ...
        ui.painter().line_segment([Pos2::new(center.x, anim_y), Pos2::new(end_x, end_y)], egui::Stroke::new(2.0, Color32::YELLOW));
        
        // Entity preview
        ui.painter().circle_filled(Pos2::new(center.x, anim_y), anim_scale, Color32::from_rgb(100, 150, 255));
        
        ui.label(format!("Y:{:.1} Rot:{:.0}° S:{:.2}", self.output.position_offset.y, self.output.rotation_y.to_degrees(), self.output.scale_multiplier));
    ...
    fn show_demo_ui(&mut self, ui: &mut egui::Ui, dt: f32) {
        ...
        ui.painter().circle_filled(Pos2::new(rect.left() + 80.0, rect.top() + y + 20.0), 18.0, Color32::GREEN);
        if ui.button("🔄 Restart").clicked() { self.bounce_tween.restart(); }
        ...
        if ui.button("🔄 Restart").clicked() { self.color_tween.restart(); }
        
        ui.add_space(10.0);
        
        // Spring demo
        ui.label("🌀 Spring Physics:");
        let (rect, response) = ui.allocate_exact_size(Vec2::new(350.0, 150.0), egui::Sense::hover());
        ...
        ui.painter().circle_filled(Pos2::new(spring_x, rect.center().y), 22.0, Color32::YELLOW);
        ...
        if self.show_easing {
            ...
            let colors = [Color32::RED, Color32::GREEN, Color32::BLUE, Color32::YELLOW, 
                         Color32::LIGHT_BLUE, Color32::from_rgb(255, 128, 0), Color32::WHITE];
            ...
            for (i, (name, tween)) in self.easing_tweens.iter_mut().enumerate() {
                ...
                if t > 0.9 {
                    ui.painter().text(Pos2::new(rect.right() + 5.0, rect.top() + i as f32 * 14.0), 
                        egui::Align2::LEFT_TOP, name, egui::FontId::proportional(9.0), c);
                }
            }
            
            if ui.button("🔄 Restart All").clicked() {
                for (_, tw) in &mut self.easing_tweens { tw.restart(); }
            }
        }
    }
