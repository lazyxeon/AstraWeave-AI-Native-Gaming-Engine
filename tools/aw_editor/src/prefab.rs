...
impl PrefabData {
    ...
    fn collect_entity_recursive(
        ...
        let pose = world.pose(entity)
            .context("Entity pose not found")?;
        
        let health = world.health(entity)
            .map(|h| h.hp)
            .unwrap_or(100);
        
        let team_id = world.team(entity)
            .map(|t| t.id as u32)
            .unwrap_or(0);
    ...
    pub fn save_to_file<P: AsRef<Path>>(&self, path: P) -> Result<()> {
        ...
        std::fs::write(path.as_ref(), ron_string)
            .context("Failed to write prefab file")?;
    ...
    pub fn load_from_file<P: AsRef<Path>>(path: P) -> Result<Self> {
        let ron_string = std::fs::read_to_string(path.as_ref())
            .context("Failed to read prefab file")?;
        
        let prefab: PrefabData = ron::from_str(&ron_string)
            .context("Failed to deserialize prefab from RON")?;
    ...
    pub fn instantiate(&self, world: &mut World, spawn_pos: (i32, i32)) -> Result<PrefabInstance> {
        ...
        let root_entity = world.spawn(
            ...
            astraweave_core::Team { id: root_data.team_id as u8 },
            root_data.health,
            0,
        );
        ...
        for (idx, entity_data) in self.entities.iter().enumerate().skip(1) {
            ...
            let entity = world.spawn(
                ...
                astraweave_core::Team { id: entity_data.team_id as u8 },
                entity_data.health,
                0,
            );
...
mod tests {
    ...
    fn test_prefab_serialization() {
        ...
        let ron_string = ron::ser::to_string_pretty(&prefab, ron::ser::PrettyConfig::default()).unwrap();
