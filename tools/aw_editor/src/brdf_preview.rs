...
impl BrdfPreview {
    ...
    fn evaluate_brdf(&self, normal: Vec3, view: Vec3, light: Vec3) -> Vec3 {
        ...
        let diffuse = k_d * Vec3::new(self.albedo[0], self.albedo[1], self.albedo[2]) / std::f32::consts::PI;
        
        // Combine diffuse and specular
        let light_color = Vec3::new(self.light_color[0], self.light_color[1], self.light_color[2]);
    ...
    pub fn show(&mut self, ui: &mut Ui, ctx: &egui::Context) {
        ...
        ui.group(|ui| {
            ...
            changed |= ui.add(
                egui::Slider::new(&mut self.metallic, 0.0..=1.0)
                    .text("Metallic")
            ).changed();
        ...
        ui.group(|ui| {
            ...
            changed |= ui.add(
                egui::Slider::new(&mut light_x, -1.0..=1.0)
                    .text("Light X")
            ).changed();
            
            changed |= ui.add(
                egui::Slider::new(&mut light_y, -1.0..=1.0)
                    .text("Light Y")
            ).changed();
            ...
            changed = false;
            changed |= ui.add(
                egui::Slider::new(&mut self.light_intensity, 0.0..=5.0)
                    .text("Intensity")
            ).changed();
        ...
        ui.group(|ui| {
            ...
            ui.label(format!("Albedo: RGB({:.2}, {:.2}, {:.2})", 
