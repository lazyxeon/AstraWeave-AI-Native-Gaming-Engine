...
use panels::{
    ...
    HierarchyAction, HierarchyPanel, Panel, PerformancePanel, TransformPanel, WorldPanel,
};
...
use telemetry::EditorTelemetryEvent;
use ui::StatusBar;
use uuid::Uuid;
...
impl Default for EditorApp {
    fn default() -> Self {
        ...
        Self {
            ...
            scene_state: Some(EditorSceneState::new(Self::create_default_world())),
            sim_tick_count: 0,
...
impl EditorApp {
    ...
    fn scene_state(&self) -> Option<&EditorSceneState> {
        self.scene_state.as_ref()
    }

    fn scene_state_mut(&mut self) -> Option<&mut EditorSceneState> {
        self.scene_state.as_mut()
    }
    ...
    fn replace_scene_state(&mut self, world: World) {
        self.scene_state = Some(EditorSceneState::new(world));
    }

    fn sync_scene_cache(&mut self) {
        if let Some(state) = self.scene_state.as_mut() {
            state.sync_all();
        }
    }

    fn ensure_scene_state(&mut self) -> &mut EditorSceneState {
        if self.scene_state.is_none() {
            self.replace_scene_state(Self::create_default_world());
        }
        self.scene_state.as_mut().expect("scene state initialized")
    }

    fn emit_snapping_event(&self) {
        telemetry::record(EditorTelemetryEvent::GridSettingsChanged {
            grid_enabled: self.snapping_config.grid_enabled,
            snap_size: self.snapping_config.grid_size,
            angle_enabled: self.snapping_config.angle_enabled,
            angle_increment: self.snapping_config.angle_increment,
        });
    }
    ...
    fn process_hierarchy_actions(&mut self, actions: Vec<HierarchyAction>) {
        if actions.is_empty() {
            return;
        }

        let mut scene_state = match self.scene_state.take() {
            Some(state) => state,
            None => {
                self.console_logs
                    .push("⚠️ Hierarchy actions require an active world".into());
                return;
            }
        };
            ...
            match action {
                HierarchyAction::CreatePrefab(entity) => {
                    ...
                    let sanitized: String = raw_name
                        ...
                        .collect();
                    let prefab_name = if sanitized.trim_matches('_').is_empty() {
                        format!("prefab_{}", entity)
                    } else {
                        sanitized
                    };

                    let prefab_result = self.prefab_manager.create_prefab(
                        scene_state.world(),
                        entity,
                        &prefab_name,
                    );
                    ...
                    match prefab_result {
                        Ok(path) => {
                            self.status = format!("💾 Prefab saved as {}", prefab_name);
                            self.console_logs
                                .push(format!("💾 Prefab created at {:?}", path));
                        }
                        Err(err) => {
                            self.status = format!("❌ Prefab save failed: {err}");
                            self.console_logs.push(format!(
                                "❌ Failed to create prefab for entity {}: {}",
                                entity, err
                            ));
                        }
                    }
                ...
                HierarchyAction::DuplicateEntity(entity) => {
                    let cmd =
                        command::DuplicateEntitiesCommand::new(vec![entity], IVec2 { x: 1, y: 1 });
                    let result = self.undo_stack.execute(cmd, scene_state.world_mut());

                    match result {
                        Ok(()) => {
                            self.status = "📋 Duplicated entity".into();
                            self.console_logs
                                .push(format!("📋 Duplicated entity {} via hierarchy", entity));
                            scene_state.sync_all();
                        }
                        Err(err) => {
                            self.status = format!("❌ Duplicate failed: {err}");
                            self.console_logs
                                .push(format!("❌ Failed to duplicate entity {}: {}", entity, err));
                        }
                    }
                }
                HierarchyAction::DeleteEntity(entity) => {
                    let cmd = command::DeleteEntitiesCommand::new(vec![entity]);
                    let result = self.undo_stack.execute(cmd, scene_state.world_mut());
                    ...
                    match result {
                        Ok(()) => {
                            ...
                            self.status = "🗑️ Deleted entity".into();
                            self.console_logs
                                .push(format!("🗑️ Deleted entity {} via hierarchy", entity));
                            scene_state.sync_all();
                        }
                        Err(err) => {
                            self.status = format!("❌ Delete failed: {err}");
                            self.console_logs
                                .push(format!("❌ Failed to delete entity {}: {}", entity, err));
                        }
                    }
                }
            }
    ...
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        ctx.input(|i| {
            ...
            if i.modifiers.ctrl && i.key_pressed(egui::Key::Z) && !i.modifiers.shift {
                if let Some(mut scene_state) = self.scene_state.take() {
                    let undo_result = self.undo_stack.undo(scene_state.world_mut());
                    self.scene_state = Some(scene_state);
                    ...
                    self.console_logs.push("⚠️  No world to undo".into());
                }
            }
            ...
            {
                if let Some(mut scene_state) = self.scene_state.take() {
                    let redo_result = self.undo_stack.redo(scene_state.world_mut());
                    self.scene_state = Some(scene_state);
                    ...
                    self.console_logs.push("⚠️  No world to redo".into());
                }
            }
            ...
            if i.modifiers.ctrl && i.key_pressed(egui::Key::O) {
                ...
                match scene_serialization::load_scene(&path) {
                    Ok(world) => {
                        self.replace_scene_state(world);
                        self.current_scene_path = Some(path.clone());
                        self.recent_files.add_file(path.clone());
                ...
                if let Some(clipboard) = &self.clipboard {
                    if let Some(mut scene_state) = self.scene_state.take() {
                        ...
                        match result {
                            Ok(()) => {
                                self.status =
                                    format!("📋 Pasted {} entities", clipboard.entities.len());
                                self.console_logs.push(format!(
                                    "✅ Pasted {} entities",
                                    clipboard.entities.len()
                                ));
                                self.sync_scene_cache();
                            }
                            Err(e) => {
                        ...
                        self.console_logs.push("⚠️  No world to paste into".into());
                    }
                } else {
            ...
            if i.modifiers.ctrl && i.key_pressed(egui::Key::D) {
                if let Some(mut scene_state) = self.scene_state.take() {
                    ...
                    self.console_logs
                        .push("⚠️  No world available for duplication".into());
                }
            }
            ...
            if i.key_pressed(egui::Key::F7) {
                if !self.editor_mode.is_editing() {
                    if let Some(snapshot) = self.world_snapshot.take() {
                        self.replace_scene_state(snapshot.to_world());
            ...
            if i.key_pressed(egui::Key::Delete) {
                if self.editor_mode.can_edit() {
                    if let Some(mut scene_state) = self.scene_state.take() {
                        ...
                        self.console_logs
                            .push("⚠️  No world available to delete from".into());
                    }
                }
            }
        });
                    ...
                    .show(ui, |ui| {
                        ...
                        ui.collapsing("🌲 Hierarchy", |ui| {
                            ...
                            if let Some(mut scene_state) = self.scene_state.take() {
                                ...
                                pending_actions = self.hierarchy_panel.take_pending_actions();
                                self.scene_state = Some(scene_state);
                            }

                            if !pending_actions.is_empty() {
                                self.process_hierarchy_actions(pending_actions);
                            }
                        });
                        ...
                        ui.collapsing("🎮 Entities", |ui| {
                            ...
                            let prefab_instance = selected_u32
                                .and_then(|entity_id| self.prefab_manager.find_instance(entity_id));

                            let component_edit = self.entity_panel.show_with_world(
                                ui,
                        ...
                        ui.collapsing("🔧 Transform", |ui| {
                            ...
                            if let Some(state) = self.scene_state() {
                                if let Some(transform) =
                                    selected_entity.and_then(|id| state.transform_for(id))
                                {
                            ...
                            let pending_transform = self.transform_panel.get_transform().cloned();
                            if let (Some(state), Some(selected_id), Some(new_transform)) =
                                (self.scene_state_mut(), selected_entity, pending_transform)
                            {
                                state.apply_transform(selected_id, &new_transform);
                            }
                        });
            ...
            if let Some(mut viewport) = self.viewport.take() {
                ...
                viewport_frame.show(ui, |ui| {
                    ...
                    ui.horizontal(|ui| {
                        ...
                        if ui
                            .checkbox(&mut self.snapping_config.grid_enabled, "Grid")
                            .changed()
                        {
                            self.emit_snapping_event();
                        }
                        ...
                        ui.separator();
                        if ui
                            .checkbox(&mut self.snapping_config.angle_enabled, "Angle")
                            .changed()
                        {
                            self.emit_snapping_event();
                        }
                    ...
                    let mut scene_state =
                        self.scene_state.take().expect("scene state ensured above");

                    let viewport_result = viewport.ui(ui, &mut scene_state, &mut self.undo_stack);
                    self.scene_state = Some(scene_state);
                ...
                let toolbar = viewport.toolbar().clone();
                self.snapping_config.grid_enabled = toolbar.snap_enabled;
                self.snapping_config.grid_size = toolbar.snap_size;
                self.snapping_config.angle_enabled = toolbar.angle_snap_enabled;
                self.snapping_config.angle_increment = toolbar.angle_snap_degrees;
