...
use aw_editor_lib::game_project::{AssetSettings, BuildSettings, GameProject, ProjectMetadata};
use std::fs;
...
fn test_project_lifecycle_end_to_end() {
    ...
    let loaded_project: GameProject =
        toml::from_str(&loaded_toml).expect("Failed to deserialize project");
    ...
    if let Ok(watcher) = FileWatcher::new(materials_dir.to_str().unwrap()) {
        ...
        while let Ok(event) = watcher.receiver.try_recv() {
            if let ReloadEvent::Material(path) = event {
                ...
                if let (Ok(p1), Ok(p2)) = (
                    ...
                    fs::canonicalize(&new_material_path),
                ) {
                    if p1 == p2 {
                        event_received = true;
                        break;
        ...
        assert!(
            event_received,
            "FileWatcher failed to detect new material creation"
        );
    } else {
