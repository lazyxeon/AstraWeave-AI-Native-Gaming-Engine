use anyhow::Result;
use astraweave_ai::orchestrator::{Orchestrator, RuleOrchestrator, UtilityOrchestrator, GoapOrchestrator};
use astraweave_core::{WorldSnapshot, IVec2, PlayerState, CompanionState, EnemyState, PlanIntent};
...
fn snapshot_rule_prefers_smoke_when_ready() -> Result<()> {
    ...
    assert!(json.contains("\"Throw\""), "expected Throw in plan: {}", json);
    Ok(())
}
...
fn snapshot_utility_advances_when_smoke_blocked() -> Result<()> {
    ...
    let json = to_json(&plan)?;
    assert!(json.contains("\"MoveTo\""), "expected MoveTo in plan: {}", json);
    Ok(())
}
...
fn snapshot_goap_independent_moves_and_covers() -> Result<()> {
    ...
    assert!(to_json(&p1)?.contains("\"MoveTo\""), "expected plan for far enemy to include MoveTo: {}", to_json(&p1)?);
    assert!(to_json(&p2)?.contains("\"CoverFire\""), "expected plan for close enemy to include CoverFire: {}", to_json(&p2)?);
    Ok(())
}
