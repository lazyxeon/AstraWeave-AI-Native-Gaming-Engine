...
fn bench_hash_many_small(c: &mut Criterion) {
    ...
    for count in [10, 100, 1000].iter() {
        ...
        group.bench_with_input(BenchmarkId::from_parameter(count), &chunks, |b, chunks| {
            b.iter(|| {
                let hashes: Vec<_> = black_box(chunks).iter().map(|c| compute_hash(c)).collect();
                black_box(hashes)
            })
        });
    }
...
criterion_group!(clone_benches, bench_hash_clone, bench_hash_to_string,);
...
criterion_group!(throughput_benches, bench_hash_throughput_mb_per_sec,);

criterion_group!(utility_benches, bench_hash_collision_check,);
