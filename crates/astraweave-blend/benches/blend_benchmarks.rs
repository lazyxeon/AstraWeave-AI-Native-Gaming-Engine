...
fn bench_options_deserialize_json(c: &mut Criterion) {
    ...
    c.bench_function("options_deserialize_json", |b| {
        b.iter(|| {
            black_box(serde_json::from_str::<ConversionOptions>(black_box(
                &serialized,
            )))
        })
    });
...
fn bench_error_creation(c: &mut Criterion) {
    ...
    group.bench_function("blender_not_found", |b| {
        b.iter(|| {
            black_box(BlendError::BlenderNotFound {
...
fn bench_error_display(c: &mut Criterion) {
    ...
    for (i, err) in errors.iter().enumerate() {
        group.bench_with_input(BenchmarkId::from_parameter(i), err, |b, e| {
            b.iter(|| black_box(format!("{}", e)))
        });
    }
    group.finish();
}
...
fn bench_options_memory_allocation(c: &mut Criterion) {
    ...
    group.bench_function("vec_allocation_10", |b| {
        b.iter(|| {
            let opts: Vec<ConversionOptions> =
                (0..10).map(|_| ConversionOptions::game_runtime()).collect();
            black_box(opts)
        })
    });
...
criterion_group!(error_benches, bench_error_creation, bench_error_display,);
