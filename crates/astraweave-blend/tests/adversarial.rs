...
fn malformed_blend_contents() -> Vec<(&'static str, Vec<u8>)> {
    vec![
        ...
        (
            "text_as_binary",
            b"This is not a blend file but looks like text".to_vec(),
        ),
        // Other file formats
        (
            "png_header",
...
fn options_timeout_extremes() {
    // Zero timeout
    let opts_zero = ConversionOptions::builder().timeout(Duration::ZERO).build();
...
fn deserialize_invalid_ron() {
    let invalid_rons = vec!["", "(", "()", "nil", "[]", "(invalid)"];
...
fn hash_length_independence() {
    ...
    assert_eq!(
        unique.len(),
        hashes.len(),
        "Length variations should produce unique hashes"
    );
}
...
fn all_texture_formats_valid() {
    ...
    for format in &formats {
        ...
        if *format != TextureFormat::Original {
            assert!(
                !ext.is_empty(),
                "Non-original format {:?} should have extension",
                format
            );
        } else {
            assert!(
                ext.is_empty(),
                "Original format should return empty extension"
            );
        }
    }
}
