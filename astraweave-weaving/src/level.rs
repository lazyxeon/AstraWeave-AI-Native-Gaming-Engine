...
impl VeilweaverLevel {
    ...
    pub fn new() -> Self {
        ...
        quest_manager.activate_quest("stabilize_anchors")
    ...
    fn try_activate_next_quest(&mut self) {
        ...
        let has_stabilize = self.quest_manager.quest("stabilize_anchors")
            .map(|q| matches!(q.state, crate::quest::QuestState::Completed))
            .unwrap_or(false);
        let has_clear = self.quest_manager.quest("clear_corruption")
            .map(|q| matches!(q.state, crate::quest::QuestState::Completed))
            .unwrap_or(false);
...
mod tests {
    ...
    fn test_level_exploration() {
        ...
        let explored = level.check_exploration(
