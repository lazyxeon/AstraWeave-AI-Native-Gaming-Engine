...
use crate::{
    Anchor, Enemy, EnemyState, EnemyBehavior, CombatSystem, CombatEvent, Killer,
    EnemySpawner, AnchorVfxState,
};
use glam::Vec3;
...
fn test_enemy_attacks_anchor_reduces_stability() {
    ...
    let behavior = enemy.update(0.016, anchor_pos + Vec3::new(2.0, 0.0, 0.0), Vec3::new(50.0, 0.0, 50.0), &[(0, anchor_pos)]);
...
fn test_enemy_prioritizes_broken_anchors() {
    ...
    let behavior = enemy.update(0.016, Vec3::new(12.0, 0.0, 6.0), player_pos, &broken_anchors);
...
fn test_player_kills_enemy_near_anchor_reduces_stress() {
    ...
    assert!(matches!(final_event, Some(CombatEvent::EnemyKilled { killer: Killer::Player, .. })));
...
fn test_player_echo_dash_kills_multiple_enemies() {
    ...
    let kill_count = kill_events.iter().filter(|e| matches!(e, CombatEvent::EnemyKilled { .. })).count();
...
fn test_spawner_increases_difficulty_when_anchors_break() {
    ...
    assert_eq!(broken.vfx_state(), AnchorVfxState::Broken, "Anchor with 0.05 stability should be Broken");
    ...
    assert_eq!(initial_difficulty, 1.5, "Initial difficulty should be 1.5 with 1 broken anchor (stability {}, state {:?})", broken.stability(), broken.vfx_state());
    ...
    assert_eq!(new_difficulty, 2.0, "New difficulty should be 2.0 with 2 broken anchors");
}
...
fn test_full_gameplay_loop() {
    ...
    assert_eq!(enemy.state, EnemyState::AttackAnchor, "Enemy should attack anchor when player is far");
    assert!(matches!(behavior, EnemyBehavior::MoveTo(_) | EnemyBehavior::Attack(_)));
...
fn test_enemy_transitions_from_patrol_to_attack_anchor() {
    ...
    let behavior = enemy.update(0.016, Vec3::new(10.0, 0.0, 5.0), player_pos, &broken_anchors);
