...
fn apply_combat_stress_to_nearby_anchors(
    anchors: &mut [&mut Anchor],
    event_pos: (f32, f32, f32),
) {
...
mod tests {
    use super::*;
    use crate::anchor::{Anchor, AbilityType};
    ...
    fn test_passive_decay_60_seconds() {
        ...
        let expected = 0.99;
        assert!((anchor.stability() - expected).abs() < 0.001,
                "Expected ~{}, got {}", expected, anchor.stability());
    }
    ...
    fn test_decay_stops_at_zero() {
        ...
        assert_eq!(anchor.stability(), 0.0, "Anchor should decay to 0.0 and stop");
    }
    ...
    fn test_combat_stress_multiple_events() {
        ...
        let events = vec![
            CombatEvent { position: (0.0, 0.0, 0.0), event_type: CombatEventType::EnemyKilled },
            CombatEvent { position: (5.0, 0.0, 0.0), event_type: CombatEventType::EnemyKilled },
            CombatEvent { position: (10.0, 0.0, 0.0), event_type: CombatEventType::EnemyKilled },
        ];
    ...
    fn test_non_kill_events_ignored() {
        ...
        let events = vec![
            CombatEvent { position: (0.0, 0.0, 0.0), event_type: CombatEventType::PlayerDamaged },
            CombatEvent { position: (0.0, 0.0, 0.0), event_type: CombatEventType::AbilityUsed },
        ];
