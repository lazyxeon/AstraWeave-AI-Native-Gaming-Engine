...
fn test_intent_budget_check() {
    ...
    assert_eq!(
        approved.len(),
        1,
        "Cheap intent should be approved within budget"
    );
    ...
    assert_eq!(
        approved.len(),
        0,
        "Expensive intent (25) should exceed budget (20) and be rejected"
    );
}
...
fn test_multiple_intent_budget_allocation() {
    ...
    assert!(
        approved.len() >= 2,
        "Should approve at least 2 intents within budget"
    );

    // Higher priority intents should be approved first
    assert!(
        approved.iter().any(|i| i.kind == "intent_a"),
        "Highest priority intent should be approved"
    );
}
...
fn test_intent_cooldown_enforcement() {
    ...
    let intent = common::create_test_intent("test_action", 0.9, 10).with_cooldown("test_cooldown");

    // First approval should succeed
    let approved = adjudicator.adjudicate(vec![intent.clone()]);
    assert_eq!(approved.len(), 1, "First intent should be approved");
    assert!(
        adjudicator.is_on_cooldown("test_cooldown"),
    ...
    adjudicator.begin_tick();
    let intent2 =
        common::create_test_intent("test_action_2", 0.9, 10).with_cooldown("test_cooldown");
    let approved2 = adjudicator.adjudicate(vec![intent2]);
    assert_eq!(
        approved2.len(),
        0,
        "Second intent should be blocked by cooldown"
    );
}
...
fn test_budget_constraints_during_combat() {
    ...
    assert!(
        approved.len() >= 2,
        "Should approve at least emergency heal + attack boost"
    );
    assert_eq!(
        adjudicator.budget_remaining(),
        0,
...
fn test_intent_priority_ordering() {
    ...
    assert_eq!(
        approved.len(),
        3,
        "All intents should be approved within budget"
    );
...
fn test_budget_reset_per_tick() {
    ...
    assert_eq!(
        adjudicator.budget_remaining(),
        20,
        "Budget should reset to 20 per tick"
    );
}
...
fn test_cooldown_decay_over_ticks() {
    ...
    let intent = common::create_test_intent("test", 0.9, 5).with_cooldown("test_cooldown");
    ...
    assert!(
        remaining < initial_cooldown,
        "Cooldown should decay over ticks"
    );
}
...
fn test_multi_intent_simultaneous_processing() {
    ...
    assert_eq!(
        approved.len(),
        4,
        "Should approve exactly 4 intents to fill budget"
    );
    assert_eq!(
        adjudicator.budget_spent(),
        20,
...
fn test_min_priority_filter() {
    ...
    assert_eq!(
        approved.len(),
        1,
        "Only above-threshold intent should be approved"
    );
    assert!(
        !approved.iter().any(|i| i.kind == "below_threshold"),
        "Below threshold intent should be filtered"
    );
}
...
fn test_deterministic_intent_ordering() {
    ...
    assert_eq!(
        approved1.len(),
        approved2.len(),
        "Approval count should be deterministic"
    );
    for i in 0..approved1.len() {
        assert_eq!(
            approved1[i].kind, approved2[i].kind,
