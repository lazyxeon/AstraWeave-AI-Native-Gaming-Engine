...
fn test_resource_scarcity_gradual_depletion() {
    ...
    assert!(patterns3[0].1 > patterns2[0].1, "Strength should increase with scarcity");
}
...
fn test_faction_conflict_multi_faction() {
    ...
    assert!(patterns.iter().any(|(id, _)| id == "faction_conflict_rebels"));
    assert!(patterns.iter().any(|(id, _)| id == "faction_conflict_empire"));
...
fn test_pattern_detection_performance_100_entities() {
    ...
    assert!(elapsed.as_millis() < 10, "Detection too slow: {:?}", elapsed);
}
...
fn test_pattern_detection_performance_1000_entities() {
    ...
    assert!(elapsed.as_millis() < 100, "Detection too slow at scale: {:?}", elapsed);
}
