...
use astraweave_core::{World, IVec2, Team, PlanIntent, validation::{validate_and_execute, ValidateCfg}};
...
pub extern "C" fn aw_version() -> AWVersion {
    ...
    let major = parts.next().and_then(|s| s.parse::<u16>().ok()).unwrap_or(0);
    let minor = parts.next().and_then(|s| s.parse::<u16>().ok()).unwrap_or(0);
    let patch = parts.next().and_then(|s| s.parse::<u16>().ok()).unwrap_or(0);
    AWVersion { major, minor, patch }
}
...
pub extern "C" fn aw_version_string(buf: *mut u8, len: usize) -> usize {
    ...
    if buf.is_null() || len == 0 { return bytes.len() + 1; }
    unsafe {
...
pub extern "C" fn aw_world_create() -> AWWorld {
    ...
    let boxed = Box::new(AwWorldWrap{ world: w, snapshot_cb: None, delta_cb: None, prev: HashMap::new() });
    AWWorld(Box::into_raw(boxed))
}

#[no_mangle]
pub extern "C" fn aw_world_destroy(_w: AWWorld) {
    if !_w.0.is_null() { unsafe { let _ = Box::from_raw(_w.0); } }
}

#[no_mangle]
pub extern "C" fn aw_world_tick(_w: AWWorld, _dt: f32) {
    if _w.0.is_null() { return; }
    ...
    if let Some(cb) = wrap.snapshot_cb {
        ...
        if let Ok(s) = serde_json::to_string(&snap) {
            unsafe {
                use std::ffi::CString;
                if let Ok(cs) = CString::new(s) { cb(cs.as_ptr()); }
            }
        }
    }
    ...
    if let Some(cb) = wrap.delta_cb {
        ...
        for id in wrap.prev.keys() {
            if !cur.contains_key(id) { removed.push(*id); }
        }
        wrap.prev = cur; // update baseline
        let d = SimpleDelta{ t: wrap.world.t, changed, removed };
...
struct SimpleEntity { id: u32, x: i32, y: i32, team: u8, hp: i32, ammo: i32 }
...
fn crate_snapshot(w: &World) -> SimpleSnapshot {
    let entities = w
        ...
        .filter_map(|id| {
            ...
            Some(SimpleEntity{ id, x: pos.x, y: pos.y, team, hp, ammo })
        })
        .collect();
    SimpleSnapshot{ t: w.t, tick: 0, entities }
}
...
fn current_map(w: &World) -> HashMap<u32, SimpleEntity> {
    ...
    for team in [0u8, 1u8, 2u8] {
        for id in w.all_of_team(team) {
            if let (Some(pos), Some(t), Some(h), Some(am)) = (w.pos_of(id), w.team(id), w.health(id), w.ammo(id)) {
                map.insert(id, SimpleEntity{ id, x: pos.x, y: pos.y, team: t.id, hp: h.hp, ammo: am.rounds });
            }
        }
    }
    map
...
fn set_last_error(msg: &str) {
    ...
    if let Ok(mut s) = m.lock() { *s = msg.to_string(); }
}

#[no_mangle]
pub extern "C" fn aw_last_error_string(buf: *mut u8, len: usize) -> usize {
    let s = LAST_ERROR.get().and_then(|m| m.lock().ok().map(|s| s.clone())).unwrap_or_default();
    write_cstr(s.as_bytes(), buf, len)
}

#[no_mangle]
pub extern "C" fn aw_world_submit_intent_json(_w: AWWorld, actor_id: u32, intent_json: *const c_char, cb: CStringCallback) -> i32 {
    if _w.0.is_null() { set_last_error("null world handle"); return AW_ERR_NULL; }
    let wrap = unsafe { &mut *(_w.0) };
    let cstr = unsafe { if intent_json.is_null() { set_last_error("intent_json is null"); return AW_ERR_PARAM; } CStr::from_ptr(intent_json).to_string_lossy().to_string() };
    let intent: Result<PlanIntent, _> = serde_json::from_str(&cstr);
    let intent = match intent { Ok(i) => i, Err(e) => { set_last_error(&format!("parse error: {}", e)); return AW_ERR_PARSE; } };
    let mut log = |s: String| { if let Some(func) = cb { unsafe { use std::ffi::CString; if let Ok(cs) = CString::new(s) { func(cs.as_ptr()); } } } };
    let cfg = ValidateCfg{ world_bounds: (0,0,19,9) };
    match validate_and_execute(&mut wrap.world, actor_id, &intent, &cfg, &mut log) {
        Ok(_) => { set_last_error(""); AW_OK },
        Err(e) => { set_last_error(&format!("exec error: {}", e)); AW_ERR_EXEC },
    }
}
...
mod tests {
    ...
    unsafe extern "C" fn snap_cb(_msg: *const c_char) { SNAP_COUNT.fetch_add(1, Ordering::SeqCst); }
    static DELTA_COUNT: AtomicUsize = AtomicUsize::new(0);
    unsafe extern "C" fn delta_cb(_msg: *const c_char) { DELTA_COUNT.fetch_add(1, Ordering::SeqCst); }
    #[test]
    fn c_abi_version() {
        let v = aw_version();
        assert_eq!(v.major, 0);
    ...
    fn last_error_is_set_on_intent_parse_error() {
        let w = aw_world_create();
        let rc = unsafe { aw_world_submit_intent_json(w, 1, std::ffi::CString::new("not json").unwrap().as_ptr(), None) };
        assert_eq!(rc, AW_ERR_PARSE);
        let mut buf = [0u8; 128];
        let n = aw_last_error_string(buf.as_mut_ptr(), buf.len());
        assert!(n > 1);
        let s = std::ffi::CStr::from_bytes_until_nul(&buf).unwrap().to_string_lossy().into_owned();
