use anyhow::Result;
use astraweave_embeddings::{Memory, MemoryCategory, MockEmbeddingClient, VectorStore};
...
async fn test_retrieve_memory() -> Result<()> {
    ...
    // Add a memory
    pipeline
        .add_memory("The dragon breathes fire".to_string())
        .await?;
...
async fn test_retrieve_with_category_filter() -> Result<()> {
    ...
    let query =
        astraweave_rag::MemoryQuery::text("interaction").with_category(MemoryCategory::Combat);
...
async fn test_context_injection() -> Result<()> {
    ...
    config.injection.injection_template =
    ...
    pipeline
        .add_memory("The secret code is 1234".to_string())
        .await?;
...
async fn test_consolidation_trigger() -> Result<()> {
    ...
    assert_eq!(
        metrics_1.consolidations_performed, 0,
        "Should not consolidate after 1"
    );

    pipeline.add_memory("Mem 2".to_string()).await?;
    let metrics_2 = pipeline.get_metrics();
    assert_eq!(
        metrics_2.consolidations_performed, 0,
        "Should not consolidate after 2"
    );
    ...
    assert_eq!(
        metrics_3.consolidations_performed, 1,
        "Should consolidate after 3"
    );
...
async fn test_retrieve_with_entity_filter() -> Result<()> {
    ...
    let query = astraweave_rag::MemoryQuery::text("found").with_entity("sword");
    ...
    for r in &results {
        println!(
            "Retrieved: {} Entities: {:?}",
            r.memory.text, r.memory.entities
        );
    }
    ...
    for result in &results {
        assert!(
            result.memory.entities.contains(&"sword".to_string()),
            "Memory {} should contain sword",
            result.memory.id
        );
    }
...
async fn test_async_add_memory() -> Result<()> {
    ...
    let h1 = tokio::spawn(async move { p1.add_memory_async("Async memory 1".to_string()).await });

    let p2 = pipeline.clone();
    let h2 = tokio::spawn(async move { p2.add_memory_async("Async memory 2".to_string()).await });
...
async fn test_diversity_strategy_semantic() -> Result<()> {
    ...
    pipeline
        .add_memory("The cat is sleeping peacefully".to_string())
        .await?;
    pipeline
        .add_memory("The cat is resting quietly".to_string())
        .await?; // Very similar
    pipeline
        .add_memory("The dragon breathes fire".to_string())
        .await?; // Different
