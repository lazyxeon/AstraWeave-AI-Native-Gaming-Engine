# AstraWeave: AI-Native Game Engine — AI Context Constitution

**The Definitive Guide for All AI Agents Working on AstraWeave**

Read this document first when working in this codebase. This serves as the authoritative context constitution for all AI agents (GitHub Copilot, Claude, GPT, Gemini, and others) contributing to this project.

---

## Preamble: The AstraWeave Mission

**AstraWeave is not just a game engine. It is a scientific proof of concept and a testament to AI-driven software development.**

This entire engine is being developed **iteratively by AI with ZERO human-written code**. This is a **frontier experiment** to prove AI's capability to build production-ready, mission-critical systems end-to-end through iterative prompting and refinement.

---

## Article I: Inviolable Principles

These principles are **non-negotiable** and must never be compromised:

### 1.1 Zero Human Code Policy
- All code must be generated by AI agents. The human user is the **prompter and director**, not the implementer.
- If stuck, ask for clarification or suggest alternatives—but never request the user to write code.

### 1.2 Mission-Critical Standards
- Treat every line of code as if it were controlling a spacecraft.
- **90%+ confidence** is required before marking any task complete.
- "It compiles" is never sufficient. Functionality must be verified through testing.

### 1.3 Security First
- Never introduce code that exposes secrets, keys, or sensitive data.
- Never commit credentials to the repository.
- Validate all external inputs. Assume all user input is malicious.

### 1.4 Determinism Guarantee
- The engine must produce bit-identical results given the same inputs and seed.
- All randomness must use seeded RNG (`rand_chacha`).
- Entity iteration order must be deterministic.

### 1.5 Production Quality
- No "toy" code, no placeholders, no TODOs left unresolved in production paths.
- All systems must be scalable, performant, and maintainable.

---

## Article II: AI Agent Collaboration Protocol

### 2.1 Specialized Agent Roles

AstraWeave leverages multiple specialized AI agents. Each agent should understand its role:

| Agent Role | Primary Responsibilities | Key Focus Areas |
|------------|-------------------------|-----------------|
| **Code-Reviewer** | Code quality, security audits, best practices | Catch bugs, enforce patterns, review PRs |
| **Explorer** | Codebase navigation, context gathering | Find relevant code, trace dependencies |
| **Verifier** | Testing, validation, CI/CD | Run tests, verify builds, check coverage |
| **Maintainer** | Documentation, cleanup, organization | Keep docs current, organize structure |
| **Auditor** | Compliance, quality gates, metrics | Track coverage, benchmark performance |
| **ToT-Reasoner** | Complex problem solving, architecture | Multi-step reasoning, design decisions |
| **Engine-Verifier** | Engine-specific validation | ECS, physics, rendering correctness |
| **Render-Asset-Guardian** | Rendering pipeline, asset management | Shaders, materials, textures, PBR |
| **Rust-Engine-Architect** | Rust-specific patterns, performance | Memory safety, zero-cost abstractions |

### 2.2 Session Handoff Protocol

When ending a session or handing off to another agent:

1. **Document State**: Update relevant master reports (see Master Report Maintenance Protocol below)
2. **Summarize Progress**: Create a session summary in `docs/journey/daily/`
3. **List Blockers**: Clearly document any unresolved issues
4. **Specify Next Steps**: Provide actionable next tasks
5. **Update TODOs**: Mark completed items, add new discovered tasks

### 2.3 Multi-Agent Coordination

When multiple agents work on related systems:

1. **Check Master Reports First**: `docs/masters/` contains authoritative state
2. **Avoid Conflicting Changes**: Coordinate through session reports
3. **Maintain Consistency**: Follow established patterns and conventions
4. **Cross-Reference**: Link related documentation across agent sessions

### 2.4 Communication Standards

- Be precise and technical in documentation
- Use consistent terminology (see Glossary in GitHub Pages: `docs/src/reference/glossary.md`)
- Include code examples for complex concepts
- Reference specific file paths with line numbers: `file.rs:123`
...
- [ ] `cargo check -p <crate>` passes with 0 errors
- [ ] `cargo clippy -p <crate>` shows no new warnings
- [ ] `cargo test -p <crate>` passes (if tests exist)
...
Rate your confidence before marking tasks complete:

| Level | Confidence | Action Required |
|-------|------------|-----------------|
| A+ | 95-100% | Ready to ship |
| A | 90-94% | Ship with monitoring |
| B | 80-89% | Needs additional testing |
| C | 70-79% | Requires review before ship |
| D | <70% | Do not ship, needs rework |

### 3.3 Verification Hierarchy

1. **Compilation**: Must pass (non-negotiable)
2. **Unit Tests**: Should pass (critical path)
3. **Integration Tests**: Should pass (system validation)
4. **Benchmarks**: Should meet performance budgets
5. **Manual Validation**: Run examples to verify functionality

---

## Article IV: Core Directives

**Your Mandate as an AI Agent**:
...
## Article V: Inviolable "Do Not" Rules

These prohibitions are **absolute**. Violating any of these will result in broken builds, lost work, or degraded quality.

### Code Quality

| Rule | Reason |
|------|--------|
| **Never use `.unwrap()` in production paths** | Use `anyhow::Result`, `.ok()`, `.unwrap_or_default()`, or `?` operator. Panics crash the engine. |
| **Never leave compilation errors unfixed** | All code must pass `cargo check` before ending a session. Broken builds block all other work. |
| **Never commit code with new warnings** | Run `cargo clippy` and fix all warnings. Zero-warning policy is enforced. |
| **Never use `unsafe` without justification** | Document all `unsafe` blocks with `// SAFETY:` comments explaining invariants. |
| **Never hardcode paths or magic numbers** | Use constants, configuration, or resources. Hardcoded values break cross-platform builds. |

### Documentation

| Rule | Reason |
|------|--------|
| **Never create docs at root level** | All documentation goes in `docs/` subdirectories. Root directory is for code and config only. |
| **Never skip master report updates** | `MASTER_ROADMAP.md`, `MASTER_BENCHMARK_REPORT.md`, `MASTER_COVERAGE_REPORT.md`, `MASTER_API_PATTERNS.md` must reflect current state. |
| **Never assume context without checking master reports** | Always read `docs/masters/` before starting significant work. |
| **Never leave TODOs in production code** | Either complete the work or remove the TODO. No "fix later" allowed. |

### Architecture

| Rule | Reason |
|------|--------|
| **Never break the ECS pattern** | All game state flows through ECS components. No global mutable state. |
| **Never introduce circular dependencies** | Crates must form a DAG. Use dependency injection if needed. |
| **Never skip determinism checks** | Randomness must use seeded RNG (`rand_chacha`). Entity order must be deterministic. |
| **Never ignore test failures** | Tests exist for a reason. Fix the code, not the test. |

### Security

| Rule | Reason |
|------|--------|
| **Never commit secrets or credentials** | Use environment variables or secure configuration. |
| **Never trust user input** | Validate all external data. Assume malicious intent. |
| **Never expose internal paths in logs** | Sanitize logging output. No filesystem paths in user-facing messages. |

### Quick Reference

```
NEVER: .unwrap() in production | compilation errors | new warnings | unsafe without SAFETY
NEVER: docs at root | skip master reports | assume context | leave TODOs
NEVER: break ECS | circular deps | non-determinism | ignore test failures
NEVER: commit secrets | trust input | expose paths
```
...
**Status: Phase 8+ Active | Editor Enhancements | Benchmark Infrastructure**

### Recent Work (December 2025 - January 2026)

- ✅ **Phase 14-15: Editor Enhancements & Code Quality** (Jan 2026)
   - **Editor Panel Headers**: Entity counts, asset counts, message counts, undo/redo counts
   - **Graph Node Count**: Behavior graph editor header improvements
   - **Profiler Enhancements**: Simulation tick count, runtime state display
   - **Inspector Improvements**: Selected entity name in header
   - **Code Quality**: Deprecated API migrations (`criterion::black_box` → `std::hint::black_box`)
   - **Clippy Fixes**: Production readiness across all crates

- ✅ **Comprehensive Render Benchmark Suite** (Dec 2025)
   - **Animation/Skinning Benchmarks**: GPU skinning performance validation
   - **Culling/LOD Benchmarks**: Visibility and level-of-detail testing
   - **Render Infrastructure Audit**: Complete audit report generated

- ✅ **GitHub Pages Documentation Production-Ready** (Jan 2026)
   - **20+ Pages Complete**: All documentation stubs filled in
   - **mdBook 0.5.2 Compatible**: Clean builds with no errors
   - **Zero Placeholders**: All content production-ready
...
- ✅ **Phase 6: Real LLM Integration COMPLETE** (Oct 14, 2025)
   ...
   - **Documentation**: docs/archive/PHASE_6_COMPLETION_SUMMARY.md (comprehensive 15k-word report)
...
- 🎯 **Phase 8: Game Engine Readiness IN PROGRESS** (Oct 14-15, 2025)
   ...
   - **Documentation**: 
     - `docs/archive/GAME_ENGINE_READINESS_ROADMAP.md` - Overall strategy
     - `docs/archive/PHASE_8_ROADMAP_REVIEW.md` - Roadmap validation vs actual codebase
     - `docs/archive/PHASE_8_PRIORITY_1_UI_PLAN.md` - 5-week UI implementation (egui-wgpu)
     - `docs/archive/PHASE_8_PRIORITY_2_RENDERING_PLAN.md` - 4-5 week rendering completion
     - `docs/archive/PHASE_8_PRIORITY_3_SAVE_LOAD_PLAN.md` - 2-3 week save/load system
     - `docs/archive/PHASE_8_PRIORITY_4_AUDIO_PLAN.md` - 2-3 week production audio
     - `docs/archive/PHASE_8_MASTER_INTEGRATION_PLAN.md` - **START HERE** for coordination
...
**CRITICAL REQUIREMENT**: AstraWeave maintains four authoritative master reports that MUST be updated on ANY significant change:

### 1. Master Roadmap (`docs/masters/MASTER_ROADMAP.md`)
**Update when**:
...
**Update process**:
1. Open `docs/masters/MASTER_ROADMAP.md`
...
### 2. Master Benchmark Report (`docs/masters/MASTER_BENCHMARK_REPORT.md`)
**Update when**:
...
**Update process**:
1. Open `docs/masters/MASTER_BENCHMARK_REPORT.md`
...
### 3. Master Coverage Report (`docs/masters/MASTER_COVERAGE_REPORT.md`)
**Update when**:
...
**Update process**:
1. Open `docs/masters/MASTER_COVERAGE_REPORT.md`
...
### 4. Master API Patterns (`docs/masters/MASTER_API_PATTERNS.md`)
**Update when**:
- Adding new ActionStep variants
- Changing schema types (WorldSnapshot, PlanIntent, etc.)
- Adding new ECS patterns (Components, Resources, Events)
- Modifying AI/LLM integration APIs
- Changing serialization formats
- **Update threshold**: Any public API change

**Update process**:
1. Open `docs/masters/MASTER_API_PATTERNS.md`
2. Update relevant section with new/changed API
3. Add code examples for new patterns
4. Update anti-patterns if new mistakes discovered
5. Update Quick Reference tables
6. Increment version number
7. Add entry to "Changelog" table

### Enforcement

**This is a HARD RULE**:
- ALWAYS check if master reports need updating after completing work
- ALWAYS update all four reports if thresholds exceeded
...
**Performance Summary** (see docs/archive/BASELINE_METRICS.md + docs/archive/WEEK_8_FINAL_SUMMARY.md + docs/archive/PHASE_6_COMPLETION_SUMMARY.md):
...
|------------|---------|---------|
| wgpu | 25.0.2 | GPU rendering (Vulkan/DX12/Metal) |
| egui | 0.32 | Immediate-mode UI framework |
| egui-wgpu | 0.32 | egui wgpu backend |
| winit | 0.30 | Window handling |
| rapier3d | 0.22 | Physics engine |
| glam | 0.30 | Math library (SIMD) |
...
```
astraweave-core/        # Core types, schemas, WorldSnapshot, PlanIntent
...
astraweave-render/      # wgpu 25.0.2 renderer, materials, IBL, GPU skinning, mesh optimization
astraweave-physics/     # Rapier3D 0.22 wrapper, character controller, spatial hash
astraweave-gameplay/    # Combat physics, attack sweep
astraweave-nav/         # Navmesh, A*, portal graphs
astraweave-audio/       # Spatial audio, rodio 0.17 backend
...
astraweave-math/        # SIMD vector/matrix operations (glam 0.30), movement optimization
astraweave-fluids/      # GPU fluid simulation
```

**AI & LLM Crates**:

```
astraweave-llm/         # LLM integration (Hermes 2 Pro via Ollama)
astraweave-embeddings/  # Text embeddings for semantic search
astraweave-context/     # Context management for LLM
astraweave-prompts/     # Prompt engineering utilities
astraweave-rag/         # Retrieval-augmented generation
astraweave-persona/     # NPC personality system
astraweave-memory/      # Memory management for AI agents
...
astraweave-quests/      # Quest system
astraweave-npc/         # NPC management
```

**Tooling & Editor**:

```
tools/aw_editor/        # Level/encounter editor (GUI, egui 0.32)
tools/aw_asset_cli/     # Asset pipeline tooling
tools/astraweave-assets/ # Autonomous PolyHaven asset fetcher
tools/aw_texture_gen/   # Texture generation
tools/aw_demo_builder/  # Demo creation utilities
crates/astract/         # React-style UI widget library
crates/astraweave-blend/ # Blender file import
```

**Persistence & Networking**:

```
persistence/aw-save/    # Save game system
astraweave-persistence-ecs/ # ECS serialization
net/aw-net-proto/       # Network protocol definitions
net/aw-net-server/      # Game server
net/aw-net-client/      # Game client
astraweave-security/    # Anti-cheat, validation
```

**Examples** (`examples/`):

- ✅ Working: `hello_companion`, `unified_showcase`, `core_loop_bt_demo`, `core_loop_goap_demo`, `weaving_pcg_demo`, `profiling_demo`, `shadow_csm_demo`, `fluids_demo`, `astract_gallery`
- ⚠️ API Drift: `ui_controls_demo`, `debug_overlay` (egui/winit version mismatches - being fixed)
...
1. **docs/archive/GAME_ENGINE_READINESS_ROADMAP.md** (NEW - October 14, 2025)
...
3. **docs/archive/LONG_HORIZON_STRATEGIC_PLAN.md** (12,000 words)
...
5. **docs/archive/PHASE_7_VALIDATION_REPORT.md** (Completion Summary)
...
7. **docs/archive/PHASE_6_COMPLETION_SUMMARY.md** (15,000 words)
...
8. **docs/archive/PHASE_7_TOOL_EXPANSION_PLAN.md** (26,000 words)
...
10. **docs/archive/PHASE_8_ROADMAP_REVIEW.md** (Roadmap Validation)
...
- **docs/archive/UNWRAP_AUDIT_ANALYSIS.md** - 637 `.unwrap()` calls cataloged (342 P0 critical)
- **docs/archive/BASELINE_METRICS.md** - Performance baselines (all subsystems)
- **docs/archive/AI_NATIVE_VALIDATION_REPORT.md** - 28 tests, A+ grade, 12,700+ capacity proven
...
**Code Quality Audits** (docs/archive/):

8. **docs/archive/UNWRAP_AUDIT_ANALYSIS.md** (Week 6)
...
9. **docs/archive/HERMES2PRO_MIGRATION_PHASE1_AUDIT.md**
   - LLM migration audit (Phi-3 → Hermes 2 Pro)
   - Root cause analysis

10. **docs/archive/AUDIT_PHASE_1_COMPLETE.md**
...
- **Strategic Plans**: `docs/archive/IMPLEMENTATION_PLANS_INDEX.md` (roadmap navigation)
- **Phase 6 Status**: `docs/archive/PHASE_6_COMPLETION_SUMMARY.md` (latest achievements)
- **Phase 7 Roadmap**: `docs/archive/PHASE_7_TOOL_EXPANSION_PLAN.md` (next implementation)
...
- See `docs/archive/UNWRAP_AUDIT_ANALYSIS.md` for safe patterns and remediation plan
...
- [Phase 7 Arbiter Report](../docs/journey/phases/PHASE_7_ARBITER_PHASE_5_COMPLETE.md) - Completion summary
...
- **`.unwrap()` Usage**: 637 total occurrences cataloged (342 P0-Critical, 58 production unwraps fixed)
  - See `docs/archive/UNWRAP_AUDIT_ANALYSIS.md` for remediation plan
...
- See `docs/archive/BASELINE_METRICS.md` + `docs/archive/WEEK_8_FINAL_SUMMARY.md` + `docs/archive/PHASE_6_COMPLETION_SUMMARY.md` for full metrics
...
- `docs/archive/GAME_ENGINE_READINESS_ROADMAP.md` - **START HERE** for Phase 8-10 planning
- `docs/archive/IMPLEMENTATION_PLANS_INDEX.md` - Navigation guide for all strategic docs
- `docs/archive/COMPREHENSIVE_STRATEGIC_ANALYSIS.md` - Gap analysis with prioritized findings
- `docs/archive/LONG_HORIZON_STRATEGIC_PLAN.md` - 12-month strategic plan (Phases A, B, C)
- `docs/archive/PHASE_7_VALIDATION_REPORT.md` - Phase 7 completion summary and deferred work
- `docs/archive/HERMES2PRO_MIGRATION_PHASE3_CODE.md` - Hermes 2 Pro migration technical details
- `docs/archive/PHASE_6_COMPLETION_SUMMARY.md` - Phase 6 achievements and metrics
- `docs/archive/PHASE_7_TOOL_EXPANSION_PLAN.md` - Phase 7 implementation roadmap
...
Consult `docs/archive/GAME_ENGINE_READINESS_ROADMAP.md` for comprehensive roadmap and gap analysis.
...
**Version**: 0.4.0 | **Rust**: 1.89.0 | **License**: MIT | **Status**: Phase 8+ Active (Jan 2026)
