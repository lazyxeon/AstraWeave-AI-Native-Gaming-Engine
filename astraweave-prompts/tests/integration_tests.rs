...
use astraweave_prompts::optimization::{CacheConfig, TemplateCache};
use astraweave_prompts::{
    ContextValue, FormattingOptions, OptimizationConfig, OptimizationEngine, PromptAnalyzer,
    PromptContext, PromptFormatter, PromptLibrary, PromptTemplate, PromptValidator, PromptsConfig,
    TemplateCategory, TemplateEngine, TemplateMetadata, UsageStats, ValidationRules,
};
...
fn test_template_with_partial_workflow() {
    ...
    ctx.set(
        "title".to_string(),
        ContextValue::String("Important Document".to_string()),
    );
    ctx.set(
        "content".to_string(),
        ContextValue::String("This is the main content.".to_string()),
    );
...
fn test_nested_object_workflow() {
    ...
    ctx.set_path(
        "user.email",
        ContextValue::String("bob@example.com".to_string()),
    );
...
fn test_array_iteration_workflow() {
    ...
    let template = PromptTemplate::new("list", "Items:\n{{#each items}}- {{this}}\n{{/each}}");
...
fn test_template_with_metadata() {
    ...
    let template = PromptTemplate::new_with_metadata("greeting", "Hello {{name}}!", meta.clone());
...
fn test_unicode_template() {
    ...
    ctx.set(
        ...
        ContextValue::String("爱丽丝".to_string()),
    );
    ctx.set(
        "russian_name".to_string(),
        ContextValue::String("Алиса".to_string()),
    );
...
fn test_emoji_in_template() {
    ...
    let template = PromptTemplate::new("emoji", "Hello {{name}}! 👋 Have a great day! 🌟");
...
fn test_large_number_of_templates() {
    ...
    for i in 0..100 {
        ...
        engine
            .register_template(&format!("template_{}", i), template)
            .unwrap();
    }
...
fn test_deeply_nested_context() {
    ...
    let template = PromptTemplate::new("nested", "Value: {{level1.level2.level3.level4.value}}");
