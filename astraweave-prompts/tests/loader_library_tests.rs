...
use astraweave_prompts::{
    CollectionMetadata, LibraryMetadata, PromptLibrary, PromptLoader, PromptTemplate,
    TemplateCategory, TemplateCollection, TemplateLibrary, TemplateMetadata,
};
use std::fs;
...
fn test_loader_with_custom_extensions() {
    let loader = PromptLoader::new().with_extensions(vec!["txt".to_string(), "tpl".to_string()]);
