...
fn test_engine_list_templates() {
    let mut engine = TemplateEngine::new();
    engine
        .register_template("t1", PromptTemplate::new("t1", "A"))
        .unwrap();
    engine
        .register_template("t2", PromptTemplate::new("t2", "B"))
        .unwrap();
...
fn test_engine_thread_safety() {
...

    {
        ...
        e.register_template(
            "thread_test",
            PromptTemplate::new("thread_test", "Hello {{id}}"),
        )
        .unwrap();
    }
...
fn test_nested_variable_support() {
    ...
    user_obj.insert(
        "role".to_string(),
        ContextValue::String("Admin".to_string()),
    );
...
fn test_set_variable_types() {
    ...
    let t = PromptTemplate::new(
        "test",
        "{{string}} {{number}} {{float}} {{bool}} {{array.[0]}}",
    );
    let result = t.render(&ctx).unwrap();
    ...
    assert!(
        result.contains("1.234"),
        "Expected '1.234' in result: {result}"
    );
