...
use astraweave_prompts::{
    helpers::{
        FormattingOptions, PromptAnalyzer, PromptFormatter, PromptValidator, ValidationRules,
    },
...
fn test_helper_uppercase() {
    ...
    engine
        .register_template("upper", PromptTemplate::new("u", "{{uppercase text}}"))
        .unwrap();
...
fn test_helper_lowercase() {
    let mut engine = TemplateEngine::new();
    engine
        .register_template("lower", PromptTemplate::new("l", "{{lowercase text}}"))
        .unwrap();
...
fn test_helper_trim() {
    let mut engine = TemplateEngine::new();
    engine
        .register_template("trim", PromptTemplate::new("t", "{{trim text}}"))
        .unwrap();

    let mut ctx = PromptContext::new();
    ctx.set(
        "text".to_string(),
        ContextValue::String("  spaces  ".to_string()),
    );
...
fn test_helper_json() {
    let mut engine = TemplateEngine::new();
    engine
        .register_template("json", PromptTemplate::new("j", "{{json data}}"))
        .unwrap();
...
fn test_helper_length_string() {
    let mut engine = TemplateEngine::new();
    engine
        .register_template("len", PromptTemplate::new("l", "{{length text}}"))
        .unwrap();
...
fn test_helper_length_array() {
    let mut engine = TemplateEngine::new();
    engine
        .register_template("len", PromptTemplate::new("l", "{{length items}}"))
        .unwrap();
...
fn test_helper_indent_default() {
    let mut engine = TemplateEngine::new();
    engine
        .register_template("indent", PromptTemplate::new("i", "{{indent text}}"))
        .unwrap();
...
fn test_helper_indent_custom_spaces() {
    let mut engine = TemplateEngine::new();
    engine
        .register_template("indent", PromptTemplate::new("i", "{{indent text 4}}"))
        .unwrap();
