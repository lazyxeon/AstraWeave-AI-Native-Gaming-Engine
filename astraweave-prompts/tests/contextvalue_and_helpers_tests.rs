...
use astraweave_prompts::{ContextValue, PromptContext, PromptTemplate, TemplateEngine};
...
fn test_render_with_conditionals() {
    let template = PromptTemplate::new("cond", "{{#if active}}Active{{else}}Inactive{{/if}}");
...
fn test_render_with_iteration() {
    let template = PromptTemplate::new("iter", "{{#each items}}{{this}}, {{/each}}");
...
fn test_render_with_partial() {
    ...
    engine
        .register_template("page", PromptTemplate::new("p", "Content\n{{> footer}}"))
        .unwrap();
...
fn test_processor_invalid_handlebars_syntax() {
    use astraweave_prompts::{ProcessorConfig, TemplateProcessor};
...
fn test_context_overwrite_in_scope() {
    ...
    ctx.set(
        "key".to_string(),
        ContextValue::String("original".to_string()),
    );

    ctx.push_scope();
    ctx.set(
        "key".to_string(),
        ContextValue::String("scoped".to_string()),
    );
...
fn test_context_merge_overwrites() {
    ...
    ctx1.set(
        "shared".to_string(),
        ContextValue::String("ctx1".to_string()),
    );

    let mut ctx2 = PromptContext::new();
    ctx2.set(
        "shared".to_string(),
        ContextValue::String("ctx2".to_string()),
    );
...
fn test_template_with_metadata() {
    use astraweave_prompts::{TemplateCategory, TemplateMetadata};
...
fn test_npc_dialogue_generation() {
    ...
    npc.insert(
        "name".to_string(),
        ContextValue::String("Guard".to_string()),
    );
    npc.insert(
        "mood".to_string(),
        ContextValue::String("suspicious".to_string()),
    );

    ctx.set("npc".to_string(), ContextValue::Object(npc));
    ctx.set(
        "dialogue".to_string(),
        ContextValue::String("Halt! Who goes there?".to_string()),
    );
...
fn test_quest_description_with_conditions() {
    ...
    ctx.set(
        "title".to_string(),
        ContextValue::String("Save the Village".to_string()),
    );
    ctx.set("is_urgent".to_string(), ContextValue::Boolean(true));
    ctx.set(
        "description".to_string(),
        ContextValue::String("Bandits are attacking!".to_string()),
    );
    ctx.set(
        "reward".to_string(),
        ContextValue::String("500 gold".to_string()),
    );
...
fn test_combat_log_with_arrays() {
    ...
    event1.insert(
        "timestamp".to_string(),
        ContextValue::String("12:00".to_string()),
    );
    event1.insert(
        "message".to_string(),
        ContextValue::String("Battle started".to_string()),
    );

    let mut event2 = HashMap::new();
    event2.insert(
        "timestamp".to_string(),
        ContextValue::String("12:01".to_string()),
    );
    event2.insert(
        "message".to_string(),
        ContextValue::String("Player attacks".to_string()),
    );
...
fn test_large_number_of_variables() {
    ...
    assert!(
        ...
        "Expected at least some variables to be extracted"
    );
    assert!(
        template.variables.len() <= 100,
        "Should not extract more than the defined variables"
    );
}
...
fn test_large_array_rendering() {
    let template = PromptTemplate::new("large_array", "{{#each items}}{{this}} {{/each}}");
