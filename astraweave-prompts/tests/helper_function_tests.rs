...
fn test_uppercase_helper() {
    ...
    let template = PromptTemplate::new("upper", "Original: {{name}}, Upper: {{uppercase name}}");
...
fn test_lowercase_helper() {
    ...
    let template = PromptTemplate::new("lower", "Original: {{name}}, Lower: {{lowercase name}}");
...
fn test_trim_helper() {
    ...
    let template = PromptTemplate::new("trim_test", "Trimmed: [{{trim text}}]");
    engine.register_template("trim_test", template).unwrap();

    let mut ctx = PromptContext::new();
    ctx.set(
        "text".to_string(),
        ContextValue::String("  hello world  ".to_string()),
    );
...
fn test_length_helper_string() {
    ...
    let template = PromptTemplate::new("len", "Length: {{length text}}");
...
fn test_length_helper_array() {
    ...
    let template = PromptTemplate::new("len_arr", "Count: {{length items}}");
...
fn test_json_helper() {
    ...
    let template = PromptTemplate::new("json_test", "JSON: {{json data}}");
...
fn test_indent_helper_default() {
    ...
    let template = PromptTemplate::new("indent_test", "{{indent text}}");
    engine.register_template("indent_test", template).unwrap();

    let mut ctx = PromptContext::new();
    ctx.set(
        "text".to_string(),
        ContextValue::String("line1\nline2".to_string()),
    );
...
fn test_indent_helper_custom_spaces() {
    ...
    let template = PromptTemplate::new("indent_custom", "{{indent text 4}}");
...
fn test_template_with_consecutive_variables() {
    ...
    let template = PromptTemplate::new("consecutive", "{{first}}{{second}}{{third}}");
...
fn test_template_with_very_long_variable_name() {
    ...
    let template = PromptTemplate::new("long_var", format!("Value: {{{{{}}}}}", long_var));
...
fn test_context_set_path_deep_nesting() {
    ...
    let json = ctx.to_json();
    assert_eq!(
        json["level1"]["level2"]["level3"]["level4"]["level5"]["value"],
        "deep"
    );
}
...
fn test_context_nested_arrays() {
    ...
    let outer = ContextValue::Array(vec![inner.clone(), inner.clone()]);
...
fn test_template_with_metadata_accessor() {
    ...
    let template = PromptTemplate::new_with_metadata("test", "Hello {{name}}", meta);
...
fn test_template_with_metadata_builder() {
    ...
    let template = PromptTemplate::new("test", "Content").with_metadata(meta.clone());
