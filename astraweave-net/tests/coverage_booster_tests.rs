...
async fn test_pusher_apply_result_and_ack() -> Result<()> {
    ...
    server.tx.send(ServerEvent::ApplyResult {
        ok: true,
        err: None,
    })?;
    server.tx.send(ServerEvent::Ack {
        seq: 123,
        tick_applied: 456,
    })?;
...
async fn test_handle_conn_client_input_and_propose_plan() -> Result<()> {
    ...
    ws_stream
        .send(Message::Text(serde_json::to_string(&propose)?.into()))
        .await?;
    ...
    assert!(
        found_apply,
        "Should receive ApplyResult for ProposePlan/Input"
    );
...
async fn test_interest_policy_variants_in_pusher() -> Result<()> {
    ...
    ws_stream
        .send(Message::Text(serde_json::to_string(&hello_fov)?.into()))
        .await?;
    ...
    ws_stream
        .send(Message::Text(serde_json::to_string(&hello_fovlos)?.into()))
        .await?;
    sleep(Duration::from_millis(100)).await;
    ...
    ws_stream
        .send(Message::Text(serde_json::to_string(&hello_unknown)?.into()))
        .await?;
    sleep(Duration::from_millis(100)).await;
