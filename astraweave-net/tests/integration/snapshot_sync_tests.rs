...
use astraweave_net::{
    ...
    FullInterest, RadiusTeamInterest, Snapshot,
};
...
fn test_snapshot_roundtrip_100_entities() {
    ...
    assert!(
        team_counts[0] >= 33,
        "Team 0 should have ~33 entities (got {})",
        team_counts[0]
    );
    assert!(
        team_counts[1] >= 33,
        "Team 1 should have ~33 entities (got {})",
        team_counts[1]
    );
    assert!(
        team_counts[2] >= 33,
        "Team 2 should have ~33 entities (got {})",
        team_counts[2]
    );
...
fn test_delta_compression_accuracy() {
    ...
    let player_recon = reconstructed
        .entities
        .iter()
        .find(|e| e.id == player)
        .unwrap();
    ...
    // Verify enemy1 HP
    let enemy1_recon = reconstructed
        .entities
        .iter()
        .find(|e| e.id == enemy1)
        .unwrap();
    ...
    // Verify enemy2 ammo
    let enemy2_recon = reconstructed
        .entities
        .iter()
        .find(|e| e.id == enemy2)
        .unwrap();
    ...
    assert!(!delta.changed.is_empty(), "Delta should contain updates");
...
fn test_interest_management_filtering() {
    ...
    assert_eq!(
        full_snapshot.entities.len(),
        7,
        "Full snapshot has all entities"
    );
    ...
    assert!(
        !filtered_ids.contains(&far_enemy1),
        "Far enemy1 should NOT be visible"
    );
    assert!(
        !filtered_ids.contains(&far_enemy2),
        "Far enemy2 should NOT be visible"
    );

    println!(
        "   Full snapshot: {} entities",
        full_snapshot.entities.len()
    );
...
fn test_entity_spawn_despawn_sync() {
    ...
    assert!(
        new_ids.contains(&enemy2),
        "Delta should include newly spawned entity"
    );
...
fn test_high_frequency_delta_chain() {
    ...
    assert_eq!(
        player_state.pos, expected_pos,
        "Position after 60 deltas should be (60, 0)"
    );
    ...
    println!(
        "   Final position: ({}, {})",
        player_state.pos.x, player_state.pos.y
    );
...
fn test_snapshot_determinism_multiple_builds() {
    ...
    let snapshots: Vec<Snapshot> = (0..5).map(|_| build_snapshot(&w, 42, 0)).collect();
    ...
    for (i, snap) in snapshots.iter().enumerate() {
        assert_eq!(
            ...
            "Snapshot {} hash should match first",
            i
        );
        assert_eq!(
            snap.entities.len(),
            50,
            "Snapshot {} should have 50 entities",
            i
        );
    }
    ...
    for i in 1..5 {
        for j in 0..50 {
            assert_eq!(
                snapshots[0].entities[j].id, snapshots[i].entities[j].id,
