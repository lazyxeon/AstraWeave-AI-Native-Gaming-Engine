...
use astraweave_persona::{EmotionalState, LlmPersona, LlmPersonaManager, PersonalityState};
...
async fn test_persona_creation() {
    ...
    assert!(matches!(
        state.personality_state.emotional_state,
...
async fn test_update_mood() {
    ...
    manager
        .generate_response("This is great, thank you!", None)
        .await
        .unwrap();
    let state = manager.get_persona_state().await;
    ...
    manager
        .generate_response("This is terrible, I hate it.", None)
        .await
        .unwrap();
...
async fn test_update_trust_level() {
    ...
    manager
        .generate_response("Hello there.", None)
        .await
        .unwrap();

    let new_trust = manager
        .get_persona_state()
        .await
        .personality_state
        .trust_level;
...
async fn test_update_energy() {
    ...
    manager
        .generate_response("You should rest now.", None)
        .await
        .unwrap();
    let state = manager.get_persona_state().await;
...
async fn test_update_confidence() {
    ...
    manager
        .generate_response("That was wrong.", None)
        .await
        .unwrap();
    let state = manager.get_persona_state().await;
...
async fn test_emotional_state_transitions() {
    ...
    for _ in 0..5 {
        manager
            .generate_response("Wonderful! Amazing! Great!", None)
            .await
            .unwrap();
    }
    let state = manager.get_persona_state().await;
    ...
    for _ in 0..10 {
        manager
            .generate_response("Terrible! Awful! Bad!", None)
            .await
            .unwrap();
    }
    let state = manager.get_persona_state().await;
...
async fn test_personality_factor_influence() {
    ...
    manager
        .evolve_personality("Let's be creative and imagine something new.")
        .await
        .unwrap();
...
async fn test_adaptation_data_tracking() {
    ...
    manager.generate_response("Hello", None).await.unwrap();
    manager
        .generate_response("How are you?", None)
        .await
        .unwrap();
...
async fn test_persona_metrics() {
    ...
    manager
        .generate_response("Test prompt", None)
        .await
        .unwrap();
...
async fn test_persona_clone() {
    ...
    assert_eq!(
        state.personality_state.trust_level,
        cloned.personality_state.trust_level
    );
...
async fn test_persona_equality() {
    ...
    assert_eq!(
        state1.personality_state.current_mood,
        state2.personality_state.current_mood
    );
}
...
async fn test_rag_integration_lifecycle() {
    ...
    manager
        .generate_response("My name is Commander Shepard.", None)
        .await
        .unwrap();
...
async fn test_response_style_defaults() {
    ...
    assert!(matches!(
        state.llm_config.response_style,
...
async fn test_context_injection_strategies() {
    ...
    assert!(matches!(
        final_state.prompt_settings.context_injection,
...
async fn test_few_shot_examples() {
    ...
    assert_eq!(
        final_state.prompt_settings.few_shot_examples[0].input,
        "Hello"
    );
}
...
async fn test_personality_influence_decay() {
    ...
    assert_eq!(
        final_state.personality_state.recent_influences[0].importance,
        0.8
    );
}
...
async fn test_player_patterns_learning() {
    ...
    assert_eq!(
        final_state.adaptation.player_patterns.avg_session_length,
        Some(45.5)
    );
}
...
async fn test_adaptation_events() {
    ...
    assert_eq!(
        final_state.adaptation.adaptation_history[0].success_rating,
        Some(0.9)
    );
}
...
async fn test_memory_consolidation_preferences() {
    ...
    assert_eq!(
        state.memory_profile.consolidation_preferences.max_memories,
        1000
    );
}
...
async fn test_episodic_memory_management() {
    ...
    state
        .memory_profile
        .episodic_memories
        .push("Met player at the tavern".to_string());
    state
        .memory_profile
        .episodic_memories
        .push("Fought alongside player".to_string());
...
async fn test_semantic_knowledge() {
    ...
    state
        .memory_profile
        .semantic_knowledge
        .push("Dragons are weak to ice".to_string());
    state
        .memory_profile
        .semantic_knowledge
        .push("Potions restore health".to_string());
...
async fn test_personality_factor_modification() {
    ...
    state
        .llm_config
        .personality_factors
        .insert("boldness".to_string(), 0.85);
    state
        .llm_config
        .personality_factors
        .insert("caution".to_string(), 0.3);
...
async fn test_conversation_history_growth() {
    ...
    manager
        .generate_response("First message", None)
        .await
        .unwrap();
    manager
        .generate_response("Second message", None)
        .await
        .unwrap();
    manager
        .generate_response("Third message", None)
        .await
        .unwrap();
...
async fn test_successful_interaction_tracking() {
    ...
    manager
        .generate_response("This is good", None)
        .await
        .unwrap();
    manager
        .generate_response("Excellent work", None)
        .await
        .unwrap();
...
async fn test_topics_to_avoid() {
    ...
    state
        .adaptation
        .topics_to_avoid
        .push("politics".to_string());
    state
        .adaptation
        .topics_to_avoid
        .push("religion".to_string());
...
async fn test_preferred_topics() {
    ...
    state
        .adaptation
        .preferred_topics
        .insert("space exploration".to_string(), 0.9);
    state
        .adaptation
        .preferred_topics
        .insert("technology".to_string(), 0.8);
...
async fn test_prompt_modifiers() {
    ...
    state
        .prompt_settings
        .prompt_modifiers
        .insert("formality".to_string(), "high".to_string());
    state
        .prompt_settings
        .prompt_modifiers
        .insert("verbosity".to_string(), "concise".to_string());
...
async fn test_personality_drift_tracking() {
    ...
    state
        .personality_state
        .personality_drift
        .insert("optimism".to_string(), 0.15);
    state
        .personality_state
        .personality_drift
        .insert("caution".to_string(), -0.1);
...
async fn test_multiple_personality_evolutions() {
    ...
    manager
        .evolve_personality("Let's create art")
        .await
        .unwrap();
    manager
        .evolve_personality("I need help with this")
        .await
        .unwrap();
...
async fn test_context_parameter_usage() {
    ...
    manager
        .generate_response("Question", Some("This is additional context"))
        .await
        .unwrap();
...
async fn test_mood_clamping() {
    ...
    for _ in 0..20 {
        manager
            .generate_response("Wonderful amazing excellent great!", None)
            .await
            .unwrap();
    }
...
async fn test_confidence_clamping() {
    ...
    for _ in 0..50 {
        manager
            .generate_response("Wrong terrible bad", None)
            .await
            .unwrap();
    }
