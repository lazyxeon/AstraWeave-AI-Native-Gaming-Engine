...
mod tests {
    ...
    fn test_batch_request_add_agent() {
        let mut batch = BatchRequest::new(vec![
            (2, create_test_snapshot(2, 2)),
        ]);
    ...
    async fn test_execute_batch_with_streaming() {
        ...
        let response = executor.execute_batch(&llm_client, "MoveTo|Attack|Reload").await;
    ...
    async fn test_execute_batch_empty_error() {
        ...
        assert!(result.unwrap_err().to_string().contains("No batch to execute"));
    }
