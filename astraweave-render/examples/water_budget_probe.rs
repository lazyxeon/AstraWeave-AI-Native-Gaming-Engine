...
fn probe_isolated(device: &wgpu::Device, queue: &wgpu::Queue, has_timestamps: bool) {
    ...
    let cams = [
        CamCfg {
            name: "near (gameplay cam ~12u above)",
            eye: Vec3::new(0.0, 14.0, 70.0),
            target: Vec3::new(0.0, 2.0, 0.0),
        },
        CamCfg {
            name: "horizon (eye at water level)",
    ...
    println!(
        ...
        if frac > 5.0 {
            "surface RASTERIZES (not culled away)"
        } else {
            "!! suspiciously empty — investigate winding/culling"
        }
    );
}
...
async fn probe_full_frame(_has_timestamps: bool) {
    ...
    if let (Some(nw), Some(ww)) = (no_water, with_water) {
        println!(
            "  marginal water cost (with-water minus no-water, wall-clock): {:.4}ms",
            (ww - nw).max(0.0)
        );
    }
}
...
fn probe_refraction(device: &wgpu::Device, queue: &wgpu::Queue, has_timestamps: bool) {
    ...
    for cam in &cams {
        ...
        for frame in 0..(WARMUP + FRAMES) {
            ...
            // 1. Opaque ground into hdr + depth.
            {
                ...
                rp.set_pipeline(&gpipe);
                rp.set_bind_group(0, &gbg, &[]);
                rp.set_vertex_buffer(0, gvb.slice(..));
                rp.set_index_buffer(gib.slice(..), wgpu::IndexFormat::Uint32);
                rp.draw_indexed(0..6, 0, 0..1);
            }
            // 2. Snapshot hdr → scene_color (the copy line item).
            if copy_ts {
                enc.write_timestamp(&copy_qset, 0);
            }
    ...
    // color tap is live, not a dummy/black sample.
    {
        ...
        for (lo, hi, label) in [(10u32, 40u32, "upper"), (40, 60, "mid"), (60, 90, "lower")] {
            ...
            println!(
                "    band {label:<6} avg rgb=({:.3},{:.3},{:.3})  warm={:.0}%  foam={:.1}%",
                sr / nn,
                sg / nn,
                sb / nn,
                w as f64 / nn * 100.0,
        ...
        println!(
            "  foam-check: {:.2}% near-white foam pixels in water region — {}",
            foam_pct,
            if foam > 0 {
                "FOAM RENDERS (wave-crest + depth-delta shoreline)"
            } else {
                "no foam detected"
            }
        );
        ...
        println!(
            ...
            pct,
            if pct > 5.0 {
                "REFRACTION LIVE (scene-color sampled, not dummy/flat)"
            } else {
                "!! no refraction tint — investigate scene-color binding"
            }
        );
    }
}
...
fn main() {
    pollster::block_on(async {
        ...
        println!(
            "Target  : {}x{}  color={:?} depth={:?}",
            W, H, COLOR_FMT, DEPTH_FMT
        );
        ...
        if matches!(info.device_type, wgpu::DeviceType::Cpu) {
            println!(
                "!! WARNING: adapter is a CPU/software rasterizer — numbers are NOT min-spec GPU."
            );
        }
