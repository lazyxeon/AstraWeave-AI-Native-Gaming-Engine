...
fn occlusion_cull_meshlet(
    ...
    let mip = ((proj_radius * 2.0).log2().ceil() as usize)
        .max(0)
        .min(hiz.len() - 1);
...
fn calculate_cascade_projection(corners: &[Vec3; 8], light_dir: Vec3) -> (Mat4, Mat4, Mat4) {
    // Light view matrix
...
fn bench_meshlet_culling(c: &mut Criterion) {
    ...
    group.bench_function("frustum_single_visible", |b| {
        b.iter(|| {
            frustum_cull_meshlet(
                black_box(&meshlet_visible),
                black_box(&camera.frustum_planes),
            )
        });
    });
...
fn bench_full_shadow_pass(c: &mut Criterion) {
    ...
    for &cascade_count in &[2, 4] {
        let cascades =
            build_shadow_cascades(camera_view_proj, light_dir, 0.1, 1000.0, cascade_count);
        let shadow_map = ShadowMap::new(2048);
