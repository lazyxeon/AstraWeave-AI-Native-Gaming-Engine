...
fn bench_clustered(c: &mut Criterion) {
    ...
    let near = 0.1f32; let far = 200.0f32; let fov = std::f32::consts::FRAC_PI_3;
    // Generate lights
    let mut lights = Vec::new();
    for i in 0..512 {
        let t = i as f32 * 0.123;
        let r = 20.0 + (i % 50) as f32 * 0.1;
        lights.push(CpuLight{ pos: glam::Vec3::new(t.sin()*r, 1.5 + (i%7) as f32, t.cos()*r), radius: 6.0 + (i%13) as f32 });
    }
    c.bench_function("clustered_cpu_binning_512", |b| {
        b.iter(|| {
            let (_counts, _indices, offsets) = bin_lights_cpu(black_box(&lights), dims, screen, near, far, fov);
            black_box(offsets);
        });
    });
