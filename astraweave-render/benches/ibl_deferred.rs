...
fn bench_spherical_harmonics(c: &mut Criterion) {
    ...
    for bands in [3u32, 4, 5] {
        ...
        group.bench_with_input(
            ...
            |b, &bands| {
                b.iter(|| {
                    black_box(generate_sh_coefficients(bands))
                })
            },
        );
    }
    ...
    for bands in [3u32, 4, 5] {
        ...
        group.bench_with_input(
            ...
            |b, &bands| {
                b.iter(|| {
                    black_box(evaluate_sh_basis(bands, &dir))
                })
            },
        );
    }
...
fn direction_to_cubemap_uv(dir: [f32; 3]) -> (u32, [f32; 2]) {
    ...
    let (face, uv) = if abs_dir[0] >= abs_dir[1] && abs_dir[0] >= abs_dir[2] {
        if dir[0] > 0.0 {
            (0, [(-dir[2] / abs_dir[0] + 1.0) * 0.5, (-dir[1] / abs_dir[0] + 1.0) * 0.5])
        } else {
            (1, [(dir[2] / abs_dir[0] + 1.0) * 0.5, (-dir[1] / abs_dir[0] + 1.0) * 0.5])
        }
    } else if abs_dir[1] >= abs_dir[0] && abs_dir[1] >= abs_dir[2] {
        if dir[1] > 0.0 {
            (2, [(dir[0] / abs_dir[1] + 1.0) * 0.5, (dir[2] / abs_dir[1] + 1.0) * 0.5])
        } else {
            (3, [(dir[0] / abs_dir[1] + 1.0) * 0.5, (-dir[2] / abs_dir[1] + 1.0) * 0.5])
        }
    } else {
        if dir[2] > 0.0 {
            (4, [(dir[0] / abs_dir[2] + 1.0) * 0.5, (-dir[1] / abs_dir[2] + 1.0) * 0.5])
        } else {
            (5, [(-dir[0] / abs_dir[2] + 1.0) * 0.5, (-dir[1] / abs_dir[2] + 1.0) * 0.5])
        }
    };
...
fn sample_cubemap_bilinear(dir: [f32; 3], mip_size: u32) -> [f32; 4] {
    ...
    [
        s00[0] * (1.0 - fu) * (1.0 - fv) + s10[0] * fu * (1.0 - fv) + s01[0] * (1.0 - fu) * fv + s11[0] * fu * fv,
        s00[1] * (1.0 - fu) * (1.0 - fv) + s10[1] * fu * (1.0 - fv) + s01[1] * (1.0 - fu) * fv + s11[1] * fu * fv,
        s00[2] * (1.0 - fu) * (1.0 - fv) + s10[2] * fu * (1.0 - fv) + s01[2] * (1.0 - fu) * fv + s11[2] * fu * fv,
        s00[3] * (1.0 - fu) * (1.0 - fv) + s10[3] * fu * (1.0 - fv) + s01[3] * (1.0 - fu) * fv + s11[3] * fu * fv,
    ]
}
...
fn bench_ggx_sampling(c: &mut Criterion) {
    ...
    for roughness in [0.1f32, 0.25, 0.5, 0.75, 1.0] {
        group.bench_with_input(
            ...
            &roughness,
            |b, &r| {
                b.iter(|| black_box(ggx_importance_sample(xi, r, normal)))
            },
        );
    }
    ...
    for sample_count in [64u32, 256, 1024] {
        for roughness in [0.25f32, 0.5, 0.75] {
            ...
            group.bench_with_input(
                ...
                |b, &(count, r)| {
                    b.iter(|| black_box(generate_prefilter_kernel(r, count)))
                },
            );
        }
    }
...
fn bench_gbuffer_operations(c: &mut Criterion) {
    ...
    let packed_normals: Vec<[u16; 2]> = normals.iter().map(|n| pack_normal_octahedral(*n)).collect();
    ...
    group.bench_function("pack_gbuffer_pixel_1k", |b| {
        b.iter(|| {
            for (albedo, normal, roughness, metallic, ao) in &test_data {
                black_box(pack_gbuffer_pixel(*albedo, *normal, *roughness, *metallic, *ao));
            }
        })
    });
...
fn bench_brdf_lut(c: &mut Criterion) {
    ...
    for sample_count in [64u32, 256, 1024] {
        group.bench_with_input(
            ...
            &sample_count,
            |b, &count| {
                b.iter(|| black_box(integrate_brdf(0.5, 0.5, count)))
            },
        );
    }
...
fn calculate_point_light(
    ...
    let n_dot_v = (normal[0] * view_dir[0] + normal[1] * view_dir[1] + normal[2] * view_dir[2]).max(0.0);
    ...
    group.bench_function("single_point_light", |b| {
        ...
        b.iter(|| {
            black_box(calculate_point_light(
                world_pos, normal, view_dir, light_pos, light_color, light_radius,
