...
fn simulate_tent_upsample(width: u32, height: u32) -> f32 {
    ...
    let weights = [0.0625, 0.125, 0.0625, 0.125, 0.25, 0.125, 0.0625, 0.125, 0.0625];
...
fn calculate_cascade_matrix(split_near: f32, split_far: f32, aspect: f32) -> [[f32; 4]; 4] {
    ...
    [
        ...
        [0.0, 0.0, -(split_far + split_near) / (split_far - split_near), 1.0],
    ]
}
...
fn bench_csm_matrix(c: &mut Criterion) {
    ...
    for i in 0..4 {
        group.bench_with_input(
            ...
            |b, &idx| {
                b.iter(|| {
                    black_box(calculate_cascade_matrix(splits[idx], splits[idx + 1], 16.0 / 9.0))
                });
            },
        );
    }
...
criterion_group!(
    bloom_benches,
    bench_bloom_threshold,
    bench_bloom_mip_chain
);
