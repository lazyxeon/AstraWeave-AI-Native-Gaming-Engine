...
fn bench_material_compile(c: &mut Criterion) {
    c.bench_function("material_compile_64_nodes", |b| {
        b.iter(|| {
            ...
            for i in 0..32 {
                nodes.insert(format!("t{}", i), Node::Texture2D{ id: format!("tex{}", i), uv: "uv".into()});
            }
            nodes.insert("a".into(), Node::Add{ a: "t0".into(), b: "t1".into()});
            nodes.insert("b".into(), Node::Multiply{ a: "a".into(), b: "t2".into()});
            let g = Graph{ nodes, base_color: "b".into(), mr: None, normal: None, clearcoat: None, anisotropy: None, transmission: None };
...
fn bench_cpu_cluster_binning(c: &mut Criterion) {
    ...
    c.bench_function("cpu_cluster_binning_1k_lights", |b| {
        ...
        for i in 0..1000 {
            let f = i as f32 * 0.01;
            lights.push(CpuLight{ pos: Vec3::new(f.sin()*10.0, f.cos()*5.0, (f*10.0).fract()*50.0 + 1.0), radius: 1.0 + (f*3.0).fract() });
        }
        b.iter(|| {
            let (c,_i,_o) = bin_lights_cpu(&lights, dims, (1920,1080), 0.1, 100.0, std::f32::consts::FRAC_PI_3);
