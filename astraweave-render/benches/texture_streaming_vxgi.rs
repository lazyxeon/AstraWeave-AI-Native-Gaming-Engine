...
impl TextureStreamingManager {
    ...
    fn simulate_load(&mut self, id: &str, width: u32, height: u32, mip_levels: u32) {
        ...
        self.assets
            .insert(id.to_string(), AssetState::Resident(handle));
...
impl VoxelGrid {
    ...
    fn world_to_voxel(&self, pos: Vec3) -> Option<(u32, u32, u32)> {
        ...
        Some((
            x.min(self.resolution - 1),
            y.min(self.resolution - 1),
            z.min(self.resolution - 1),
        ))
    }
...
fn voxelize_triangle(grid: &mut VoxelGrid, v0: Vec3, v1: Vec3, v2: Vec3, radiance: VoxelRadiance) {
        ...
        for z in min_v.2..=max_v.2 {
            for y in min_v.1..=max_v.1 {
                for x in min_v.0..=max_v.0 {
                    ...
                    if voxel_center.x >= min.x - voxel_size
                        && voxel_center.x <= max.x + voxel_size
                        && voxel_center.y >= min.y - voxel_size
                        && voxel_center.y <= max.y + voxel_size
                        && voxel_center.z >= min.z - voxel_size
                        && voxel_center.z <= max.z + voxel_size
                    {
...
fn bench_texture_streaming(c: &mut Criterion) {
    ...
    for memory_mb in [256, 512, 1024, 2048] {
        group.bench_with_input(
            ...
            &memory_mb,
            |b, &mem| {
                b.iter(|| black_box(TextureStreamingManager::new(mem)));
            },
        );
    }
    ...
    for count in [100, 500, 1000] {
        group.bench_with_input(BenchmarkId::new("evict_lru", count), &count, |b, &cnt| {
            b.iter(|| {
    ...
    // Get stats
    {
        ...
        group.bench_function("get_stats_500_textures", |b| {
            b.iter(|| black_box(manager.get_stats()));
        });
    }
...
fn bench_vxgi_grid(c: &mut Criterion) {
    ...
    for resolution in [64, 128, 256] {
        group.throughput(Throughput::Elements(
            (resolution * resolution * resolution) as u64,
        ));
        group.bench_with_input(
            BenchmarkId::new("create", format!("{}^3", resolution)),
            &resolution,
            |b, &res| {
                b.iter(|| black_box(VoxelGrid::new(res, 1000.0)));
            },
        );
    }
...
fn bench_vxgi_cone_tracing(c: &mut Criterion) {
    ...
    for count in [1, 4, 6, 12, 32] {
        group.bench_with_input(
            ...
            &count,
            |b, &cnt| {
                b.iter(|| black_box(generate_cone_directions(cnt)));
            },
        );
    }
        ...
        for max_dist in [25.0, 50.0, 100.0] {
            group.bench_with_input(
                ...
                |b, &dist| {
                    ...
                    b.iter(|| black_box(cone_trace(&grid, origin, direction, aperture, dist, 0.5)));
                },
            );
        }
    }
        ...
        for cone_count in [1, 4, 6] {
            ...
            group.bench_with_input(
                ...
                |b, cfg| {
                    ...
                    b.iter(|| black_box(compute_indirect_lighting(&grid, position, normal, cfg)));
                },
            );
        }
    }
