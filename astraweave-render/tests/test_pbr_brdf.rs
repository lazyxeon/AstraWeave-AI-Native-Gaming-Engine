...
fn test_ggx_increases_with_roughness_at_grazing() {
    ...
    assert!(rough > smooth, "Rougher surfaces have broader highlights at grazing angles");
}
...
fn test_ggx_decreases_with_roughness_at_peak() {
    ...
    assert!(smooth > rough, "Smooth surfaces have sharper specular peaks");
}
...
fn test_ggx_long_tail() {
    ...
    assert!(grazing > 0.0, "GGX has non-zero values at grazing angles (long tail characteristic)");
}
...
fn test_smith_decreases_with_roughness() {
    ...
    assert!(smooth_g > rough_g, "Smith geometry should decrease with roughness (more shadowing)");
}
...
fn test_smith_decreases_at_grazing_angles() {
    ...
    assert!(normal_g > grazing_g, "Smith G should decrease at grazing angles");
}
...
fn test_smith_is_product_of_view_and_light() {
    ...
    assert!(diff < 1e-6, "Smith G should be product of G1(V) × G1(L), diff: {}", diff);
}
...
fn test_fresnel_at_normal_incidence() {
    ...
    assert!(diff < 1e-6, "Fresnel at normal incidence should equal F0, diff: {}", diff);
}
...
fn test_fresnel_at_grazing_angle() {
    ...
    assert!(grazing > 0.9, "Fresnel at grazing angle should approach 1.0, got {}", grazing);
}
...
fn test_fresnel_monotonic_increase() {
    ...
    assert!(f_grazing > f_mid, "Fresnel should increase toward grazing angle");
    assert!(f_mid > f_normal, "Fresnel should increase from normal to mid angle");
}
...
fn test_fresnel_with_metal_f0() {
    ...
    assert!((normal - f0_metal).abs() < 1e-6, "Metal Fresnel at normal incidence");
    assert!(grazing > normal, "Metal Fresnel still increases at grazing angle");
...
fn test_energy_conservation_dielectric() {
    ...
    assert!(total_energy <= 1.0 + 1e-6, "Diffuse + specular must not exceed incident light, got {}", total_energy);
...
fn test_energy_conservation_metal() {
    ...
    assert!(kd.abs() < 1e-6, "Metals should have no diffuse component, got kD={}", kd);
...
fn test_energy_conservation_partial_metal() {
    ...
    assert!(total_contribution <= 1.0 + 1e-6, "Energy conservation violated for partial metal, got {}", total_contribution);
}
...
fn test_brdf_specular_increases_with_smoothness() {
    ...
    assert!(smooth_d > rough_d, "Smooth surfaces (r=0.1) have sharper specular peaks at N·H=1, smooth={}, rough={}", smooth_d, rough_d);
}
...
fn test_brdf_zero_at_backface() {
    ...
    assert!(g_backface < 0.1, "BRDF should approach zero at backface, got G={}", g_backface);
}
...
fn test_smith_numerical_stability() {
    ...
    assert!(g_extreme.is_finite(), "Smith G should be finite at extreme values");
...
fn test_fresnel_clamping() {
    ...
    assert!(f.is_finite(), "Fresnel should handle out-of-range input gracefully");
    assert!(f <= 1.0 + 1e-5, "Fresnel should not significantly exceed 1.0");
}
...
fn test_ggx_known_value() {
    ...
    assert!(diff < 1e-4, "GGX value should match reference, expected {}, got {}, diff {}", expected, result, diff);
}
...
fn test_smith_known_value() {
    ...
    let diff = (g - 1.0).abs();
    assert!(diff < 1e-4, "Smith G at normal incidence should be 1.0, got {}, diff {}", g, diff);
}
...
fn test_fresnel_schlick_known_value() {
    ...
    assert!(diff < 1e-5, "Fresnel at 60° should match reference, expected {}, got {}, diff {}", expected, f, diff);
}
