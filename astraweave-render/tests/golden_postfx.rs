...
fn aces_tonemap(x: [f32;3]) -> [f32;3] {
    let a = 2.51f32; let b = 0.03f32; let c = 2.43f32; let d = 0.59f32; let e = 0.14f32;
...
fn image_delta(a: &[u8], b: &[u8]) -> (u8, f32) {
    ...
    let mut sum = 0u64; let mut maxd = 0u8;
    for i in 0..a.len() { let d = a[i].abs_diff(b[i]); sum += d as u64; if d>maxd { maxd=d; } }
...
fn golden_postfx_compose_matches_cpu() {
    ...
    hdr_bytes[idx+2] = (0.25f32 * 255.0f32).round() as u8;
        ...
        ao_bytes[idx+1] = 0; ao_bytes[idx+2] = 0; ao_bytes[idx+3] = 255;
        // gi constant color
    gi_bytes[idx+0] = (0.10f32 * 255.0f32).round() as u8;
    gi_bytes[idx+1] = (0.05f32 * 255.0f32).round() as u8;
        gi_bytes[idx+2] = 0; gi_bytes[idx+3] = 255;
    }}
    let layout = wgpu::ImageDataLayout{ offset: 0, bytes_per_row: Some(w*4), rows_per_image: Some(h) };
    ...
    let target = device.create_texture(&wgpu::TextureDescriptor{
        label: Some("target"),
        size: extent, mip_level_count: 1, sample_count: 1, dimension: wgpu::TextureDimension::D2,
    ...
    let bg = device.create_bind_group(&wgpu::BindGroupDescriptor{ label: Some("bg"), layout: &bgl, entries: &[
        ...
        wgpu::BindGroupEntry{ binding:2, resource: wgpu::BindingResource::TextureView(&gi_view)},
    ...
    let pipeline = device.create_render_pipeline(&wgpu::RenderPipelineDescriptor{
        label: Some("post-fx pipeline"), layout: Some(&pl),
        ...
        fragment: Some(wgpu::FragmentState{ module: &shader, entry_point: "fs_main", targets: &[Some(wgpu::ColorTargetState{ format: wgpu::TextureFormat::Rgba8UnormSrgb, blend: Some(wgpu::BlendState::REPLACE), write_mask: wgpu::ColorWrites::ALL })], compilation_options: wgpu::PipelineCompilationOptions::default() }),
    ...
    {
        let mut rp = enc.begin_render_pass(&wgpu::RenderPassDescriptor{ label: Some("rp"), color_attachments: &[Some(wgpu::RenderPassColorAttachment{ view: &target_view, resolve_target: None, ops: wgpu::Operations{ load: wgpu::LoadOp::Clear(wgpu::Color::BLACK), store: wgpu::StoreOp::Store } })], depth_stencil_attachment: None, timestamp_writes: None, occlusion_query_set: None });
        rp.set_pipeline(&pipeline);
        rp.set_bind_group(0, &bg, &[]);
        rp.draw(0..3, 0..1);
    }
    // Readback
    let bytes_pp = 4u64; let row_bytes = w as u64 * bytes_pp; let align = wgpu::COPY_BYTES_PER_ROW_ALIGNMENT as u64; let padded = ((row_bytes + align - 1) / align) * align;
    let buf = device.create_buffer(&wgpu::BufferDescriptor{ label: Some("read"), size: padded * h as u64, usage: wgpu::BufferUsages::MAP_READ | wgpu::BufferUsages::COPY_DST, mapped_at_creation: false });
    enc.copy_texture_to_buffer(target.as_image_copy(), wgpu::ImageCopyBuffer{ buffer: &buf, layout: wgpu::ImageDataLayout{ offset: 0, bytes_per_row: Some(padded as u32), rows_per_image: Some(h) } }, wgpu::Extent3d{ width:w, height:h, depth_or_array_layers:1 });
    ...
    for y in 0..h as usize { let src = y as u64 * padded; let dst = y as usize * w as usize * 4; img[dst..dst + (w as usize * 4)].copy_from_slice(&data[src as usize..(src + row_bytes) as usize]); }
    drop(data); buf.unmap();

    // Build expected
    let mut exp = vec![0u8; (w*h*4) as usize];
    for y in 0..h { for x in 0..w {
        let u = x as f32 / (w as f32 - 1.0); let v = y as f32 / (h as f32 - 1.0);
        let hdr = [u, v, 0.25f32];
