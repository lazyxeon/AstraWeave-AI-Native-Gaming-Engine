...
mod terrain_material_tests {
    use astraweave_render::{TerrainLayerDesc, TerrainLayerGpu, TerrainMaterialDesc, TerrainMaterialGpu};
    ...
    fn test_triplanar_blend_factor_vertical_cliff() {
        ...
        let up_dot = (vertical_normal[0] * 0.0 + vertical_normal[1] * 1.0 + vertical_normal[2] * 0.0).abs();
    ...
    fn test_height_range_validation() {
        ...
        for layer in &grassland.layers {
            if let Some(range) = layer.height_range {
                ...
                assert!(range[0] < range[1], "Layer {} has invalid height range", layer.name);
            }
        }
    }
    ...
    fn test_terrain_material_toml_roundtrip() {
        ...
        // Deserialize back
        let deserialized: TerrainMaterialDesc = toml::from_str(&toml_string)
        ...
        assert_eq!(deserialized.triplanar_slope_threshold, original.triplanar_slope_threshold);
    }
    ...
    fn test_terrain_layer_toml_defaults() {
        ...
        let layer: TerrainLayerDesc = toml::from_str(toml_minimal)
            .expect("Failed to parse minimal TOML");
