...
fn test_guid_uniqueness() {
    ...
    assert_ne!(
        guid1, guid3,
        "Different directories must have different GUIDs"
    );
}
...
fn test_aabb_center_formula() {
    ...
    assert!(
        (center - Vec3::ZERO).length() < 0.001,
        "Symmetric AABB centered at origin"
    );
}
...
fn test_aabb_from_points() {
    let points = [[-1.0, 0.0, 2.0], [3.0, -2.0, 1.0], [0.0, 5.0, 0.0]];
...
fn test_aabb_contains() {
    ...
    assert!(
        !aabb.contains(Vec3::new(1.5, 0.5, 0.5)),
        "Outside X is not inside"
    );
    assert!(
        !aabb.contains(Vec3::new(-0.1, 0.5, 0.5)),
        "Negative X is not inside"
    );
}
...
fn test_aabb_merge() {
    ...
    assert!(
        (merged.min - Vec3::ZERO).length() < 0.001,
        "Merged min is smaller min"
    );
    assert!(
        (merged.max - Vec3::new(2.0, 2.0, 2.0)).length() < 0.001,
        "Merged max is larger max"
    );
}
...
fn test_bounding_cone_backfacing() {
    ...
    assert!(
        cone.is_backfacing(Vec3::NEG_Z),
        "Viewing from front = backfacing for this cone"
    );

    // View from front (-Z looking into +Z)
    assert!(
        !cone.is_backfacing(Vec3::Z),
        "Viewing from behind = not backfacing"
    );
}
...
fn test_bounding_cone_cutoff() {
    ...
    assert!(
        !cone.is_backfacing(view_45.normalize()),
        "45° view (cos=0.707) > cutoff 0.5"
    );
...
fn test_quadric_error_formula() {
    ...
    assert!(
        (error - 1.0).abs() < 0.001,
        "Point 1 unit from plane has error 1"
    );
    ...
    assert!(
        (error - 4.0).abs() < 0.001,
        "Point 2 units from plane has error 4 (squared)"
    );
}
...
fn test_quadric_from_triangle() {
    ...
    assert!(
        (normal.y - (-1.0)).abs() < 0.001,
        "XZ triangle normal is -Y"
    );
...
fn test_entity_data_identity_quaternion() {
    ...
    let mag =
        (rotation[0].powi(2) + rotation[1].powi(2) + rotation[2].powi(2) + rotation[3].powi(2))
            .sqrt();
...
fn test_blend_file_detection() {
    ...
    assert!(
        is_blend_file(Path::new("model.BLEND")),
        ".BLEND detected (case insensitive)"
    );
    assert!(
        is_blend_file(Path::new("path/to/model.blend")),
        "Nested .blend detected"
    );
    assert!(
        !is_blend_file(Path::new("model.gltf")),
        ".gltf not detected as blend"
    );
...
fn test_dependency_graph_bidirectional() {
    ...
    // Add dependency
    reverse_deps
        .entry(material_guid.clone())
        .or_default()
        .insert(texture_guid.clone());
    dependency_graph
        .entry(texture_guid.clone())
        .or_default()
