...
fn test_large_navmesh_100_triangles_baking() {
    ...
    assert_eq!(nav.tris.len(), 100, "All triangles should be flat and included");
    assert!(elapsed.as_millis() < 100, "Baking 100 triangles should take <100ms, took {:?}", elapsed);
}
...
fn test_large_navmesh_100_triangles_pathfinding() {
    ...
    assert!(elapsed.as_millis() < 10, "Pathfinding in 100-tri mesh should take <10ms (coverage overhead), took {:?}", elapsed);
    
    // Verify path goes from start to goal approximately
    assert!((path[0] - start).length() < 1.0, "Path should start near start position");
    assert!((path.last().unwrap() - goal).length() < 1.0, "Path should end near goal position");
}
...
fn test_large_navmesh_1000_triangles_baking() {
    ...
    assert!(elapsed.as_millis() < 2000, "Baking ~1000 triangles should take <2s (coverage overhead), took {:?}", elapsed);
    
    // Verify adjacency is built (spot check first triangle has neighbors)
    assert!(nav.tris[0].neighbors.len() > 0, "Grid triangles should have neighbors");
}
...
fn test_large_navmesh_1000_triangles_pathfinding() {
    ...
    assert!(path.len() >= 2);
    assert!(elapsed.as_millis() < 100, "Pathfinding in 1000-tri mesh should take <100ms (coverage overhead), took {:?}", elapsed);
}
...
fn test_large_navmesh_10000_triangles_stress() {
    ...
    assert!(bake_elapsed.as_secs() < 10, "Baking 10k triangles should take <10s, took {:?}", bake_elapsed);
    ...
    assert!(path.len() >= 2);
    assert!(path_elapsed.as_millis() < 100, "100-hop path should take <100ms (coverage overhead), took {:?}", path_elapsed);
}
...
fn test_dense_connectivity_graph() {
    ...
    assert!(multi_neighbor_count >= 20, "Most triangles in grid should have 2+ neighbors, found {}", multi_neighbor_count);
...
fn test_sparse_connectivity_linear_strip() {
    ...
    let avg_neighbors: f32 = nav.tris.iter().map(|t| t.neighbors.len()).sum::<usize>() as f32 / nav.tris.len() as f32;
    assert!(avg_neighbors >= 1.0 && avg_neighbors <= 3.0, "Linear strip should have 1-3 avg neighbors, found {}", avg_neighbors);
...
fn test_hierarchical_disconnected_islands() {
    ...
    assert_eq!(path2.len(), 0, "Path between disconnected islands should not exist");
}
...
fn test_long_path_50_hops() {
    ...
    assert!(path.len() >= 2);
    assert!(elapsed.as_millis() < 50, "50-hop path should take <50ms (coverage overhead), took {:?}", elapsed);
}
...
fn test_long_path_100_hops() {
    ...
    assert!(path.len() >= 2);
    assert!(elapsed.as_millis() < 10, "100-hop path should take <10ms, took {:?}", elapsed);
    
    // Verify smoothing worked (path should have fewer waypoints than 100+2)
    assert!(path.len() < 102, "Smoothing should reduce waypoint count from 102 to fewer, got {}", path.len());
}
...
fn test_multiple_sequential_queries() {
    ...
    for i in 0..100 {
        ...
        let path = nav.find_path(start, goal);
        assert!(path.len() >= 2 || (x < 1.0 && z < 1.0), "Query {} should find path or be same triangle", i);
    }
    let elapsed = start_time.elapsed();
    
    // 100 queries with coverage overhead
    assert!(elapsed.as_millis() < 1000, "100 queries should take <1s (coverage overhead), took {:?}", elapsed);
}
...
fn test_interleaved_baking_and_pathfinding() {
    ...
    assert!(path3.len() >= 2, "Original navmesh should still work after creating new one");
}
...
fn test_memory_consistency_1000_queries() {
    ...
    for i in 0..1000 {
        ...
        if i % 100 == 0 {
            assert!(path.len() >= 2 || (x < 1.0 && z < 1.0), "Query {} should remain consistent", i);
        }
    }
}
...
fn test_zero_length_path_same_position() {
    ...
    assert!(path.len() >= 2, "Zero-length path should still return start+goal");
...
fn test_pathfinding_with_max_step_validation() {
    ...
    assert_eq!(nav.max_step, 0.8, "max_step should be preserved during baking");
