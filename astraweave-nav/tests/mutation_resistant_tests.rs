...
mod triangle_geometry_tests {
    ...
    fn center_of_offset_triangle() {
        ...
        assert!(
            (c.x - 4.0).abs() < 1e-6,
            "center x should be 4.0, got {}",
            c.x
        );
        assert!(
            (c.y - 5.0).abs() < 1e-6,
            "center y should be 5.0, got {}",
            c.y
        );
        assert!(
            (c.z - 6.0).abs() < 1e-6,
            "center z should be 6.0, got {}",
            c.z
        );
    }
    ...
    fn area_of_degenerate_collinear_points() {
        ...
        assert!(
            area < 1e-6,
            "degenerate triangle area should be ~0, got {}",
            area
        );
    }
    ...
    fn perimeter_of_3_4_5_right_triangle() {
        ...
        assert!(
            (perimeter - 12.0).abs() < 1e-5,
            "perimeter should be 12.0, got {}",
            perimeter
        );
    }
    ...
    fn perimeter_of_equilateral_triangle_side_3() {
        ...
        let perimeter = t.perimeter();
        assert!(
            (perimeter - 9.0).abs() < 1e-5,
            "perimeter should be 9.0, got {}",
            perimeter
        );
    }
    ...
    fn edge_lengths_of_3_4_5_triangle() {
        ...
        // ca = distance(c, a) = 4.0
        assert!(
            (edges[0] - 3.0).abs() < 1e-5,
            "edge ab should be 3.0, got {}",
            edges[0]
        );
        assert!(
            (edges[1] - 5.0).abs() < 1e-5,
            "edge bc should be 5.0, got {}",
            edges[1]
        );
        assert!(
            (edges[2] - 4.0).abs() < 1e-5,
            "edge ca should be 4.0, got {}",
            edges[2]
        );
    }
    ...
    fn min_edge_length_is_3() {
        ...
        assert!(
            (min_edge - 3.0).abs() < 1e-5,
            "min edge should be 3.0, got {}",
            min_edge
        );
    }
    ...
    fn max_edge_length_is_5() {
        ...
        assert!(
            (max_edge - 5.0).abs() < 1e-5,
            "max edge should be 5.0, got {}",
            max_edge
        );
    }
    ...
    fn normal_normalized_has_length_1() {
        ...
        assert!(
            (len - 1.0).abs() < 1e-5,
            "normalized normal length should be 1.0, got {}",
            len
        );
    }
...
mod aabb_geometry_tests {
    ...
    fn center_of_offset_aabb() {
        ...
        assert!(
            (c.x - 3.0).abs() < 1e-6,
            "center x should be 3.0, got {}",
            c.x
        );
        assert!(
            (c.y - 6.0).abs() < 1e-6,
            "center y should be 6.0, got {}",
            c.y
        );
        assert!(
            (c.z - 8.0).abs() < 1e-6,
            "center z should be 8.0, got {}",
            c.z
        );
    }
    ...
    fn size_of_unit_cube_is_one() {
        ...
        assert!(
            (s.x - 1.0).abs() < 1e-6,
            "size x should be 1.0, got {}",
            s.x
        );
        assert!(
            (s.y - 1.0).abs() < 1e-6,
            "size y should be 1.0, got {}",
            s.y
        );
        assert!(
            (s.z - 1.0).abs() < 1e-6,
            "size z should be 1.0, got {}",
            s.z
        );
    }
    ...
    fn size_of_rectangular_box() {
        ...
        assert!(
            (s.x - 2.0).abs() < 1e-6,
            "size x should be 2.0, got {}",
            s.x
        );
        assert!(
            (s.y - 3.0).abs() < 1e-6,
            "size y should be 3.0, got {}",
            s.y
        );
        assert!(
            (s.z - 4.0).abs() < 1e-6,
            "size z should be 4.0, got {}",
            s.z
        );
    }
    ...
    fn half_extents_of_2x4x6_box() {
        ...
        assert!(
            (he.x - 1.0).abs() < 1e-6,
            "half_extent x should be 1.0, got {}",
            he.x
        );
        assert!(
            (he.y - 2.0).abs() < 1e-6,
            "half_extent y should be 2.0, got {}",
            he.y
        );
        assert!(
            (he.z - 3.0).abs() < 1e-6,
            "half_extent z should be 3.0, got {}",
            he.z
        );
    }
    ...
    fn volume_of_zero_aabb_is_0() {
        ...
        assert!(
            v.abs() < 1e-6,
            "volume of zero AABB should be ~0, got {}",
            v
        );
    }
    ...
    fn surface_area_of_unit_cube_is_6() {
        ...
        assert!(
            (sa - 6.0).abs() < 1e-6,
            "surface area should be 6.0, got {}",
            sa
        );
    }
    ...
    fn surface_area_of_2x3x4_box_is_52() {
        ...
        assert!(
            (sa - 52.0).abs() < 1e-6,
            "surface area should be 52.0, got {}",
            sa
        );
    }
    ...
    fn longest_axis_of_1x2x3_box_is_3() {
        ...
        assert!(
            ...
            "longest axis should be 3.0, got {}",
            longest
        );
    }
    ...
    fn shortest_axis_of_1x2x3_box_is_1() {
        ...
        assert!(
            (shortest - 1.0).abs() < 1e-6,
            "shortest axis should be 1.0, got {}",
            shortest
        );
    }
...
mod navtri_tests {
    ...
    fn navtri_slope_degrees_for_flat_is_0() {
        ...
        assert!(
            slope.abs() < 1e-3,
            "flat triangle slope should be ~0, got {}",
            slope
        );
    }
    ...
    fn navtri_slope_degrees_for_vertical_is_90() {
        ...
        assert!(
            (slope - 90.0).abs() < 1e-3,
            "vertical slope should be ~90, got {}",
            slope
        );
    }
    ...
    fn navtri_is_walkable_for_upward_normal() {
        ...
        assert!(
            tri.is_walkable(),
            "triangle with +Y normal should be walkable"
        );
    }
    ...
    fn navtri_is_not_walkable_for_downward_normal() {
        ...
        assert!(
            !tri.is_walkable(),
            "triangle with -Y normal should not be walkable"
        );
    }
    ...
    fn navtri_is_not_edge_with_3_neighbors() {
        ...
        assert!(
            !tri.is_edge(),
            "triangle with 3 neighbors should not be edge"
        );
    }
}
...
mod navmesh_stats_tests {
    ...
    fn average_neighbor_count_is_1() {
        ...
        assert!(
            (avg - 1.0).abs() < 1e-6,
            "average neighbor count should be 1.0, got {}",
            avg
        );
    }
