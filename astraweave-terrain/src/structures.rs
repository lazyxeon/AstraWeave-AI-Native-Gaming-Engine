...
impl StructureType {
    ...
    pub fn for_biome(biome: BiomeType) -> Vec<Self> {
        ...
        match biome {
            ...
            BiomeType::Swamp => vec![Cabin, Temple, AncientRuin, StoneCircle, Cave, Bridge],
            BiomeType::Beach => vec![Cottage, Tavern, Lighthouse, Temple, Cave, RockFormation],
            BiomeType::River => vec![
...
impl StructureGenerator {
    ...
    pub fn generate_structures(
        ...
        for _ in 0..structure_attempts {
            if let Some(structure) =
                self.try_place_structure(chunk, chunk_size, &available_structures, &result)?
            {
    ...
    fn try_place_structure(
        ...
        for _ in 0..MAX_ATTEMPTS {
            ...
            let x = self
                .rng
                .random_range(self.config.edge_buffer..chunk_size - self.config.edge_buffer);
            let z = self
                .rng
                .random_range(self.config.edge_buffer..chunk_size - self.config.edge_buffer);
            ...
            if local_x >= chunk.heightmap().resolution()
                || local_z >= chunk.heightmap().resolution()
            {
                continue;
            }
...
mod tests {
    use super::*;
    use crate::{ChunkId, WorldConfig, WorldGenerator};
