...
use crate::{ChunkId, meshing::ChunkMesh};
use glam::Vec3;
...
impl ChunkLodCache {
    ...
    pub fn memory_usage(&self) -> usize {
        let mut total = 0;
        if let Some(m) = &self.l0_mesh { total += m.memory_usage(); }
        if let Some(m) = &self.l1_mesh { total += m.memory_usage(); }
        if let Some(m) = &self.l2_mesh { total += m.memory_usage(); }
        if let Some(m) = &self.l3_mesh { total += m.memory_usage(); }
        total
    }
}
...
impl LodManager {
    ...
    pub fn cache_mesh(&mut self, chunk_id: ChunkId, lod: LodLevel, mesh: Arc<ChunkMesh>) {
        let cache = self.mesh_cache.entry(chunk_id).or_insert_with(ChunkLodCache::new);
    ...
    pub fn update_chunk_lod(&mut self, chunk_id: ChunkId, camera_pos: Vec3) -> bool {
        ...
        let state = self.chunk_states.entry(chunk_id).or_insert_with(|| {
            ChunkLodState {
        ...
        if target_lod != state.current_lod {
            ...
            let threshold = self.config.get_threshold(state.current_lod, target_lod, increasing_detail);
