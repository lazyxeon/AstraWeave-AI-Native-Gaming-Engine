...
fn stress_test_100k_entities() {
    ...
    for i in 0..100_000 {
        ...
        world.insert(entity, Health { hp: 100, max_hp: 100 });
        entities.push(entity);
    }
    
    let spawn_time = start.elapsed();
    println!("  Spawned in {:?} ({:.2} entities/sec)", 
        spawn_time, 
    ...
    println!("  Modified in {:?} ({:.2} entities/sec)", 
        modify_time,
    ...
    println!("  Queried in {:?} (found {} entities)", query_time, positions.len());
    ...
    println!("  Despawned in {:?} ({:.2} entities/sec)", 
        despawn_time,
    ...
    println!("  Spawn:   {:?} ({:.2}%)", spawn_time, 100.0 * spawn_time.as_secs_f64() / total_time.as_secs_f64());
    println!("  Modify:  {:?} ({:.2}%)", modify_time, 100.0 * modify_time.as_secs_f64() / total_time.as_secs_f64());
    println!("  Query:   {:?} ({:.2}%)", query_time, 100.0 * query_time.as_secs_f64() / total_time.as_secs_f64());
    println!("  Despawn: {:?} ({:.2}%)", despawn_time, 100.0 * despawn_time.as_secs_f64() / total_time.as_secs_f64());
    
    // Validate performance
    assert!(spawn_time.as_secs_f64() < 5.0, "Spawning too slow: {:?}", spawn_time);
    assert!(modify_time.as_secs_f64() < 5.0, "Modifying too slow: {:?}", modify_time);
    assert!(query_time.as_secs_f64() < 1.0, "Querying too slow: {:?}", query_time);
    assert!(despawn_time.as_secs_f64() < 5.0, "Despawning too slow: {:?}", despawn_time);
}
...
fn stress_test_component_thrashing() {
    ...
    for cycle in 0..1000 {
        ...
        for &entity in &entities {
            world.insert(entity, Velocity { dx: cycle, dy: cycle });
        }
    ...
    let first_100_avg: std::time::Duration = cycle_times[0..100].iter().sum::<std::time::Duration>() / 100;
    let last_100_avg: std::time::Duration = cycle_times[900..1000].iter().sum::<std::time::Duration>() / 100;
...
fn stress_test_memory_leak_detection() {
    ...
    for cycle in 0..10_000 {
        ...
        assert_eq!(world.entity_count(), 0, "Memory leak detected at cycle {}", cycle);
...
fn stress_test_query_performance() {
    ...
    println!("  Found {} entities in {:?}", pos_entities.len(), query1_time);
    ...
    println!("  Found {} entities in {:?}", vel_entities.len(), query2_time);
    ...
    println!("  Found {} entities in {:?}", health_entities.len(), query3_time);
    ...
    println!("  Found {} entities in {:?}", armor_entities.len(), query4_time);
    ...
    assert!(query1_time.as_millis() < 100, "Query 1 too slow: {:?}", query1_time);
    assert!(query2_time.as_millis() < 100, "Query 2 too slow: {:?}", query2_time);
    assert!(query3_time.as_millis() < 100, "Query 3 too slow: {:?}", query3_time);
    assert!(query4_time.as_millis() < 100, "Query 4 too slow: {:?}", query4_time);
}
...
fn stress_test_archetype_explosion() {
    ...
    for i in 0..1024_u32 {
        ...
        if i & 0b0000000001 != 0 { world.insert(entity, Position { x: i as i32, y: i as i32 }); }
        if i & 0b0000000010 != 0 { world.insert(entity, Velocity { dx: i as i32, dy: i as i32 }); }
        if i & 0b0000000100 != 0 { world.insert(entity, Health { hp: 100, max_hp: 100 }); }
        if i & 0b0000001000 != 0 { world.insert(entity, Armor { defense: 50 }); }
        if i & 0b0000010000 != 0 { world.insert(entity, Damage { attack: 25 }); }
        if i & 0b0000100000 != 0 { world.insert(entity, Name { id: i }); }
        if i & 0b0001000000 != 0 { world.insert(entity, Tag1); }
        if i & 0b0010000000 != 0 { world.insert(entity, Tag2); }
        if i & 0b0100000000 != 0 { world.insert(entity, Tag3); }
        if i & 0b1000000000 != 0 { world.insert(entity, Tag4); }
    ...
    assert!(pos_count >= 400 && pos_count <= 600, "Position count unexpected: {}", pos_count);
    assert!(vel_count >= 400 && vel_count <= 600, "Velocity count unexpected: {}", vel_count);
    assert!(health_count >= 400 && health_count <= 600, "Health count unexpected: {}", health_count);
    
    // Validate query performance (< 50ms across many archetypes)
    assert!(query_time.as_millis() < 50, "Query too slow: {:?}", query_time);
    ...
    for i in 0..10_000 {
        match i % 10 {
            0..=3 => {
                // Spawn (40%)
                let entity = world.spawn();
                world.insert(entity, Position { x: i as i32, y: i as i32 });
                entities.push(entity);
            }
            4..=5 => {
                // Insert component (20%)
                if !entities.is_empty() {
                    ...
                    world.insert(entity, Velocity { dx: i as i32, dy: i as i32 });
                }
            }
            6..=7 => {
                // Query (20%)
