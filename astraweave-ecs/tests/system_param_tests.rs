...
use astraweave_ecs::{Component, World, Query, Query2, Query2Mut};
...
fn test_query2_immutability() {
    ...
    // Verify original data unchanged
    assert_eq!(world.get::<Position>(entity), Some(&Position { x: 10.0, y: 20.0 }));
    assert_eq!(world.get::<Velocity>(entity), Some(&Velocity { x: 1.0, y: 2.0 }));
}
...
fn test_query2mut_filtered_by_second_component() {
    ...
    assert_eq!(world.get::<Position>(e2), Some(&Position { x: 3.5, y: 4.5 }));
}
...
fn test_query2mut_different_archetypes() {
    ...
    assert_eq!(world.get::<Position>(e1), Some(&Position { x: 1.5, y: 2.5 }));
    assert_eq!(world.get::<Position>(e2), Some(&Position { x: 4.0, y: 5.0 }));
}
...
fn test_query2mut_second_component_immutable() {
    ...
    assert_eq!(world.get::<Position>(entity), Some(&Position { x: 15.0, y: 20.0 }));
    assert_eq!(world.get::<Velocity>(entity), Some(&Velocity { x: 1.0, y: 2.0 }));
}
...
fn test_query2mut_archetype_filtering() {
    ...
    // e2 should be updated
    assert_eq!(world.get::<Position>(e2), Some(&Position { x: 4.0, y: 4.0 }));
}
