...
use astraweave_ecs::{World, CommandBuffer, Entity};
use std::any::TypeId;
...
fn test_archetype_storage_iter() {
    use astraweave_ecs::archetype::{ArchetypeStorage, ArchetypeSignature};
...
fn test_archetype_storage_archetypes_with_component() {
    ...
    let with_position: Vec<_> = storage.archetypes_with_component(TypeId::of::<Position>()).collect();
    assert_eq!(with_position.len(), 2);
    
    // Find archetypes with Health (should return 0)
    let with_health: Vec<_> = storage.archetypes_with_component(TypeId::of::<Health>()).collect();
...
fn test_rng_gen_range_float() {
    ...
    for _ in 0..50 {
        ...
        assert!(val >= 0.0 && val < 1.0, "Value should be in range [0.0, 1.0)");
    }
    
    // Test negative range
    for _ in 0..50 {
        let val = rng.gen_range(-10.0..10.0);
        assert!(val >= -10.0 && val < 10.0, "Value should be in range [-10.0, 10.0)");
    }
}
