...
use astraweave_ecs::{Entity, blob_vec::BlobVec, sparse_set::{SparseSet, SparseSetData}};
    ...
    for count in [100, 1000, 10000] {
        ...
        group.bench_with_input(BenchmarkId::new("BlobVec", count), &count, |b, &count| {
            b.iter(|| {
                ...
                for i in 0..count {
                    let pos = Position { x: i as f32, y: i as f32 * 2.0, z: i as f32 * 3.0 };
                    unsafe { blob.push(pos); }
                }
                black_box(blob);
            });
        });
...
fn bench_blobvec_vs_boxed_iteration(c: &mut Criterion) {
    ...
    for count in [100, 1000, 10000] {
        ...
        group.bench_with_input(BenchmarkId::new("Vec_Box_downcast", count), &count, |b, _| {
            b.iter(|| {
                let mut sum = 0.0f32;
                for item in &vec {
...
fn bench_blobvec_vs_boxed_mutation(c: &mut Criterion) {
    ...
    for count in [100, 1000, 10000] {
        ...
        group.bench_with_input(BenchmarkId::new("BlobVec_slice_mut", count), &count, |b, _| {
            b.iter(|| {
        ...
        group.bench_with_input(BenchmarkId::new("Vec_Box_downcast_mut", count), &count, |b, _| {
            b.iter(|| {
                for item in &mut vec {
