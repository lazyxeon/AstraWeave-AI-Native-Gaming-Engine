...
use astraweave_render_bevy::{
    DirectionalLight, RenderAdapter, RenderMaterial, RenderMesh, RenderTransform, ShadowRenderer,
    CascadeShadowConfig, CASCADE_COUNT, CASCADE_RESOLUTION,
};
...
impl DemoApp {
    fn setup_scene(&mut self) {
        ...
        self.world.insert(light, DirectionalLight {
    ...
    fn create_render_resources(&mut self) {
        ...
        let vertices: &[Vertex] = &[
            ...
            Vertex { position: [-15.0, 0.0, -15.0], normal: [0.0, 1.0, 0.0] },
            Vertex { position: [ 15.0, 0.0, -15.0], normal: [0.0, 1.0, 0.0] },
            Vertex { position: [ 15.0, 0.0,  15.0], normal: [0.0, 1.0, 0.0] },
            Vertex { position: [-15.0, 0.0,  15.0], normal: [0.0, 1.0, 0.0] },
        ];
        ...
        println!("   Resolution: {}×{} per cascade", CASCADE_RESOLUTION, CASCADE_RESOLUTION);
        println!("   Total shadow map memory: ~{} MB", 
                 (CASCADE_COUNT * CASCADE_RESOLUTION as usize * CASCADE_RESOLUTION as usize * 4) / (1024 * 1024));
    ...
    fn update_and_render(&mut self) -> Result<()> {
        ...
        if let Some(shadow_renderer) = &mut self.shadow_renderer {
            ...
            if let Some(light_data) = lights.first() {
                shadow_renderer.calculate_cascades(&view_mat, &proj_mat, light_data.light.direction.normalize());
                ...
                unsafe {
                    if !PRINTED_CASCADE_DEBUG {
                        ...
                        let light_space = shadow_renderer.cascades[cascade_idx]
                            .view_proj_matrix
                        ...
                        let ground_light = shadow_renderer.cascades[cascade_idx].view_proj_matrix * ground_point.extend(1.0);
                ...
                unsafe {
                    if !PRINTED {
                        println!("\n🌞 Shadow Cascade Info:");
                        for (i, cascade) in shadow_renderer.cascades.iter().enumerate() {
                            println!("   Cascade {}: {:.1}m → {:.1}m", i, cascade.near, cascade.far);
                        }
                        PRINTED = true;
                    }
                }
        ...
        if let (Some(shadow_renderer), Some(shadow_pipeline), Some(vbuf), Some(ibuf)) = (
            ...
            let cascade_data: Vec<CascadeUniform> = shadow_renderer.cascades
                .iter()
            ...
            for (cascade_idx, cascade) in shadow_renderer.cascades.iter().enumerate() {
                ...
                let cascade_view = shadow_renderer.shadow_texture.create_view(&wgpu::TextureViewDescriptor {
                ...
                // Render ground into shadow map
                {
                    ...
                    shadow_pass.set_bind_group(0, self.shadow_ground_bind_group.as_ref().unwrap(), &[]);
                ...
                // Render cube into shadow map
                {
                    ...
                    shadow_pass.set_bind_group(0, self.shadow_cube_bind_group.as_ref().unwrap(), &[]);
        ...
        if let (Some(pipeline), Some(ground_bg), Some(cube_bg), Some(shadow_bg), Some(vbuf), Some(ibuf), Some(depth_view)) = (
            &self.main_pipeline,
            ...
            // Get directional light direction
            let light_dir = self.adapter.directional_lights()
                .first()
