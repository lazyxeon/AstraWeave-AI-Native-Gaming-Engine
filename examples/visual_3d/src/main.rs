...
fn main() -> anyhow::Result<()> {
    ...
    struct MaterialLiveDoc { base_color: [f32;4], metallic: f32, roughness: f32, texture_path: Option<String> }
                ...
                Event::AboutToWait => {
                    ...
                    if use_checkerboard != last_checkerboard {
                        if use_checkerboard {
                            ...
                            let w: u32 = 256; let h: u32 = 256; let tile: u32 = 32;
                            let mut data = vec![0u8; (w*h*4) as usize];
                            for y in 0..h {
                                for x in 0..w {
                                    let tx = (x / tile) % 2; let ty = (y / tile) % 2;
                    ...
                    if load_mesh && !mesh_loaded {
                        ...
                        if let Ok(bytes) = std::fs::read("assets/demo_plane.gltf") {
                            if let Ok((mesh, mat)) = gl::load_first_mesh_and_material(&bytes) {
                                ...
                                let gpu_mesh = renderer.create_mesh_from_full_arrays(&mesh.positions, &mesh.normals, &mesh.tangents, &mesh.texcoords, &mesh.indices);
                                ...
                                if let Some(img) = mat.metallic_roughness_texture { renderer.set_metallic_roughness_from_rgba8(img.width, img.height, &img.rgba8); }
                                if let Some(img) = mat.normal_texture { renderer.set_normal_from_rgba8(img.width, img.height, &img.rgba8); }
                                mesh_loaded = true;
                            } else {
                    ...
                    if load_skinned_gltf && !skinned_gltf_loaded {
                        ...
                        if let Ok(bytes) = std::fs::read(path) {
                            if let Ok((mesh, clip, mat_opt)) = gl::load_first_skinned_mesh_and_idle(&bytes) {
                                // Convert to renderer format
                                let verts: Vec<SkinnedVertex> = mesh.vertices.into_iter().map(|v| SkinnedVertex{ position: v.position, normal: v.normal, tangent: v.tangent, joints: v.joints, weights: v.weights }).collect();
                                renderer.set_skinned_mesh(&verts, &mesh.indices);
                                if let Some(mat) = mat_opt {
                                    renderer.set_material_params(mat.base_color_factor, mat.metallic_factor, mat.roughness_factor);
                                    if let Some(img) = mat.base_color_texture { renderer.set_albedo_from_rgba8(img.width, img.height, &img.rgba8); }
                                    if let Some(img) = mat.metallic_roughness_texture { renderer.set_metallic_roughness_from_rgba8(img.width, img.height, &img.rgba8); }
                                    if let Some(img) = mat.normal_texture { renderer.set_normal_from_rgba8(img.width, img.height, &img.rgba8); }
                                }
                                ...
                                for _ in 0..skinned_gltf_joint_count { mats.push(glam::Mat4::IDENTITY); }
                                renderer.update_skin_palette(&mats);
                                // Store clip if present
                                if let Some(c) = clip { skinned_gltf_clip = Some((c.times, c.rotations)); }
                    ...
                    if skinned_gltf_loaded {
                        if let Some((times, rotations)) = &skinned_gltf_clip {
                            if !times.is_empty() && !rotations.is_empty() {
                                ...
                                let mut idx = 0usize;
                                while idx + 1 < times.len() && times[idx + 1] < lt { idx += 1; }
                                let i0 = idx;
                                ...
                                if skinned_gltf_joint_count == 0 {
                                    ...
                                    mats.push(glam::Mat4::from_quat(q));
                                    for _ in 1..skinned_gltf_joint_count { mats.push(glam::Mat4::IDENTITY); }
                                }
