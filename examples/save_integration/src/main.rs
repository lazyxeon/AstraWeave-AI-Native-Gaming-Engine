...
fn main() -> Result<()> {
    ...
    println!(
        "  Player at {:?} with {} HP",
        world.pos_of(player).unwrap(),
        world.health(player).unwrap().hp
    );
    println!(
        "  Companion at {:?} with {} HP",
        world.pos_of(companion).unwrap(),
        world.health(companion).unwrap().hp
    );
    println!(
        "  Enemy at {:?} with {} HP",
        world.pos_of(enemy).unwrap(),
        world.health(enemy).unwrap().hp
    );
    ...
    println!(
        "Original inventory credits: {}",
        save_bundle.inventory.credits
    );
    println!(
        "Loaded inventory credits: {}",
        loaded_bundle.inventory.credits
    );
    ...
    for save_meta in save_manager.list_saves("demo_player")? {
        println!(
            "[Slot {:02}] {} - Schema v{}",
            save_meta.slot, save_meta.created_at, save_meta.schema
        );
    }
