...
fn compute_tangents(positions: &[[f32; 3]], uvs: &[[f32; 2]], normals: &[[f32; 3]], indices: &[u32]) -> Vec<[f32; 4]> {
    ...
    tangents.into_iter().zip(bitangents.into_iter()).zip(normals.iter()).map(|((t, b), n)| {
...
pub fn load_gltf(path: impl AsRef<Path>) -> Result<LoadedMesh> {
    ...
    let import_res = match gltf::import(path) {
        Ok(x) => Ok(x),
        Err(e) => {
            log::warn!("gltf::import(path) failed for {}: {:?}, attempting import_slice fallback", path.display(), e);
    ...
    let (document, buffers, _images) = import_res
    ...
    for mesh in document.meshes() {
        ...
        log::info!("Processing mesh: '{}' with {} primitives", mesh_name, mesh.primitives().len());
        ...
        for (prim_idx, primitive) in mesh.primitives().enumerate() {
            ...
            let normals: Vec<[f32; 3]> = reader
                ...
                .unwrap_or_else(|| {
                    log::info!("  Primitive {}: No normals found, generating smooth normals", prim_idx);
            ...
            let uvs: Vec<[f32; 2]> = reader
                ...
                .unwrap_or_else(|| {
                    log::info!("  Primitive {}: No UVs found, generating planar UVs", prim_idx);
            ...
            let tangents: Vec<[f32; 4]> = if let Some(tangents_iter) = reader.read_tangents() {
                ...
                log::info!("  Primitive {}: No tangents found, computing tangents", prim_idx);
    ...
    log::info!(
        ...
        all_vertices.len(),
        all_indices.len() / 3,
        document.meshes().map(|m| m.primitives().len()).sum::<usize>()
    );
