...
impl ShowcaseApp {
    ...
    fn load_texture(&self, path: &str) -> Result<(wgpu::Texture, wgpu::TextureView), Box<dyn std::error::Error>> {
    ...
    fn create_material(&mut self, name: &str, texture_path: &str) -> Result<usize, Box<dyn std::error::Error>> {
        // Load albedo
        ...
        let bind_group = self.device.create_bind_group(&wgpu::BindGroupDescriptor {
            ...
            entries: &[
                ...
                wgpu::BindGroupEntry {
                    binding: 2,
                    resource: wgpu::BindingResource::TextureView(normal_view.as_ref().unwrap_or(&view)),
                },
                // Roughness/MRA map (binding 3) if present, otherwise fallback to albedo view
                wgpu::BindGroupEntry {
                    binding: 3,
                    resource: wgpu::BindingResource::TextureView(roughness_view.as_ref().unwrap_or(&view)),
                },
            ],
        });
    ...
    fn create_material_with_fallback(&mut self, name: &str, texture_path: &str, fallback_color: [u8; 4]) -> usize {
    ...
    /// Load a GLTF mesh
    fn load_gltf_mesh(&mut self, path: &str, material_index: usize) -> Result<usize, Box<dyn std::error::Error>> {
        // Try to canonicalize path for better error messages
            ...
            Err(e) => {
                ...
                let fallback = std::path::Path::new("assets").join(path.strip_prefix("assets/").unwrap_or(path));
                log::warn!("Primary gltf import failed for '{}', trying fallback path: {}", path, fallback.display());
                ...
                if res.is_err() {
                    // Print detailed diagnostics before returning the original error
                        ...
                        log::warn!("Canonical import failed, trying fallback path: {}", fallback.display());
                        let res2 = gltf_loader::load_gltf(&fallback);
                        if res2.is_err() {
                            log::error!("Detailed GLTF load failure for '{}': {}\nCanonical: {}\nFallback: {}", path, e, canonical.display(), fallback.display());
                        }
                }
        ...
        println!("  ✅ Loaded GLTF: {} ({} vertices, {} triangles)", 
            path, loaded.vertices.len(), loaded.indices.len() / 3);
    ...
    let vertices = vec![
            ...
            Vertex { position: [-half_size, 0.0, -half_size], normal: [0.0, 1.0, 0.0], uv: [0.0, 0.0], color: green, tangent: [1.0, 0.0, 0.0, 1.0] },
            Vertex { position: [half_size, 0.0, -half_size], normal: [0.0, 1.0, 0.0], uv: [uv_repeat, 0.0], color: green, tangent: [1.0, 0.0, 0.0, 1.0] },
            Vertex { position: [half_size, 0.0, half_size], normal: [0.0, 1.0, 0.0], uv: [uv_repeat, uv_repeat], color: green, tangent: [1.0, 0.0, 0.0, 1.0] },
            Vertex { position: [-half_size, 0.0, half_size], normal: [0.0, 1.0, 0.0], uv: [0.0, uv_repeat], color: green, tangent: [1.0, 0.0, 0.0, 1.0] },
        ];
    ...
    let vertices = vec![
            // Front face
            Vertex { position: [-s, -s,  s], normal: [0.0, 0.0, 1.0], uv: [0.0, 1.0], color: white, tangent: [1.0, 0.0, 0.0, 1.0] },
            Vertex { position: [ s, -s,  s], normal: [0.0, 0.0, 1.0], uv: [1.0, 1.0], color: white, tangent: [1.0, 0.0, 0.0, 1.0] },
            Vertex { position: [ s,  s,  s], normal: [0.0, 0.0, 1.0], uv: [1.0, 0.0], color: white, tangent: [1.0, 0.0, 0.0, 1.0] },
            Vertex { position: [-s,  s,  s], normal: [0.0, 0.0, 1.0], uv: [0.0, 0.0], color: white, tangent: [1.0, 0.0, 0.0, 1.0] },
            // Back face
            Vertex { position: [ s, -s, -s], normal: [0.0, 0.0, -1.0], uv: [0.0, 1.0], color: white, tangent: [1.0, 0.0, 0.0, 1.0] },
            Vertex { position: [-s, -s, -s], normal: [0.0, 0.0, -1.0], uv: [1.0, 1.0], color: white, tangent: [1.0, 0.0, 0.0, 1.0] },
            Vertex { position: [-s,  s, -s], normal: [0.0, 0.0, -1.0], uv: [1.0, 0.0], color: white, tangent: [1.0, 0.0, 0.0, 1.0] },
            Vertex { position: [ s,  s, -s], normal: [0.0, 0.0, -1.0], uv: [0.0, 0.0], color: white, tangent: [1.0, 0.0, 0.0, 1.0] },
            // Top face
            Vertex { position: [-s,  s,  s], normal: [0.0, 1.0, 0.0], uv: [0.0, 1.0], color: white, tangent: [1.0, 0.0, 0.0, 1.0] },
            Vertex { position: [ s,  s,  s], normal: [0.0, 1.0, 0.0], uv: [1.0, 1.0], color: white, tangent: [1.0, 0.0, 0.0, 1.0] },
            Vertex { position: [ s,  s, -s], normal: [0.0, 1.0, 0.0], uv: [1.0, 0.0], color: white, tangent: [1.0, 0.0, 0.0, 1.0] },
            Vertex { position: [-s,  s, -s], normal: [0.0, 1.0, 0.0], uv: [0.0, 0.0], color: white, tangent: [1.0, 0.0, 0.0, 1.0] },
            // Bottom face
            Vertex { position: [-s, -s, -s], normal: [0.0, -1.0, 0.0], uv: [0.0, 1.0], color: white, tangent: [1.0, 0.0, 0.0, 1.0] },
            Vertex { position: [ s, -s, -s], normal: [0.0, -1.0, 0.0], uv: [1.0, 1.0], color: white, tangent: [1.0, 0.0, 0.0, 1.0] },
            Vertex { position: [ s, -s,  s], normal: [0.0, -1.0, 0.0], uv: [1.0, 0.0], color: white, tangent: [1.0, 0.0, 0.0, 1.0] },
            Vertex { position: [-s, -s,  s], normal: [0.0, -1.0, 0.0], uv: [0.0, 0.0], color: white, tangent: [1.0, 0.0, 0.0, 1.0] },
            // Right face
            Vertex { position: [ s, -s,  s], normal: [1.0, 0.0, 0.0], uv: [0.0, 1.0], color: white, tangent: [0.0, 0.0, 1.0, 1.0] },
            Vertex { position: [ s, -s, -s], normal: [1.0, 0.0, 0.0], uv: [1.0, 1.0], color: white, tangent: [0.0, 0.0, 1.0, 1.0] },
            Vertex { position: [ s,  s, -s], normal: [1.0, 0.0, 0.0], uv: [1.0, 0.0], color: white, tangent: [0.0, 0.0, 1.0, 1.0] },
            Vertex { position: [ s,  s,  s], normal: [1.0, 0.0, 0.0], uv: [0.0, 0.0], color: white, tangent: [0.0, 0.0, 1.0, 1.0] },
            // Left face
            Vertex { position: [-s, -s, -s], normal: [-1.0, 0.0, 0.0], uv: [0.0, 1.0], color: white, tangent: [0.0, 0.0, -1.0, 1.0] },
            Vertex { position: [-s, -s,  s], normal: [-1.0, 0.0, 0.0], uv: [1.0, 1.0], color: white, tangent: [0.0, 0.0, -1.0, 1.0] },
            Vertex { position: [-s,  s,  s], normal: [-1.0, 0.0, 0.0], uv: [1.0, 0.0], color: white, tangent: [0.0, 0.0, -1.0, 1.0] },
    ...
    fn create_model_bind_group(&self, position: Vec3, rotation: Quat, scale: Vec3) -> wgpu::BindGroup {
        // Build model matrix
    ...
    fn load_scene(&mut self) {
        ...
        let tree_default = self.load_gltf_mesh_with_fallback("assets/models/tree_default.glb", brown_mat);
        let tree_oak = self.load_gltf_mesh_with_fallback("assets/models/tree_oak.glb", brown_mat);
        let tree_pine = self.load_gltf_mesh_with_fallback("assets/models/tree_pineDefaultA.glb", brown_mat);
        let tree_meshes = vec![tree_default, tree_oak, tree_pine];
        
        // Load multiple rock variants (gray material for stones)
        let rock_large_a = self.load_gltf_mesh_with_fallback("assets/models/rock_largeA.glb", gray_mat);
        let rock_large_b = self.load_gltf_mesh_with_fallback("assets/models/rock_largeB.glb", gray_mat);
        let rock_small_a = self.load_gltf_mesh_with_fallback("assets/models/rock_smallA.glb", gray_mat);
        ...
        let building_mesh = self.load_gltf_mesh_with_fallback("assets/models/rock_largeA.glb", gray_mat);
        ...
        self.objects.push(SceneObject {
            ...
            scale: Vec3::ONE,
            model_bind_group: self.create_model_bind_group(Vec3::new(0.0, -5.0, 0.0), Quat::IDENTITY, Vec3::ONE),
        });
        ...
        match event {
            Event::WindowEvent { event, .. } => match event {
                ...
                WindowEvent::RedrawRequested => {
                    ...
                    fps_timer += dt;
                    if fps_timer >= 1.0 {
                        println!("📊 FPS: {} ({:.2} ms/frame)", frame_count, 1000.0 / frame_count as f32);
                        frame_count = 0;
                        fps_timer = 0.0;
                    }
