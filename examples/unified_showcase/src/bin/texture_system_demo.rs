...
use texture_utils::{find_normal_map, generate_placeholder_texture, resolve_texture_path};
...
fn main() -> Result<()> {
    ...
    if !config_path.exists() {
        println!(
            "WARNING: Atlas configuration not found at {}",
            config_path.display()
        );
            ...
            if !texture_path.exists() {
                ...
                if let Some(normal_path) = &entry.normal_path {
                    ...
                    if !normal_full_path.exists() {
                        texture_utils::save_placeholder_texture(
                            &normal_full_path,
                            size,
                            [128, 128, 255, 255],
                        )?;
