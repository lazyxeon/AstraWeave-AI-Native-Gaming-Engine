...
impl TextureGenerator {
    ...
    pub fn generate_normal_map(&self, height_map: &RgbaImage) -> RgbaImage {
        ...
        for y in 1..height - 1 {
            for x in 1..width - 1 {
                ...
                normal_map.put_pixel(x, y, image::Rgba([nx as u8, ny as u8, nz as u8, 255]));
            }
        }
    ...
    pub fn generate_dirt(&self, width: u32, height: u32) -> RgbaImage {
        ...
        for y in 0..height {
            for x in 0..width {
                ...
                image.put_pixel(x, y, image::Rgba([r as u8, g as u8, b as u8, 255]));
            }
        }
            ...
            for x in 0..width {
                ...
                match wall_type {
                    "wood" => {
                        ...
                        let wood_grain =
                            self.perlin.get([nx * pattern_scale.0, plank_y * 50.0, 0.0]) * 0.5
                                + 0.5;
                        let detail = self.perlin.get([nx * 40.0, ny * 40.0, 0.0]) * 0.1;
                    ...
                    "stone" => {
                        ...
                        let block_color =
                            self.perlin.get([block_x * 100.0, block_y * 100.0, 0.0]) * 0.2 + 0.8;
                    ...
                    "adobe" => {
                        ...
                        let base =
                            self.perlin
                                .get([nx * pattern_scale.0, ny * pattern_scale.1, 0.0])
                                * 0.3
                                + 0.7;
            ...
            for x in 0..width {
                ...
                let special_detail = match character_type {
                    ...
                    "merchant" => {
                        ...
                        if part == "body" && (((nx * 8.0).floor() + (ny * 8.0).floor()) % 2.0 < 1.0)
                        {
                            0.15
                        } else {
                            0.0
    ...
    pub fn generate_skybox_side(
        ...
        let (horizon_color, zenith_color, sun_color) = match time_of_day {
            ...
            _ => ((0.7, 0.8, 0.9), (0.4, 0.6, 1.0), (1.0, 0.95, 0.8)),
        };
        ...
        for y in 0..height {
            for x in 0..width {
                ...
                if time_of_day == "day" && side != "bottom" {
                    ...
                    let cloud_base =
                        self.perlin.get([nx * cloud_scale, ny * cloud_scale, 0.0]) * 0.5 + 0.5;
                    let cloud_detail =
                        self.perlin
                            .get([nx * cloud_scale * 3.0, ny * cloud_scale * 3.0, 0.0])
                            * 0.25;

                    if cloud_base + cloud_detail > cloud_coverage {
                        let cloud_density = ((cloud_base + cloud_detail - cloud_coverage)
                            / (1.0 - cloud_coverage))
                            .powf(0.5);
                ...
                if side != "bottom" {
                    let sun_distance =
                        ((nx - sun_dir.0).powf(2.0) + (ny - sun_dir.1).powf(2.0)).sqrt();
                ...
                // Combine all factors
                let r_out = r * (1.0 - celestial_factor - cloud_factor)
                    + sun_color.0 * celestial_factor
                    + cloud_factor
                    + star_factor;
                let g_out = g * (1.0 - celestial_factor - cloud_factor)
                    + sun_color.1 * celestial_factor
                    + cloud_factor
                    + star_factor;
                let b_out = b * (1.0 - celestial_factor - cloud_factor)
                    + sun_color.2 * celestial_factor
                    + cloud_factor
                    + star_factor;
    ...
    pub fn generate_water(&self, width: u32, height: u32) -> (RgbaImage, RgbaImage) {
        ...
        for y in 0..height {
            for x in 0..width {
                ...
                height_map.put_pixel(x, y, image::Luma([height_value as u8]));
            }
        }
        ...
        for y in 0..height {
            for x in 0..width {
                ...
                diffuse.put_pixel(
                    ...
                    y,
                    image::Rgba([
                        r as u8, g as u8, b as u8, 200, // Slight transparency
                    ]),
                );
            }
        }
        ...
        for y in 1..height - 1 {
            for x in 1..width - 1 {
                ...
                normal.put_pixel(x, y, image::Rgba([nx as u8, ny as u8, nz as u8, 255]));
            }
        }
...
pub fn generate_all_textures(
    ...
    for time in &["day", "night"] {
        for side in &["top", "bottom", "left", "right", "front", "back"] {
            let skybox_side =
                generator.generate_skybox_side(texture_size, texture_size, side, time);
