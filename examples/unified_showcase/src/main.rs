...
fn pattern_noise(seed: u32, x: u32, y: u32) -> u8 {
    let mut value = seed
        ...
        .wrapping_add((x + 1) * 668_265_263)
        .wrapping_add((y + 1) * 362_437_202);
    value ^= value >> 13;
    value = value.wrapping_mul(1_270_234_577);
    ((value >> 24) & 0xFF) as u8
}

fn try_load_material_image(
    key: &str,
    suffix: &str,
    width: u32,
    height: u32,
    ...
    let candidates = [
        format!("materials/{}{}.png", key, suffix),
        format!("{}{}.png", key, suffix),
    ];
    ...
    for candidate in candidates.iter() {
        ...
        if path.exists() {
            match load_rgba_image(&path) {
                ...
                Err(err) => {
                    eprintln!(
                        "⚠️ Failed to read material texture {} ({}). Using fallback.",
                        path.display(),
                        err
                    );
                }
...
fn load_material_layer_images_fallback(
    ...
    let albedo = if let Some(image) =
        try_load_material_image(def.key, "", width, height)
            .or_else(|| try_load_material_image(def.key, "_albedo", width, height))
    {
        image
    } else {
        println!(
            "• Procedural fallback albedo generated for material '{}'.",
            def.display_name
        );
        def.fallback_albedo(width, height)
    };

    let normal = try_load_material_image(def.key, "_n", width, height)
        .unwrap_or_else(|| create_flat_normal_image(width, height));

    let mra = try_load_material_image(def.key, "_mra", width, height)
        .unwrap_or_else(|| create_default_mra_image(width, height));

    (albedo, normal, mra)
}

fn atlas_entry_for_layer(key: &str) -> Option<&'static str> {
    match key {
        "grass" => Some("terrain.grass"),
        "dirt" => Some("terrain.dirt"),
        "stone" => Some("terrain.stone"),
        "sand" => Some("terrain.sand"),
        "forest_floor" => Some("terrain.forest_floor"),
        "tree_bark" => Some("trees.trunk"),
        "tree_leaves" => Some("trees.leaves_oak"),
        "rock_lichen" => Some("terrain.stone"),
        "rock_slate" => Some("terrain.stone"),
        "plaster" => Some("houses.stone_wall"),
        "roof_tile" => Some("houses.wood_roof"),
        "cloth" => Some("characters.merchant"),
        _ => None,
    }
}
...
fn create_mra_from_values(width: u32, height: u32, roughness: f32, metallic: f32) -> RgbaImage {
    ...
    let r = (roughness.clamp(0.0, 1.0) * 255.0).round() as u8;
    create_solid_rgba_image(width, height, [m, r, 255, 255])
}
...
fn load_material_layer_from_manager(
    ...
    let entry_id = atlas_entry_for_layer(def.key)?;
    let (diffuse_path, normal_path, entry_roughness, entry_metallic) = {
        let textures = manager.get_all_textures();
        let entry = textures.get(entry_id)?;
        (
            entry.diffuse_path.clone(),
    ...
    if let Err(err) = manager.load_texture(entry_id, asset_root) {
        eprintln!(
            "⚠️ Failed to load atlas texture '{}': {}",
            diffuse_path, err
        );
        return None;
    }
    ...
    let albedo = resize_image_if_needed(&diffuse_rgba, width, height);

    let mut normal_image = None;
    if let Some(normal_path) = &normal_path {
        let full_path = asset_root.join(normal_path);
        if full_path.exists() {
            match load_rgba_image(&full_path) {
                Ok(img) => {
                    normal_image = Some(resize_image_if_needed(&img, width, height));
                }
                Err(err) => {
                    eprintln!(
                        "⚠️ Failed to load normal map '{}': {}",
                        full_path.display(),
                        err
                    );
                }
    ...
    if normal_image.is_none() {
        let diffuse_full_path = asset_root.join(&diffuse_path);
        if let Some(candidate) = find_normal_map(&diffuse_full_path) {
            if candidate.exists() {
                match load_rgba_image(&candidate) {
                    ...
                    Err(err) => {
                        eprintln!(
                            "⚠️ Failed to load inferred normal map '{}': {}",
                            candidate.display(),
                            err
                        );
                    }
    ...
    let normal = normal_image.unwrap_or_else(|| create_flat_normal_image(width, height));
    let mra = create_mra_from_values(width, height, entry_roughness, entry_metallic);
...
fn upload_material_layers_from_library(render: &mut RenderStuff) -> Result<()> {
    let Some(library) = render.material_library.as_ref() else {
        return Ok(());
    };

    let width = render.material_albedo.size.width;
    let height = render.material_albedo.size.height;
    let asset_root = asset_dir();

    let materials: Vec<(usize, material::Material)> = MATERIAL_LAYERS
        .iter()
        .enumerate()
        .map(|(idx, def)| (idx, library.get_material(def.key).clone()))
        .collect();

    let mut layer_images = Vec::with_capacity(materials.len());
    if let Some(manager) = render.texture_manager.as_mut() {
        for (idx, material) in &materials {
            let def = &MATERIAL_LAYERS[*idx];
            let images = load_material_images_for_layer_with_manager(
                def,
                material,
                width,
                height,
            ...
            .unwrap_or_else(|| {
                load_material_images_for_layer_without_manager(
                    def,
                    material,
                    width,
                    height,
            ...
            layer_images.push(images);
        }
    } else {
        for (idx, material) in &materials {
            let def = &MATERIAL_LAYERS[*idx];
            let images = load_material_images_for_layer_without_manager(
                def,
                material,
                width,
                height,
    ...
    for (layer, (albedo, normal, mra)) in layer_images.into_iter().enumerate() {
        render.material_albedo.write_layer(&render.device, &render.queue, layer as u32, albedo.as_raw());
        render.material_normal.write_layer(&render.device, &render.queue, layer as u32, normal.as_raw());
        render.material_mra.write_layer(&render.device, &render.queue, layer as u32, mra.as_raw());
    }
...
fn create_material_array_textures(
    ...
    if let Some(manager) = texture_manager {
        for (layer, def) in MATERIAL_LAYERS.iter().enumerate() {
            let (albedo_image, normal_image, mra_image) =
                load_material_layer_from_manager(def, width, height, manager, &asset_root)
                    .unwrap_or_else(|| load_material_layer_images_fallback(def, width, height));

            material_albedo.write_layer(device, queue, layer as u32, albedo_image.as_raw());
            material_normal.write_layer(device, queue, layer as u32, normal_image.as_raw());
            material_mra.write_layer(device, queue, layer as u32, mra_image.as_raw());
        }
    } else {
        for (layer, def) in MATERIAL_LAYERS.iter().enumerate() {
            let (albedo_image, normal_image, mra_image) =
...
impl ArrayTexture {
    fn write_layer(&self, device: &wgpu::Device, queue: &wgpu::Queue, layer: u32, data: &[u8]) {
        ...
        let bytes_per_row = 4 * self.size.width;
...
struct RenderStuff {
    ...
    material_albedo: ArrayTexture,
    material_normal: ArrayTexture,
    material_mra: ArrayTexture,
...
fn apply_material_override(
    ...
    if overrides.albedo.is_none() && overrides.normal.is_none() && overrides.mra.is_none() {
        return;
    }

    if layer_index >= MATERIAL_LAYERS.len() {
        return;
    }

    let layer_def = &MATERIAL_LAYERS[layer_index];
    let width = render.material_albedo.size.width;
    let height = render.material_albedo.size.height;
    let layer = layer_index as u32;

    if let Some(path) = overrides.albedo.as_deref() {
        if let Some(image) = load_override_material_image(path, width, height) {
            render.material_albedo.write_layer(
                &render.device,
                &render.queue,
                layer,
                image.as_raw(),
            ...
            println!(
                "✓ Applied albedo override '{}' for material '{}'.",
                path,
    ...
    if let Some(path) = overrides.normal.as_deref() {
        if let Some(image) = load_override_material_image(path, width, height) {
            render.material_normal.write_layer(
                &render.device,
                &render.queue,
                layer,
                image.as_raw(),
            ...
            println!(
                "✓ Applied normal override '{}' for material '{}'.",
                path,
                layer_def.display_name
            );
        }
    ...
    if let Some(path) = overrides.mra.as_deref() {
        if let Some(image) = load_override_material_image(path, width, height) {
            render.material_mra.write_layer(
                &render.device,
                &render.queue,
                layer,
                image.as_raw(),
            ...
            println!(
                "✓ Applied MRA override '{}' for material '{}'.",
                path,
                layer_def.display_name
            );
        }
...
fn reload_texture_pack(render: &mut RenderStuff, texture_pack_name: &str) -> Result<()> {
    ...
    let target_width = render.material_albedo.size.width;
    let target_height = render.material_albedo.size.height;
    ...
    let albedo_image = match load_rgba_image(&texture_path) {
        Ok(img) => {
            println!("✓ Albedo loaded: {}", texture_path.display());
            resize_image_if_needed(&img, target_width, target_height)
        }
        Err(err) => {
            eprintln!(
                "⚠️ Albedo '{}' missing ({}). Using fallback tint.",
                texture_path.display(),
                err
            );
            create_solid_rgba_image(
                target_width,
                target_height,
                fallback_color_for_material(tex_stem),
            )
        }
    };

    let normal_path = resolve_asset_path(&format!("{}_n.png", tex_stem));
    let normal_image = if normal_path.exists() {
        match load_rgba_image(&normal_path) {
            Ok(img) => {
                println!("✓ Normal map: {}", normal_path.display());
                resize_image_if_needed(&img, target_width, target_height)
            }
            Err(err) => {
                eprintln!(
                    "⚠️ Failed to read normal map {} ({}). Using flat normal.",
                    normal_path.display(),
                    err
                );
                create_flat_normal_image(target_width, target_height)
            }
        }
    } else {
        create_flat_normal_image(target_width, target_height)
    ...
    let mra_image = if mra_path.exists() {
        match load_rgba_image(&mra_path) {
            Ok(img) => {
                println!("✓ MRA map: {}", mra_path.display());
                resize_image_if_needed(&img, target_width, target_height)
            }
            Err(err) => {
                eprintln!(
                    "⚠️ Failed to read MRA map {} ({}). Using default MRA.",
                    mra_path.display(),
                    err
                );
                create_default_mra_image(target_width, target_height)
            }
        }
    } else {
        create_default_mra_image(target_width, target_height)
    ...
    for (material_key, overrides) in &pack.materials {
        match material_layer_index(material_key) {
            Some(layer_index) => {
                apply_material_override(render, layer_index, overrides);
            }
            None => {
                eprintln!(
                    "⚠️ Texture pack '{}' referenced unknown material layer '{}'",
                    texture_pack_name, material_key
                );
            }
        }
    }
    ...
    println!(
        "✓ Updated material set for pack '{}' (ground + overrides)",
        texture_pack_name
    );
    Ok(())
}
...
fn initialize_biome_materials(render: &mut RenderStuff, biome_name: &str) -> Result<()> {
    ...
    if render.material_library.is_none() {
        render.material_library = Some(MaterialLibrary::new());
    }

    let material_library = render
        .material_library
        .as_mut()
    ...
    let material_bytes = bytemuck::bytes_of(&material_gpu);

    let material_buffer = match render.material_buffer.take() {
        Some(buffer) => {
            render.queue.write_buffer(&buffer, 0, material_bytes);
            buffer
        }
        None => {
            let buffer = render.device.create_buffer(&wgpu::BufferDescriptor {
    ...
    upload_material_layers_from_library(render)?;
...
"#;
    ...
    // Texture bind group layout for layered PBR materials
    ...
    // Build layered ground material textures and bind group
    let (material_albedo, material_normal, material_mra) = create_material_array_textures(
        &device,
        &queue,
        texture_manager.as_mut(),
    )?;
    let ground_bind_group = device.create_bind_group(&wgpu::BindGroupDescriptor {
        label: Some("ground-material-array"),
        layout: &texture_bind_group_layout,
        entries: &[
            wgpu::BindGroupEntry {
                binding: 0,
                resource: wgpu::BindingResource::TextureView(&material_albedo.view),
            },
            wgpu::BindGroupEntry {
                binding: 1,
                resource: wgpu::BindingResource::Sampler(&material_albedo.sampler),
            },
            wgpu::BindGroupEntry {
                binding: 2,
                resource: wgpu::BindingResource::TextureView(&material_normal.view),
            },
            wgpu::BindGroupEntry {
                binding: 3,
                resource: wgpu::BindingResource::Sampler(&material_normal.sampler),
            },
            wgpu::BindGroupEntry {
                binding: 4,
                resource: wgpu::BindingResource::TextureView(&material_mra.view),
            },
            wgpu::BindGroupEntry {
                binding: 5,
                resource: wgpu::BindingResource::Sampler(&material_mra.sampler),
            },
        ],
    });
...
fn sample_material_layer(layer: i32, uv: vec2<f32>) -> SampleSet {
    ...
    let n = textureSample(material_normal, material_normal_sampler, coords).rgb;
...
fn fs_main(in: VsOut) -> @location(0) vec4<f32> {
  ...
  // Mesh-specific rendering based on mesh type
    if (in.mesh_type == 1u) { // Tree (separate bark and foliage materials)
        ...
        let diffuse = kd * base_color / 3.14159;
        let specular = (F * D * G_geom) / max(4.0 * NdotV * NdotL + 1e-5, 1e-5);
        let sky_ambient = sky_color(normalize(in.world_pos), time) * 0.22;
        ...
        let diffuse = kd * base_color / 3.14159;
        let specular = (F * D * G_geom) / max(4.0 * NdotV * NdotL + 1e-5, 1e-5);
        let sky_ambient = sky_color(normalize(in.world_pos), time) * 0.27;
