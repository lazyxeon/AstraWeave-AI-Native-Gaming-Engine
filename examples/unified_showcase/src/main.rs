...
const MATERIAL_LAYER_NAMES: [&str; 5] = ["grass", "dirt", "stone", "sand", "forest_floor"];
const MATERIAL_LAYER_PRIMARY: u32 = 0;
const MATERIAL_LAYER_DIRT: u32 = 1;
const MATERIAL_LAYER_STONE: u32 = 2;
const MATERIAL_LAYER_SAND: u32 = 3;
const MATERIAL_LAYER_FOREST: u32 = 4;
...
impl ArrayTexture {
    fn write_layer(&self, device: &wgpu::Device, queue: &wgpu::Queue, layer: u32, pixels: &[u8]) {
        let expected_len = (self.size.width as usize) * (self.size.height as usize) * 4usize;
        if pixels.len() != expected_len {
            println!(
                "Warning: pixel upload size {} does not match expected {} for layer {}",
                pixels.len(),
                expected_len,
                layer
            );
...
fn create_material_array_textures(
    ...
    let mut albedo_images = Vec::new();
    let mut normal_images = Vec::new();
    let mut mra_images = Vec::new();
    let mut max_width = 0u32;
    let mut max_height = 0u32;

    for &material in &MATERIAL_LAYER_NAMES {
        let albedo_path = resolve_asset_path(&format!("{material}.png"));
        let albedo_img = match load_rgba_image(&albedo_path) {
            Ok(img) => img,
            Err(err) => {
                eprintln!(
                    "Material albedo '{}' missing ({}). Using fallback color.",
                    albedo_path.display(),
                    err
                );
                create_solid_rgba_image(1, 1, fallback_color_for_material(material))
            }
        };

        let normal_path = resolve_asset_path(&format!("{material}_n.png"));
        let normal_img = match load_rgba_image(&normal_path) {
            Ok(img) => img,
            Err(_) => create_flat_normal_image(1, 1),
        };

        let mra_path = resolve_asset_path(&format!("{material}_mra.png"));
        let mra_img = match load_rgba_image(&mra_path) {
            Ok(img) => img,
            Err(_) => create_default_mra_image(1, 1),
        };

        max_width = max_width
            .max(albedo_img.width())
            .max(normal_img.width())
            .max(mra_img.width());
        max_height = max_height
            .max(albedo_img.height())
            .max(normal_img.height())
            .max(mra_img.height());

        albedo_images.push(albedo_img);
        normal_images.push(normal_img);
        mra_images.push(mra_img);
    }
    ...
    let albedo_images: Vec<RgbaImage> = albedo_images
        .into_iter()
        .map(|img| resize_image_if_needed(&img, target_width, target_height))
        .collect();
    let normal_images: Vec<RgbaImage> = normal_images
        .into_iter()
        .map(|img| resize_image_if_needed(&img, target_width, target_height))
        .collect();
    let mra_images: Vec<RgbaImage> = mra_images
        .into_iter()
        .map(|img| resize_image_if_needed(&img, target_width, target_height))
        .collect();

    let albedo = create_array_texture_from_images(
        device,
        queue,
        &albedo_images,
        wgpu::TextureFormat::Rgba8UnormSrgb,
        "material-albedo-array",
        &wgpu::SamplerDescriptor {
            label: Some("material-albedo-sampler"),
    ...
    let normal = create_array_texture_from_images(
        device,
        queue,
        &normal_images,
        wgpu::TextureFormat::Rgba8Unorm,
        "material-normal-array",
        &wgpu::SamplerDescriptor {
            label: Some("material-normal-sampler"),
            address_mode_u: wgpu::AddressMode::Repeat,
            address_mode_v: wgpu::AddressMode::Repeat,
            address_mode_w: wgpu::AddressMode::Repeat,
            mag_filter: wgpu::FilterMode::Linear,
    ...
    let mra = create_array_texture_from_images(
        device,
        queue,
        &mra_images,
        wgpu::TextureFormat::Rgba8Unorm,
        "material-mra-array",
        &wgpu::SamplerDescriptor {
            label: Some("material-mra-sampler"),
            address_mode_u: wgpu::AddressMode::Repeat,
            address_mode_v: wgpu::AddressMode::Repeat,
            address_mode_w: wgpu::AddressMode::Repeat,
            mag_filter: wgpu::FilterMode::Linear,
...
const CHARACTER_INDICES: &[u16] = &[
    ...
    // Left leg
    16, 17, 18, 16, 18, 19, 17, 23, 22, 17, 22, 18, 23, 20, 21, 23, 21, 22, 20, 16, 19, 20, 19, 21,
    // Right leg
    24, 25, 26, 24, 26, 27, 25, 31, 30, 25, 30, 26, 31, 28, 29, 31, 29, 30, 28, 24, 27, 28, 27, 29,
];
...
const SKYBOX_INDICES: &[u16] = &[
    ...
    1, 6, 7, 1, 2, 6, // Back face
    7, 5, 4, 7, 6, 5, // Left face
    4, 3, 0, 4, 5, 3, // Top face
    3, 6, 2, 3, 5, 6, // Bottom face
    4, 1, 7, 4, 0, 1,
];
...
fn reload_texture_pack(render: &mut RenderStuff, texture_pack_name: &str) -> Result<()> {
    ...
    println!(
        "✓ Updated ground material layer for pack '{}'",
        texture_pack_name
    );
    Ok(())
}
...
async fn run() -> Result<()> {
    ...
    println!(
        "🌿 Generated {} character instances for initial grassland environment",
        characters.len()
    );
...
fn batch_instances_by_mesh_type(instances: &[InstanceRaw]) -> Vec<InstanceBatch> {
    ...
    for &mesh_type_val in &[
        MeshType::Skybox as u32,
        MeshType::Cube as u32,
        MeshType::Tree as u32,
        MeshType::House as u32,
        MeshType::Character as u32,
    ] {
...
fn to_u32_indices(src: &[u16]) -> Vec<u32> {
    src.iter().map(|&i| i as u32).collect()
}
...
fn create_all_meshes(device: &wgpu::Device) -> std::collections::HashMap<MeshType, Mesh> {
    ...
    meshes.insert(
        MeshType::Cube,
        create_mesh(device, &terrain_vertices, &terrain_indices),
    );
    meshes.insert(
        MeshType::Tree,
        create_mesh(device, TREE_VERTICES, &to_u32_indices(TREE_INDICES)),
    );
    meshes.insert(
        MeshType::House,
        create_mesh(device, HOUSE_VERTICES, &to_u32_indices(HOUSE_INDICES)),
    );
    meshes.insert(
        MeshType::Character,
        create_mesh(
            device,
            CHARACTER_VERTICES,
            &to_u32_indices(CHARACTER_INDICES),
        ),
    );
    meshes.insert(
        MeshType::Skybox,
        create_mesh(device, SKYBOX_VERTICES, &to_u32_indices(SKYBOX_INDICES)),
    );
...
"#;
    ...
    let (material_albedo, material_normal, material_mra) =
        create_material_array_textures(&device, &queue)?;
...
fn sample_material_layer(layer: i32, uv: vec2<f32>) -> SampleSet {
    let idx = clamp(layer, 0, MATERIAL_LAYER_COUNT - 1);
...
fn sample_set(biome: i32, uv: vec2<f32>) -> SampleSet {
    ...
    return sample_material_layer(MATERIAL_FOREST, uv);
}
