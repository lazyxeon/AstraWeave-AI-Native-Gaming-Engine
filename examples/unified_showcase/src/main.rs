...
mod material;
mod material_hot_reload; // GPU hot-reload system (Phase PBR-G)
...
use self::material_hot_reload::{FileWatcher, MaterialReloadManager, ReloadEvent};
...
struct RenderStuff {
    ...
    reload_ui_message: String,
    reload_ui_message_time: Instant,
}
...
async fn run() -> Result<()> {
    ...
    let _ = elwt.run(move |event, elwt_window_target| {
        ...
        match event {
                        ...
                        match physical_key {
                            PhysicalKey::Code(code) => {
                                ...
                                match code {
                                    ...
                                    KeyCode::F6 => {
                                        if pressed && ui.pbr_e_demo_enabled {
                                            // Cycle through material types
                                            ui.pbr_e_material_type = match ui.pbr_e_material_type {
                                                pbr_e_demo::DemoMaterialType::Clearcoat => pbr_e_demo::DemoMaterialType::Anisotropy,
                                                pbr_e_demo::DemoMaterialType::Anisotropy => pbr_e_demo::DemoMaterialType::Subsurface,
                                                pbr_e_demo::DemoMaterialType::Subsurface => pbr_e_demo::DemoMaterialType::Sheen,
                                                pbr_e_demo::DemoMaterialType::Sheen => pbr_e_demo::DemoMaterialType::Transmission,
            ...
            Event::AboutToWait => {
                // Phase PBR-G: Process GPU hot-reload events
                        ...
                        for event in events {
                            let result = match event {
                                ReloadEvent::Material { ref path, material_type } => {
                                    // Check if this material belongs to current biome
                                    if render.reload_manager.is_current_biome_path(path) {
                                        // Get material buffer (default for now, extend for multiple buffers)
                                        if let Some(ref material_buffer) = render.material_buffer {
                                            ...
                                            Err(anyhow::anyhow!("Material buffer not initialized"))
                                        }
                                    } else {
                                        continue; // Skip materials from other biomes
                                    }
                                }
                                ReloadEvent::Texture { ref path, texture_type: _, color_space: _ } => {
                                    // Check if this texture belongs to current biome
                                    if render.reload_manager.is_current_biome_path(path) {
                                        // Find material that uses this texture
                                        if let Some(material_id) = render.reload_manager.find_material_by_path(path) {
                                            // Get texture type and array indices
                                            if let Some(texture_type) = render.reload_manager.get_texture_type_for_path(material_id, path) {
                                                if let Some(array_indices) = render.reload_manager.get_array_indices(material_id) {
                                                    // Get current biome's texture arrays from MaterialIntegrator
                                                    if let Some(_runtime) = render.material_integrator.current() {
                                                        ...
                                                        if let Some(texture) = texture_array {
                                                            ...
                                                            match image::open(path) {
                                                                ...
                                                                Err(e) => {
                                                                    Err(anyhow::anyhow!("Failed to load texture {}: {}", path.display(), e))
                                                                }
                                                            }
                                                        } else {
                                                            Err(anyhow::anyhow!("Texture array not available"))
                                                        }
                                                    } else {
                                                        Err(anyhow::anyhow!("No biome loaded in MaterialIntegrator"))
                                                    }
                                                } else {
                                                    Err(anyhow::anyhow!("Material array indices not registered"))
                                                }
                                            } else {
                                                Err(anyhow::anyhow!("Could not determine texture type from path"))
                                            }
                                        } else {
                                            Err(anyhow::anyhow!("Texture not associated with any material"))
                                        }
                                    } else {
                                        continue; // Skip textures from other biomes
                                    }
                                }
                                ReloadEvent::ArrayManifest { ref path } => {
                                    // Array manifest changed - requires full reload
                                    println!("ℹ️  Array manifest changed: {}", path.display());
                                    println!("   Full biome reload required (not implemented in hot-reload)");
                                    Ok(())
                                }
                                _ => {
                                    // Extended/terrain materials
                            ...
                            match result {
                                Ok(_) => {
                                    // Success message already printed by reload_manager
                                    render.reload_ui_message = format!("✅ Hot-reload successful");
                                    render.reload_ui_message_time = Instant::now();
                                }
                                Err(e) => {
                                    eprintln!("❌ Hot-reload error: {}", e);
                                    render.reload_ui_message = format!("❌ Hot-reload failed: {}", e);
                                    render.reload_ui_message_time = Instant::now();
                                }
                            }
                        }
    ...
    Ok(RenderStuff {
        ...
        file_watcher: match FileWatcher::new("assets/materials") {
            ...
            Err(e) => {
                ...
                eprintln!("   Hot-reload disabled. Materials will load statically.");
                None
            }
        },
