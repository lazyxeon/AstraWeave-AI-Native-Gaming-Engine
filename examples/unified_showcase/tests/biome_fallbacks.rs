...
fn test_dedicated_test_biome_exact_fallbacks() {
    ...
    let (total_layers, alb_ok, nor_ok, mra_ok) = result;
    assert_eq!(alb_ok, 0, "No albedos should match standard layers in the test biome");
    assert_eq!(nor_ok, 0, "No normals should match standard layers in the test biome");
    assert_eq!(mra_ok, 0, "No MRAs should match standard layers in the test biome");
    assert!(total_layers > 0, "We still report the total number of material layers");
}
