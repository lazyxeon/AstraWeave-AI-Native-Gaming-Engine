...
fn main() -> Result<()> {
    ...
    create_cube(&mut vertices, &mut indices, Vec3::new(-30.0, 1.5, 20.0), 3.0);
    create_cube(&mut vertices, &mut indices, Vec3::new(30.0, 1.5, -20.0), 3.0);
    create_cube(&mut vertices, &mut indices, Vec3::new(-15.0, 1.5, -30.0), 3.0);
    
    // 2 fallen logs (10m x 1m x 1m boxes)
    create_box(&mut vertices, &mut indices, Vec3::new(0.0, 0.5, 30.0), 10.0, 1.0, 1.0);
    create_box(&mut vertices, &mut indices, Vec3::new(20.0, 0.5, 0.0), 10.0, 1.0, 1.0);
    ...
    fs::write("../../assets/models/greybox/echo_grove_greybox.gltf", serde_json::to_string_pretty(&gltf)?)?;
...
fn create_box(vertices: &mut Vec<Vertex>, indices: &mut Vec<u16>, c: Vec3, width: f32, height: f32, depth: f32) {
    let hw = width / 2.0; let hh = height / 2.0; let hd = depth / 2.0;
    let faces = [
    ...
    for (n, ps) in &faces {
        let b = vertices.len() as u16;
        for p in ps { let w = Vec3::new(p[0], p[1], p[2]) + c; vertices.push(Vertex { position: [w.x, w.y, w.z], normal: *n, texcoord: [0.0, 0.0] }); }
        indices.extend_from_slice(&[b, b+1, b+2, b, b+2, b+3]);
    }
}
