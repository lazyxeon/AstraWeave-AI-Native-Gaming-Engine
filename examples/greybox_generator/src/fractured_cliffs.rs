...
fn main() -> Result<()> {
    ...
    create_platform(&mut vertices, &mut indices, 15.0, 1.0, Vec3::new(0.0, 10.0, 200.0));
    ...
    for i in 0..5 {
        ...
        create_step(&mut vertices, &mut indices, 8.0, 10.0, 0.3, Vec3::new(0.0, y, z));
    }
    ...
    create_cube(&mut vertices, &mut indices, Vec3::new(0.0, 10.5, 200.0), 1.0);
    ...
    fs::write("../../assets/models/greybox/fractured_cliffs_greybox.gltf", serde_json::to_string_pretty(&gltf)?)?;
    println!("Success! {} vertices, {} triangles", vertices.len(), indices.len() / 3);
    Ok(())
}

fn create_path(vertices: &mut Vec<Vertex>, indices: &mut Vec<u16>, length: f32, width: f32, height: f32) {
    let hw = width / 2.0;
    ...
    for (x, z) in [(-hw, 0.0), (hw, 0.0), (hw, length), (-hw, length)] {
        vertices.push(Vertex { position: [x, height, z], normal: [0.0, 1.0, 0.0], texcoord: [(x+hw)/width, z/length] });
    }
    indices.extend_from_slice(&[b, b+1, b+2, b, b+2, b+3]);
}

fn create_cliff_wall(vertices: &mut Vec<Vertex>, indices: &mut Vec<u16>, x_offset: f32, length: f32, height: f32, thickness: f32) {
    let b = vertices.len() as u16;
    let ht = thickness / 2.0;
    
    // Single tall wall face (front facing inward toward path)
    let normal = if x_offset < 0.0 { [1.0, 0.0, 0.0] } else { [-1.0, 0.0, 0.0] };
    for (y, z) in [(0.0, 0.0), (0.0, length), (height, length), (height, 0.0)] {
        vertices.push(Vertex { position: [x_offset, y, z], normal, texcoord: [z/length, y/height] });
    }
    indices.extend_from_slice(&[b, b+1, b+2, b, b+2, b+3]);
}

fn create_platform(vertices: &mut Vec<Vertex>, indices: &mut Vec<u16>, size: f32, height: f32, center: Vec3) {
    let h = size / 2.0; let b = vertices.len() as u16;
    for (x, z) in [(-h, -h), (h, -h), (h, h), (-h, h)] {
        let world_pos = Vec3::new(x, height, z) + center;
        vertices.push(Vertex { position: [world_pos.x, world_pos.y, world_pos.z], normal: [0.0, 1.0, 0.0], texcoord: [(x+h)/size, (z+h)/size] });
    }
    indices.extend_from_slice(&[b, b+1, b+2, b, b+2, b+3]);
}

fn create_step(vertices: &mut Vec<Vertex>, indices: &mut Vec<u16>, width: f32, depth: f32, height: f32, center: Vec3) {
    let hw = width / 2.0; let hd = depth / 2.0; let b = vertices.len() as u16;
    
    // Top surface only (simplified step)
    for (x, z) in [(-hw, -hd), (hw, -hd), (hw, hd), (-hw, hd)] {
        let world_pos = Vec3::new(x, height, z) + center;
        vertices.push(Vertex { position: [world_pos.x, world_pos.y, world_pos.z], normal: [0.0, 1.0, 0.0], texcoord: [(x+hw)/width, (z+hd)/depth] });
    }
    indices.extend_from_slice(&[b, b+1, b+2, b, b+2, b+3]);
}
