...
use astraweave_core::{CPos, CHealth, CTeam, CAmmo, CCooldowns, CDesiredPos, IVec2};
...
fn print_entities(world: &ecs::World) {
    ...
    for (entity, pos) in ecs::query!(world, CPos) {
        ...
        println!("  Entity {:?}: Team {}, Pos {:?}, HP {}", entity.id(), team, pos.pos, hp);
    }
}
