...
async fn generate_demo_world(partition: &Arc<RwLock<WorldPartition>>, grid_size: i32) {
    ...
    println!(
        "Generating demo world with {}x{} cells...",
        grid_size, grid_size
    );
    ...
    for x in 0..grid_size {
        for z in 0..grid_size {
            ...
            cell.assets
                .push(astraweave_scene::world_partition::AssetRef {
    ...
    println!(
        "Generated {} cells with entities and assets",
        grid_size * grid_size
    );
}
...
async fn main() -> anyhow::Result<()> {
    ...
    println!(
        "Generated camera path with {} waypoints\n",
        camera_path.len()
    );
    ...
    println!(
        ...
        final_metrics.memory_usage_bytes as f64 / 1_048_576.0
    );
    ...
    println!(
        "✓ No stalls >100ms: {} (max: {:.2}ms)",
        if no_stalls { "PASS" } else { "FAIL" },
        max_frame_ms
    );

    let seamless = final_metrics.failed_loads == 0;
    println!(
        "✓ Seamless exploration (no failed loads): {}",
