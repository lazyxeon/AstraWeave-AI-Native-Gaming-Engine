use anyhow::Result;
use glam::Vec3;
use astraweave_nav::{Triangle, NavMesh};
use astraweave_physics::PhysicsWorld;

fn main() -> Result<()> {
    // Bake a simple flat navmesh (two triangles making a square)
    let tris = vec![
        Triangle { a: Vec3::new(0.0,0.0,0.0), b: Vec3::new(0.0,0.0,5.0), c: Vec3::new(5.0,0.0,0.0) },
        Triangle { a: Vec3::new(5.0,0.0,0.0), b: Vec3::new(0.0,0.0,5.0), c: Vec3::new(5.0,0.0,5.0) },
    ];
    ...
    let speed = 2.0; // m/s
    for _ in 0..(10*60) { // 10 seconds @60Hz
        let pos = pw.body_transform(char_id).unwrap();
        let p = Vec3::new(pos.w_axis.x, 0.0, pos.w_axis.z);
        let target = if cur_wp < path.len() { path[cur_wp] } else { goal };
        let to = (target - p).clamp_length_max(1.0);
        let dir = if to.length() > 1e-3 { to.normalize() } else { Vec3::ZERO };
        pw.control_character(char_id, Vec3::new(dir.x * speed, 0.0, dir.z * speed), 1.0/60.0, false);
        pw.step();
        if p.distance(target) < 0.15 && cur_wp < path.len() { cur_wp += 1; }
    }
    ...
    assert!(final2d.distance(goal) < 0.5, "character should reach near goal");
    Ok(())
}
