...
fn mk_snap_from_world(w: &World, me: Entity, player: Entity, enemy: Entity) -> WorldSnapshot {
    ...
    astraweave_core::WorldSnapshot{
        t: w.t,
        player: astraweave_core::PlayerState{ hp: w.health(player).unwrap().hp, pos: player_pose, stance: "stand".into(), orders: vec![] },
        me: astraweave_core::CompanionState{ ammo: w.ammo(me).unwrap().rounds, cooldowns: BTreeMap::new(), morale: 0.5, pos: me_pose },
        enemies: vec![astraweave_core::EnemyState{ id: enemy, pos: enemy_pose, hp: w.health(enemy).unwrap().hp, cover: "low".into(), last_seen: 0.0 }],
        pois: vec![],
        objective: None,
    }
}
...
async fn main() {
    ...
    let mut w = World::new();
    let player = w.spawn("player", IVec2{ x:0, y:0 }, astraweave_core::Team{ id:0 }, 100, 0);
    let me = w.spawn("companion", IVec2{ x:0, y:1 }, astraweave_core::Team{ id:1 }, 100, 5);
    let enemy = w.spawn("enemy", IVec2{ x:6, y:1 }, astraweave_core::Team{ id:2 }, 50, 0);

    let cfg = ValidateCfg{ world_bounds: (-20,-20,20,20) };
...
fn run_plan_with_logging(w: &mut World, actor: Entity, intent: &PlanIntent, cfg: &ValidateCfg) -> Result<(), astraweave_core::EngineError> {
