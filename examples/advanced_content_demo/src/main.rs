    ...
    while time < 10.0 {
        ...
        if let ObjectiveType::Escort { npc } = &mut quest.objectives[0] {
            ...
            if time as u32 % 2 == 0 && time.fract() < delta_time {
                println!("📊 [t={:.1}s] Progress: {:.1}%", time, quest.objectives[0].progress() * 100.0);
            }
...
fn demo_scenario_2_defend_quest() {
    ...
    let defend_obj = quest_types::DefendObjective::new(
        "Ancient Anchor",
        Vec3::ZERO,
        10.0,
        200.0,
    ...
    while time < 12.0 {
        ...
        if wave_timer >= 3.0 {
            ...
            if let ObjectiveType::Defend { objective, .. } = &mut quest.objectives[0] {
                ...
                println!("🌊 [t={:.1}s] Wave {} spawned!", time, objective.waves_survived);
...
fn demo_scenario_3_boss_fight() {
    ...
    .with_objective(ObjectiveType::Boss { objective: boss_obj })
    ...
    while time < 15.0 {
        ...
        if let ObjectiveType::Boss { objective } = &mut quest.objectives[0] {
            if time as u32 % 2 == 0 && time.fract() < delta_time {
                println!("👹 [t={:.1}s] Boss attacks (Phase {:?})!", time, objective.current_phase);
...
fn demo_scenario_4_time_trial() {
    ...
    .with_objective(ObjectiveType::TimeTrial { objective: time_trial_obj })
    ...
    while time < 16.0 {
        ...
        if let ObjectiveType::TimeTrial { objective } = &mut quest.objectives[0] {
            ...
            if time as u32 % 3 == 0 && time.fract() < delta_time {
                println!("📊 [t={:.1}s] Time: {:.1}s", time, objective.remaining_time());
            }
...
fn demo_scenario_5_collect_quest() {
    ...
    let collect_obj = quest_types::CollectObjective::new(
        "Echo Fragment",
        positions,
        5.0,
    );
    ...
    .with_objective(ObjectiveType::Collect { objective: collect_obj })
    .with_reward(QuestReward::EchoCurrency(100));
        ...
        if let ObjectiveType::Collect { objective } = &mut quest.objectives[0] {
            if time as u32 % 2 == 0 && collected < 5 {
                if collected < objective.items.len() {
                    ...
                    collected += 1;
                    println!("✨ [t={:.1}s] Collected: {}", time, objective.items[collected-1].item_name);
