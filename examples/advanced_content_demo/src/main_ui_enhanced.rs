...
fn main() {
    ...
    println!("Features: Ability cooldown UI, Quest progress HUD, Particle effects, Audio simulation\n");
...
fn demo_scenario_1_escort_quest_with_ui() {
    ...
    if let Ok(()) = player.use_shield() {
        ...
        println!("{}", render_particle_effect("shield_bubble", player.position));
        println!("✅ Shield activated! Echo: {} → {}", 100, player.echo_currency);
    }
    ...
    if let ObjectiveType::Escort { ref mut npc } = quest.objectives[0] {
        ...
        println!("🚶 Merchant reached waypoint! Health: {} / {}", npc.health, 100.0);
    }
    ...
    println!("{}", render_notification("Quest Complete!", "You earned 50 Echo ⚡", "✅"));
...
fn demo_scenario_2_defend_quest_with_ui() {
    ...
    .with_objective(ObjectiveType::Defend { anchor: defend_anchor })
    ...
    quest.state = QuestState::Complete;
    println!("{}", play_audio_effect("quest_complete"));
    println!("{}", render_notification("Defense Complete!", "Village survived! +100 Echo ⚡", "🏆"));
...
fn demo_scenario_3_boss_fight_with_ui() {
    ...
    println!("\n👹 Void Boss appears!");
    println!("{}", render_particle_effect("spawn_portal", Vec3::new(50.0, 0.0, 0.0)));
    println!("{}", play_audio_effect("spawn_portal"));
    
    // Boss health bar (separate UI element)
    println!("\n{}════════════════════════════════════════════════════════════{}",
             colors::RED, colors::RESET);
    println!("{}👹 Void Boss {} | HP: {}████████████████████{} 1000 / 1000{}",
             colors::BOLD, colors::RESET, colors::RED, colors::RESET, colors::RESET);
    println!("{}════════════════════════════════════════════════════════════{}",
             colors::RED, colors::RESET);
    ...
    if let ObjectiveType::Boss { ref mut boss } = quest.objectives[0] {
        ...
        println!("{}👹 Void Boss HP: {}{}████████████░░░░{} {} / 1000{}",
                 colors::BOLD, colors::RED, colors::BG_RED, colors::RESET, 
                 boss.health as i32, colors::RESET);
    }
    ...
    if let ObjectiveType::Boss { ref mut boss } = quest.objectives[0] {
        ...
        println!("{}👹 Void Boss (ENRAGED) HP: {}{}██████░░░░░░░░░░{} {} / 1000{}",
                 colors::BOLD, colors::RED, colors::BG_RED, colors::RESET,
                 boss.health as i32, colors::RESET);
    }
    ...
    if let ObjectiveType::Boss { ref mut boss } = quest.objectives[0] {
        ...
        println!("{}👹 Void Boss HP: {}{}░░░░░░░░░░░░░░░░{} 0 / 1000 (DEFEATED){}",
                 colors::BOLD, colors::DIM, colors::RESET, colors::RESET, colors::RESET);
    }
    
    quest.state = QuestState::Complete;
    println!("{}", play_audio_effect("quest_complete"));
    println!("{}", render_notification("Boss Defeated!", "Reality restored! +250 Echo ⚡", "👑"));
