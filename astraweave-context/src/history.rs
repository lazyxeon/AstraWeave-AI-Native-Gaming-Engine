...
use crate::{
    ContextConfig, ContextMetrics, ContextOperation, Message, OverflowStrategy, Role,
    TokenCounter, current_timestamp,
};
    ...
    async fn prune_if_needed(&self) -> Result<()> {
        ...
        match self.config.overflow_strategy {
            ...
            OverflowStrategy::Hybrid => {
                self.prune_hybrid().await
            }
            OverflowStrategy::TruncateStart => {
                self.prune_truncate_start().await
            }
            OverflowStrategy::TruncateMiddle => {
                self.prune_truncate_middle().await
            }
        }
    }
...
mod tests {
    ...
    async fn test_sliding_window_pruning() {
        ...
        history.add_message(Role::User, "Message 1".to_string()).await.unwrap();
        history.add_message(Role::User, "Message 2".to_string()).await.unwrap();
        history.add_message(Role::User, "Message 3".to_string()).await.unwrap();
    ...
    async fn test_preserved_messages() {
        ...
        let preserved_msg = Message::new_preserved(Role::System, "Important system message".to_string());
        {
        ...
        history.add_message(Role::User, "Regular message".to_string()).await.unwrap();
    ...
    async fn test_export_import() {
        ...
        history1.add_message(Role::User, "Hello".to_string()).await.unwrap();
        history1.add_message(Role::Assistant, "Hi".to_string()).await.unwrap();
