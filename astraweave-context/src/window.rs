...
use crate::{ContextConfig, Message, Role, TokenCounter, current_timestamp};
use anyhow::Result;
...
impl ContextWindow {
    ...
    pub fn add_message(&mut self, mut message: Message) -> Result<()> {
        ...
        self.attention_weights.insert(message.id.clone(), attention_weight);
    ...
    fn calculate_attention_weight(&self, message: &Message) -> f32 {
        ...
        if let Some(&role_weight) = self.config.attention_config.role_weights.get(message.role.as_str()) {
            weight *= role_weight;
        }
        ...
        if !self.config.attention_config.content_keywords.is_empty() {
            ...
            let keyword_matches = self.config.attention_config.content_keywords
                .iter()
    ...
    fn update_stats(&mut self) {
        ...
        if self.stats.total_messages_added > 0 {
            self.stats.avg_message_tokens = 
                (self.stats.total_tokens_processed as f32) / (self.stats.total_messages_added as f32);
        }
    }
...
impl MultiAgentContextManager {
    ...
    fn matches_rule(&self, source_agent: &str, message: &Message, rule: &RoutingRule) -> bool {
        ...
        if let Some(ref filter) = rule.content_filter {
            if !message.content.to_lowercase().contains(&filter.to_lowercase()) {
                return false;
            }
        }
...
mod tests {
    ...
    fn test_sliding_window_pruning() {
        ...
        window.add_message(Message::new(Role::User, "Message 2".to_string())).unwrap();
        window.add_message(Message::new(Role::User, "Message 3".to_string())).unwrap();
    ...
    fn test_attention_based_pruning() {
        ...
        window.add_message(Message::new(Role::User, "Regular message 1".to_string())).unwrap();
        window.add_message(important_msg).unwrap();
        window.add_message(Message::new(Role::User, "Regular message 2".to_string())).unwrap();
        
        assert_eq!(window.message_count(), 2);
        
        // Should keep important message
        let messages = window.get_messages();
        assert!(messages.iter().any(|m| m.content.contains("Important system message")));
    }
    ...
    fn test_multi_agent_context_manager() {
        ...
        // Add messages
        manager.add_message_to_agent("agent1", Message::new(Role::User, "Hello from agent1".to_string())).unwrap();
    ...
    fn test_context_formatting() {
        ...
        window.add_message(Message::new(Role::Assistant, "Hi there".to_string())).unwrap();
