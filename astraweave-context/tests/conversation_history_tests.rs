...
use anyhow::Result;
use astraweave_context::{ContextConfig, ConversationHistory, OverflowStrategy, Role};
    ...
    for i in 0..10 {
        history
            .add_message(
                Role::User,
                format!(
                    "This is a longer message to ensure we exceed the token limit {}",
                    i
                ),
            )
            .await
...
async fn test_concurrent_access() {
    ...
    for i in 0..10 {
        ...
        handles.push(tokio::spawn(async move {
            h.add_message(Role::User, format!("Message {}", i))
                .await
                .unwrap();
        }));
    }
...
async fn test_message_metadata() {
    ...
    let msg_id = history
        .add_message_with_metadata(Role::User, "Metadata message".to_string(), metadata)
        .await
        .unwrap();
...
async fn test_clear_history() {
    ...
    history
        .add_message(Role::User, "Msg 1".to_string())
        .await
        .unwrap();
...
async fn test_get_by_role() {
    ...
    history
        .add_message(Role::User, "User 1".to_string())
        .await
        .unwrap();
    history
        .add_message(Role::Assistant, "Assist 1".to_string())
        .await
        .unwrap();
    history
        .add_message(Role::User, "User 2".to_string())
        .await
        .unwrap();
...
async fn test_get_by_time_range() {
    ...
    history
        .add_message(Role::User, "Msg 2".to_string())
        .await
        .unwrap();
