...
use astraweave_context::{
    ContextWindow, ContextWindowConfig, Message, Role, SerializableContextWindow, WindowType,
};
...
fn test_context_window_serialization() {
    ...
    window
        ...
        .unwrap();
    window
        .add_message(Message::new(Role::Assistant, "Response 1".to_string()))
        .unwrap();
...
fn test_context_compression_via_pruning() {
    ...
    for i in 0..10 {
        window
            .add_message(Message::new(Role::User, format!("{} {}", i, long_msg)))
            .unwrap();
    }
