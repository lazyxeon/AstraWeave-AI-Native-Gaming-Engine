...
use astraweave_context::{
    ContextWindow, ContextWindowConfig, Message, MultiAgentContextManager, Role, RoutingConfig,
    RoutingRule, WindowType,
};
...
fn test_attention_weights() {
    ...
    config
        .attention_config
        .role_weights
        .insert("system".to_string(), 2.0);
    config
        .attention_config
        .role_weights
        .insert("user".to_string(), 1.0);
...
fn test_role_based_attention() {
    ...
    window
        ...
        .unwrap();
    window
        .add_message(Message::new(Role::System, "System 1".to_string()))
        .unwrap();
    window
...
fn test_content_based_attention() {
    ...
    window
        .add_message(Message::new(Role::User, "Regular msg".to_string()))
        .unwrap();
    window
        .add_message(Message::new(Role::User, "This is important".to_string()))
        .unwrap();
    window
        .add_message(Message::new(Role::User, "Another regular".to_string()))
        .unwrap();
...
fn test_multi_agent_sharing() {
    ...
    manager
        .add_message_to_agent(
            "agent1",
            Message::new(Role::User, "Hello agent1".to_string()),
        )
        .unwrap();
...
fn test_context_isolation() {
    ...
    manager
        .add_message_to_agent("agent1", Message::new(Role::User, "Secret".to_string()))
        .unwrap();
...
fn test_shared_window_access() {
    ...
    manager
        .add_message_to_shared(Message::new(Role::System, "Global context".to_string()))
        .unwrap();
...
fn test_routing_rules() {
    ...
    manager
        .add_message_to_agent(
            "agent1",
            Message::new(Role::User, "Normal message".to_string()),
        )
        .unwrap();
    assert_eq!(
        manager.get_agent_window("agent2").unwrap().message_count(),
        0
    );
    ...
    manager
        .add_message_to_agent(
            "agent1",
            Message::new(Role::User, "This is an alert".to_string()),
        )
        .unwrap();
    assert_eq!(
        manager.get_agent_window("agent2").unwrap().message_count(),
