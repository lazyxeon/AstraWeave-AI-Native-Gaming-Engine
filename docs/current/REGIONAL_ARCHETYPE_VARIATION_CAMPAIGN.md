...
**Current resumption pointer (Multi-Tool 5.C, 2026-07-22; supersedes the pre-SP5 pointer preserved in the chronology above)**: G-pointer-events-fix is **SUBSUMED** by Editor Multi-Tool SP5 (`2f6f853a5` + `9bae816e3`), its §7.4 Andrew-gate PASS record (`docs/audits/T_SERIES_RATIFICATION_2026-07-20.md` §1), and the real-panel pointer-consumption regression guard (`2e92b8a3b`). This campaign remains paused through the Multi-Tool Mediator Removal + Sub-phase 6 closeout; it then resumes directly at **H-saveload-diagnostic**. No separate G-fix session remains.
...
### 2026-05-03, Sub-phase Editor-Multi-Tool-Architecture-Research (cross-reference; campaign launch sequence Session 2), commits 8ba6cd13e (Research.A audit) + 29b8c53b3 (Research.B campaign doc cross-reference)

**Sub-phase entry — captures Editor Multi-Tool Architecture research pass execution. Cross-reference entry; the new campaign's own doc launches in Session 3. This entry preserves the linkage from Regional Archetype Variation to the spinoff campaign's outputs so future-Andrew (or future-Claude) can navigate from the paused campaign to the spinoff's audit + future campaign-design doc.**

**Pre-execution verification** (per Editor Multi-Tool Architecture research prompt §1):

- §1.1 Pause artifacts state: confirmed at `a64f12320` (pause.A) + `98fc063d9` (pause.B) + `13ef70132` (pause.C).
- §1.2 G-research inheritance scoping: completed; inherited canonical findings catalogued in audit §2 (egui dispatch mechanics, legacy AAA approaches, Rust 3D editor base patterns).
- §1.3 Pause artifacts §10 entry re-read: methodological lesson internalized (research-pass-before-reframe discipline pattern §0 inheritance for Editor Multi-Tool Architecture campaign-design pass).
- §1.4 Anti-anchoring discipline: held throughout session — no AstraWeave code inspection.

**Deliverables**:

- **Research.A** (commit `8ba6cd13e`): audit document at `docs/audits/editor_multi_tool_architecture_research_2026-05-03.md` (~770 lines, 11 sections + bibliography). Five concerns investigated; Concern E synthesis produced approach taxonomy + tradeoff matrix.

- **Research.B** (this commit): cross-reference §9 status update (Status header line 3 includes Editor Multi-Tool Architecture research pass COMPLETE entry with audit reference + 5-approach taxonomy summary + Session 3 pending pointer) + this §10 entry.

**Approach taxonomy result** (per audit §7.1):

5 distinct dispatcher-architecture approaches identified from cross-concern survey:

1. **Approach I — Explicit registry with mutex enforcement at registry level**: Builder/factory pattern + central manager + framework-enforced single-active mutex. Examples: Unreal `UInteractiveToolsContext` + `UInteractiveToolManager`, Unity `ToolManager`, Krita `KoToolManager`.
2. **Approach II — Trait-object collection with iteration-based dispatch**: trait + `Box<dyn Trait>` collection + dispatcher tracks active by UUID/index + per-event method calls. Example: Fyrox `InteractionMode` (production-grade Rust + egui canonical).
3. **Approach III — Plugin-style discovery with attribute/manifest-based registration**: attribute scanning + plugin manifest + tool-self-arbitration (sometimes). Examples: Unity `EditorToolAttribute`, GIMP plugin SDK, Photoshop CTool.
4. **Approach IV — Workspace/context-based registration**: tool state per workspace; tool-as-key-binding-mechanism + operator launching. Example: Blender WorkSpaceTool (instance-per-event semantics; operator carries state).
5. **Approach V — ECS-style component registration**: tools as components; dispatcher as system; activation via marker component. Bevy ecosystem prototype phase; not yet canonical.

**Approach II hybrid** (Approach I + II combination, Fyrox-style) scores highest on Rust+egui ecosystem-fit per audit §7.6 tradeoff matrix. Per audit §7.7 synthesis: "the canonical Rust + egui pattern is **Approach I + II hybrid** — registry/manager owns trait-object collection; dispatcher uses per-event method calls on the active trait-implementation; UUID identity provides open-set extensibility."

**Forward chain**:

1. **Session 3 — Editor Multi-Tool Architecture campaign-design pass**: drafts `docs/current/EDITOR_MULTI_TOOL_ARCHITECTURE_CAMPAIGN.md` with §0-§10 framing matching Regional Archetype Variation's structural template. §2 architectural decisions resolved using audit §7 framework (ActiveTool trait shape, EventDisposition enum semantics, dispatcher mechanism, registration model, mediator fate, integration with existing ViewportWidget, mutex semantics, tool composition rules, state persistence, action transactionality — per audit §8.1 enumeration). Sub-phase breakdown sized to scope per audit §8.2 estimate (likely 5 sub-phases: trait + dispatcher core → TerrainPanel migration with Andrew-gate → integration tests + Pattern A regression → RegionalArchetypePanel registration → closeout). Andrew-gate gating per §0 discipline applied to visible-output sub-phases per audit §8.3. Campaign-design pass §0 should bake "research-pass-before-reframe" discipline pattern per pause.B §10 + audit §9.4 methodological lesson.

2. **Sessions 4+ — Editor Multi-Tool Architecture campaign execution**: per the campaign-design plan from Session 3.

3. **Resume Regional Archetype Variation**: post-Editor-Multi-Tool-Architecture closure, G-pointer-events-fix executes as `ActiveTool` implementation + dispatcher registration (single small commit; likely subsumed by Sub-phase 4 of the new campaign per audit §8.4). H-saveload-diagnostic + H-saveload-fix follow. Final Andrew-gate full PASS → F.5-paint COMPLETE → F.5-overlay-and-gate → F.6 → F.7 (principal Andrew-gate) → F.8 closeout.

**Scope held**: Editor Multi-Tool Architecture research pass session only modified `docs/audits/editor_multi_tool_architecture_research_2026-05-03.md` (commit `8ba6cd13e`) and this Regional Archetype Variation campaign doc §9 + §10 (this commit). NO AstraWeave code inspection (anti-anchoring discipline). NO Session 3 campaign-design content. NO recommendation of which approach AstraWeave should use (Session 3 + Andrew's call). NO single-approach narrowing in audit (5-approach taxonomy + tradeoff matrix is full menu of options for Session 3).

**Methodological lessons preserved** (per audit §9):

- **Research-then-diagnostic pattern as canonical workflow for spinoff sequences**: original campaign's sub-phase research → diagnostic → architectural decision → spinoff → research-pass-of-new-campaign → campaign-design-pass. Future spinoffs inherit this structure.
- **Inheritance-from-predecessor-research pattern**: G-research findings classified inherited canonical / expansion target / out of scope; inherited canonical cited by reference (audit §2) without re-derivation. Time saved on inheritance reinvested in expansion targets (broader source coverage with similar wall-clock budget).
- **Anti-anchoring discipline preservation**: research session's findings form independently of target codebase's state to prevent confirmation bias when campaign-design pass evaluates against canonical patterns. Future research-pass sessions inherit this framing.

### 2026-05-04, Sub-phase Editor-Multi-Tool-Architecture-Design (cross-reference; campaign launch sequence Session 3), commits 75b68e7c7 (Design.A campaign doc) + 8fad61bd3 (Design.B Regional Archetype Variation cross-reference)

**Sub-phase entry — captures Editor Multi-Tool Architecture campaign-design pass execution. Cross-reference entry; the new campaign's own doc (`docs/current/EDITOR_MULTI_TOOL_ARCHITECTURE_CAMPAIGN.md`, commit `75b68e7c7`) is the load-bearing artifact.**

**Pre-execution verification** (per Editor Multi-Tool Architecture campaign-design prompt §1):

- §1.1 Research audit re-read for §8.1 decision enumeration: complete; 10-decision map internalized + grounded in research audit §3-§7 framework.
- §1.2 Pause artifacts state: confirmed at `a64f12320` + `98fc063d9` + `13ef70132`; research pass at `8ba6cd13e` + `29b8c53b3` + `c3bc7ca0c`.
- §1.3 Regional Archetype Variation campaign doc state: confirmed at post-Research.B state with Editor Multi-Tool Architecture research pass COMPLETE entry in Status header + §9 + §10.
- §1.4 Anti-anchoring discipline for diagnostic deferral: held — single AstraWeave reference points used (G-diagnostic findings on ViewportWidget:163, main.rs:3833-3877, terrain_panel.rs:797, regional_archetype_panel.rs:75, viewport/widget.rs:1219-1234) without exploratory inspection beyond G-diagnostic's existing findings.

**Deliverables**:

- **Design.A** (commit `75b68e7c7`): campaign doc at `docs/current/EDITOR_MULTI_TOOL_ARCHITECTURE_CAMPAIGN.md` (~810 lines, §0-§12 framing matching Regional Archetype Variation's structural template). §2 architectural decisions resolved per audit §8.1's ten enumeration using audit §7 framework + Andrew Q1-Q10 strategic factors. Sub-phase chain: 6 numbered sub-phases + 1 dedicated Mediator Removal session per Q2 + Q6 + Q7 + Q8 structure.

- **Design.B** (this commit): cross-reference §10 entry in Regional Archetype Variation campaign doc + Status header line update to reflect campaign-design pass COMPLETE + Sub-phases pending.

**§2 architectural commitments** (full treatment in Editor Multi-Tool Architecture campaign doc §2; summary here):

- **§2.2 ActiveTool trait shape**: Fyrox InteractionMode pattern with adjustments — ~10 per-event default-implementation methods (mouse press/up/move/enter/leave + keyboard down/up + UI message) + lifecycle (activate/deactivate/update/on_drop) + UI integration (make_button) + UUID identity. Per-event methods default to PassThrough so tools override only what they care about.
- **§2.3 EventDisposition**: binary `Consumed`/`PassThrough` at campaign close per Q4. `#[non_exhaustive]` for forward-compatibility (ConsumedSelective variant addition for hover-feedback tools per Godot 4 CUSTOM-analog precedent without breaking consumers via match guards).
- **§2.4 Dispatcher mechanism**: push-based per-event subscription per audit §7.2 + §7.7. Dispatcher tracks active tool by UUID; HashMap<Uuid, Box<dyn ActiveTool>> collection; per-event method calls only on active tool.
- **§2.5 Registration model**: explicit `register_tool(Box<dyn ActiveTool>)` API per Q5 mod-friendliness. UUID open-set identity (third-party UUIDs don't conflict). Approach I+II hybrid synthesis per audit §7.7.
- **§2.6 Mediator fate**: replace completely per Q6. Dedicated Mediator Removal session with fresh context (NOT bundled sub-phase work) — main.rs:3833-3877 mediator code + ViewportWidget per-tool fields removed.
- **§2.7 ViewportWidget integration**: ViewportWidget owns rendering + raw input capture + depth-buffer access (preserved from current pattern at viewport/widget.rs:1219-1234); dispatcher owns tool arbitration. Post-removal end-state: no per-tool fields on ViewportWidget.
- **§2.8 Mutex arbitration**: framework-enforced via dispatcher's `active_tool: Option<Uuid>`; `set_active_tool` transitions previous deactivate → new activate.
- **§2.9 Tool composition**: deferred per Q1 + Q3 production-readiness threshold; trait surface designed to NOT preclude future extensions.
- **§2.10 Tool state persistence**: per-tool responsibility; dispatcher doesn't enforce pattern.
- **§2.11 Tool action transactionality**: per-tool responsibility; dispatcher doesn't enforce Command pattern; AstraWeave's existing `TerrainBrushCommand` undo/redo is integration target for tool implementations.

**Sub-phase chain** (full treatment in Editor Multi-Tool Architecture campaign doc §3-§9; summary here):

1. **Sub-phase 1 — Diagnostic** per Q7. Andrew-gate IF gaps surface in §2 decisions (architectural-decision touchpoint per Q9). Audit deliverable.
2. **Sub-phase 2 — ActiveTool trait + dispatcher core + register_tool API**. Code-level only; not Andrew-gated.
3. **Sub-phase 3 — TerrainPanel ActiveTool implementation (additive)** per Q2 risk-bounding. Andrew-gate REQUIRED (8 brush modes verified).
4. **Sub-phase 4 — Pattern A regression infrastructure for dispatcher class**. Code-level only; not Andrew-gated.
5. **Sub-phase 5 — RegionalArchetypePanel ActiveTool implementation + registration**. Andrew-gate REQUIRED (brush UX click+drag works end-to-end). **Likely subsumes G-pointer-events-fix per audit §8.4**.
6. **Dedicated Session — Mediator Removal** per Q6. Full fresh context; pure removal. Andrew-gate REQUIRED (comprehensive verification of both tools' brush UX).
7. **Sub-phase 6 — Closeout**. Doc-only updates; not Andrew-gated.

Estimated ~7 distinct campaign sessions; varies by sub-phase scope (code-level only ~2-3 hours; Andrew-gated visible-output ~3-5 hours each).

**Forward chain post-Design.B**:

1. Sub-phase 1 prompt drafting (separate session).
2. Sub-phase 1 execution (Sessions 4+).
3. Through Sub-phase 6 + Mediator Removal session.
4. **Resume Regional Archetype Variation**: post-Editor-Multi-Tool-Architecture closure, G-pointer-events-fix likely subsumed by Sub-phase 5 per audit §8.4. H-saveload-diagnostic + H-saveload-fix follow. Final Andrew-gate full PASS → F.5-paint COMPLETE → F.5-overlay-and-gate → F.6 → F.7 (principal Andrew-gate) → F.8 closeout.

**Scope held**: Editor Multi-Tool Architecture campaign-design pass session only modified `docs/current/EDITOR_MULTI_TOOL_ARCHITECTURE_CAMPAIGN.md` (commit `75b68e7c7`) and this Regional Archetype Variation campaign doc Status header + §10 (this commit). NO production code changes. NO test changes. NO new audit documents. NO sub-phase execution prompts (Sessions 4+ produce those). NO single-approach narrowing beyond §2 architectural commitments (which were research-informed via audit §7 + Andrew Q1-Q10 strategic factors).

**Methodological inheritance preserved** (per Q10 + audit §9):

- Regional Archetype Variation's "research-pass-before-reframe" §0 discipline pattern explicitly inherited (Editor Multi-Tool Architecture campaign doc §0 references this as canonical precedent).
- Andrew-gate authoritative for visible-output sub-phases lesson application.
- Sub-phase completion + status header maintenance discipline.
- §2 architectural commitments respected — sub-phase execution implements per §2.

The campaign-pause-and-spinoff workflow pattern Regional Archetype Variation demonstrated is canonical; future spinoffs from Editor Multi-Tool Architecture (if any) inherit it.

### 2026-07-22, Multi-Tool Sub-phase 5.C cross-reference — G-pointer-events-fix SUBSUMED

**Verdict**: **SUBSUMED**, not partial. The Pause-Artifacts entry narrowed post-pause G-fix to two requirements: implement `ActiveTool` for `RegionalArchetypePanel` and register it with the Editor Multi-Tool dispatcher. Multi-Tool SP5.B (`9bae816e3`, preceded by SP5.A `2f6f853a5`) delivers both: the real implementation is at `tools/aw_editor/src/panels/regional_archetype_panel.rs:548-629`; press/drag/release return `EventDisposition::Consumed` at `:593/:611/:627`; editor initialization registers the dispatcher-owned instance at `tools/aw_editor/src/main.rs:2797-2801`. The side channel drains into the canonical panel and undo path at `main.rs:4124-4149` and `tab_viewer/mod.rs:1485,1503`.

**Gate + regression evidence**: Multi-Tool §7.4 Andrew-gate PASS was declared 2026-07-20 and recorded in `docs/audits/T_SERIES_RATIFICATION_2026-07-20.md` §1. Multi-Tool 5.C commit `2e92b8a3b` adds `real_panel_paint_click_drag_consumes_pointer_events`, which instantiates the real dispatcher panel and proves press + drag + release all return `Consumed` (**1 passed; 4023 filtered out**; surrounding panel suite **31 passed; 3993 filtered out**; dispatcher suite **21 passed; 4003 filtered out**; full editor lib **4019 passed; 0 failed; 5 ignored**).

**Resumption**: this campaign remains paused until Editor Multi-Tool completes its dedicated Mediator Removal session and Sub-phase 6 closeout. It then skips G and resumes at **H-saveload-diagnostic** → H-saveload-fix → final F.5-paint gate. Save/load diagnosis, Climate Preview, terrain/editor polish, and all later RAV work remain untouched by 5.C.
