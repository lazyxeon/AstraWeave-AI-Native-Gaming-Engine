...
> **Observation — the dependency-drift pattern (a future hygiene item).** Two of the failing-test items trace to **dependency drift, not AstraWeave bugs**: `render`'s stale assertion (a dependency's `chunk_size` 256→512 change the test didn't track) and **possibly `physics`** (a Rapier behavior change the wrapper didn't track — the §5.1 diagnostic fork). This is the Path-B *"tests/wrappers lag a changed dependency"* class. It suggests a **periodic dependency-currency check** is worth adding as a standing hygiene item — the same shape as the architecture-trace-currency check, but for upstream dependency behavior. Recorded here so the pattern is visible; not a v1.0 milestone.
