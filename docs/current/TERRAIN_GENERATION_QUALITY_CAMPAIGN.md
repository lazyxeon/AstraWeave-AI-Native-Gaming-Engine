...
**Status**: CAMPAIGN CLOSED 2026-04-29. F.0–F.3 COMPLETE. F.4.B.1, F.4.B.2 COMPLETE. F.4.B.3.A, F.4.B.3.C, F.4.B.3.D.1–D.5 + D.5-fix + D.5-research COMPLETE. F.4.B.3.B REVERTED. F.4.B.3.D CLOSED VIA ARCHITECTURAL PIVOT to `REGIONAL_ARCHETYPE_VARIATION_CAMPAIGN.md`. F.4.B.3.E and F.4.B.3.F demoted (absorbed into F.4.B.3.D.3). F.4.B.3.G NOT STARTED (47.4 WU phase-2 grassland precision floor; standalone follow-up). F.5 CLOSED WITH REDUCED SCOPE. Drafted 2026-04-21 as F.0 artifact; closed 2026-04-29 via architectural pivot.
...
F.4 — Climate as spatial field + Target B scale rework: IN PROGRESS
  ...
  F.4.B.3.D (climate-field-driven Whittaker biomes — campaign reframe): CLOSED VIA ARCHITECTURAL PIVOT 2026-04-29, commit d7c5be917. D.1-D.5 within-region machinery complete and ship-quality (climate field, Whittaker biome lookup, per-biome parameters, scattered-convolution biome blending, world archetype catalog + UI, Path B amplitude reduction). Per-archetype world shape variety NOT delivered within this campaign's architecture; the architectural correction is the upstream regional layer (paintable archetype mask + climate-driven shape splines per archetype) which is the new Regional Archetype Variation campaign's scope. See `docs/current/REGIONAL_ARCHETYPE_VARIATION_CAMPAIGN.md` (F.0 commit `0e51763d4`) for the new campaign's ground-truth document; D.1-D.5 deliverables are preserved as the within-region machinery the new campaign builds on top of. Original framing (architectural correction within-campaign) preserved below for historical context.

  Replaces the original "multi-scale locality" framing with a full architectural correction. F.4.B.3.B + F.4.B.3.C REGRESSes established that biome presets are the wrong abstraction at Target B+ scale; per-vertex transforms cannot fix preset-imposed uniformity. F.4.B.3.D replaces the eight-biome preset system with: per-vertex `ClimateMap::sample()` returning real-world units (D.1), Whittaker `(temp, moisture, elevation) → BiomeId` lookup over a fixed 11-terrestrial + 5-aquatic + 3-overlay biome taxonomy (D.2), per-`BiomeId` parameter system replacing `BiomeNoisePreset` (D.3), scattered-convolution biome blending (D.4), six `WorldArchetype` UI presets (D.5), and Andrew-gate closeout (D.6). Absorbs the originally-planned F.4.A (climate-as-spatial-field), F.4.B.3.E (ridge integration → demoted to per-biome `ridge_strength` parameter), and F.4.B.3.F (altitude/concavity → demoted to per-biome parameter). Mountain preset is **known-broken** between commit `f7a43759d` (F.4.B.3.C) and end of D.3; expected, not addressed (preset is being removed).
    ...
    F.4.B.3.D.5 (world archetype UI): code-level COMPLETE 2026-04-28, commits 88c1d2669 (D.5a) + 6538acbed (D.5b). Andrew-gate REGRESS (2026-04-28): explosive radial spike pattern across all six archetypes. Diagnostic chain: `docs/audits/f4b3d5_diagnostic_report_2026-04-28.md` (synthetic-uniform sampling: 41-53% mountain-character biomes) + `docs/audits/f4b3d5_diagnostic_2_real_heightmap_2026-04-28.md` (real-chunk: 28-29% MC, 99.6% above 280m, COHERENT spatial pattern, max elevation 1214m vs 510m Target B target). Recommended remediation **Path B** landed in F.4.B.3.D.5-fix.
    F.4.B.3.D.5-fix (Path B amplitude reduction for elevation-overlay biomes): COMPLETE 2026-04-28 (code level), commit 2d7459a75 + hash-fixup 35c6b4244. Per-biome `mountains_amplitude` reduced for the four elevation-overlay biomes per D.5-diagnostic-2's Path B sketch: Alpine 2.5→1.4 (-44%), SnowCap 2.5→1.4 (-44%), MountainRocky 3.0→1.6 (-47%), Scree 2.0→1.2 (-40%). One iteration round converged: re-run of `phase_1_6_f4_b_3_d_5_diagnostic_2_real_heightmap` measured max real-chunk elevation 1214m → 698.5m (-43%) for both Continental Temperate and Equatorial Tropical, MC% unchanged at 28-29%, spatial pattern COHERENT. D.4 `gradient_smoothness` threshold tightened 0.5 → 0.4 (smaller amplitude differentials reduce per-step delta). Phase-2 continuity tolerances tightened: grassland 140 → 80 WU (measured 69.3 / 67.8 max post-Path-B vs 124.3 / 101.3 at D.5a), mountain 25 → 10 WU (measured 4.4 / 3.3 max post-Path-B vs 9.6 / 20.0 at D.4). D.3a's `alpine_biomes_have_dramatic_mountains` test renamed to `alpine_biomes_have_elevated_mountains` and updated to assert `[1.0, 2.0]` range. Test scoreboard: 738/738 lib tests pass, all targeted regression suites green, `cargo check` clean for terrain + aw_editor + render. Andrew-gate ablation post-Path-B revealed FOUR archetypes (CT, ET, Desert, Mediterranean) producing visually identical worlds despite Path B successfully bringing max elevation into Target B's design range; triggered F.4.B.3.D.5-diagnostic-3. D.5c (Climate Preview overlay) DEFERRED per §1.5 fallback (rendering pipeline integration out of scope; D.6 uses distribution tests + interactive viewing for verification). New `astraweave-terrain/src/world_archetypes.rs` module (~330 lines + 12 unit tests) ships the six archetypes: Continental Temperate (default; lifted from D.1's `WorldArchetype::default()` into the catalog with §1.1 tuned variances 600→400 / 0.25→0.2), Equatorial Tropical, Boreal/Subarctic, Mediterranean, Desert, Custom. `WorldArchetypeId` enum exposes `all()`, `display_name()`, `description()`, `default_archetype()` for editor consumption. Per-archetype distribution tests (10K samples each) verify documented expectations: tropical-family ≥30% in Equatorial, cold-family ≥30% in Boreal, arid-family ≥40% in Desert, etc. `every_biome_appears_in_some_archetype` test verifies §1.6 verification criterion. One tuning iteration during D.5a: Equatorial Tropical moisture (2200 ± 800) → (1900 ± 1300) to reach Savanna's 250-1000mm band per documented expectation. `WorldArchetype::default()` delegates to `world_archetypes::continental_temperate()` for catalog symmetry. Editor changes (D.5b): "Primary Biome" dropdown replaced with "World Archetype" dropdown (repurposed at same position per §1.3), tooltip shows archetype description, Custom mode exposes 7 climate-envelope sliders bounded to validate() ranges, Mountain Drama slider REMOVED per §1.4 (was inert since D.3c). New `TerrainState::set_world_archetype` method; `regenerate_terrain` plumbs selected archetype through to `ClimateConfig.archetype`. Phase-2 continuity grassland tolerance updated 90→140 WU (lifted variances tightened biome boundaries → larger amplitude flips at chunk shared edges; measured 124.3 WU at seed 12345; pre-existing 47.4 WU floor still flagged for F.4.B.3.G). Test scoreboard at D.5 close: 738/738 lib tests pass (12 new D.5a tests added on top of 726), all targeted regression suites green, `cargo check` clean for terrain + aw_editor + render. Architectural correction landed at user-facing layer: editor now asks "What kind of world do you want?" (archetype answers) rather than "What biome dominates this world?" (preset answers).
    F.4.B.3.D.5-diagnostic-3 (cross-archetype terrain similarity measurement): COMPLETE 2026-04-28 (investigation-only; no production code changes). See `docs/audits/f4b3d5_diagnostic_3_cross_archetype_2026-04-28.md`. Single `#[ignore]`-marked test `phase_1_6_f4_b_3_d_5_diagnostic_3_cross_archetype.rs` (~36s) measured cross-archetype heightmap correlation, per-source variance decomposition, per-archetype amplitude distributions, and audited `WorldConfig`/`NoiseConfig` parameters for archetype-awareness. **Findings**: cross-archetype Pearson 0.978-0.989 (CT vs Desert 0.989; CT vs ET 0.978) — nearly identical worlds; bootstrap noise contributes 72-81% of terrain shape variance, per-biome modulation contributes 19-28%; per-archetype amplitude distributions cluster narrowly (means 1.097-1.379, 60-85% of samples in `[1.0, 1.5)`); 1 of 18 bootstrap parameters is archetype-aware (the per-vertex `mountain_amplitude_multiplier`). The visual identicality is correctly explained by the data: per-archetype variation is a scalar height-scaling, not a shape change. Architecture is producing the same world with ~12% perturbation per archetype. **Two campaign-level paths surfaced (decision is Andrew's appetite call, not data)**: Path 1 — branch new sub-campaign for per-archetype bootstrap shaping (6-10 sessions, inherits all D.1-D.5 work); Path 2 — close current campaign as PARTIAL with documented deferred work (per-archetype variety, Climate Preview, ET archetype-specific tuning, bootstrap elevation skew, F.4.B.3.G inheritances). No production changes; campaign branching deferred to D.6 prompt with Andrew's path selection.
    F.4.B.3.D.5-research (regional archetype variation research pass): COMPLETE 2026-04-29 (investigation-only; no production code; precondition for new-campaign launch). See `docs/audits/regional_archetype_variation_research_2026-04-29.md`. Research pass surveyed shipping AAA prior art (Crimson Desert primary; Enshrouded concrete-implementation; Minecraft 1.18+ / NMS / Witcher 3 / Horizon / Skyrim / Hytale supplementary) plus algorithmic literature (Voronoi region assignment, climate-field extension, painted regions with falloff, hierarchical noise, Minecraft-style climate-driven shape splines). Six algorithmic approaches A-F surveyed against five criteria each. **Convergent finding across 6 of 7 AAA references**: hand-authored macro layout + procedural detail filling is the dominant shipping AAA pattern; Minecraft 1.18+ is the algorithmic outlier with climate-driven shape variation via spline-based density functions. **Recommendation: Hybrid C + F** — paintable archetype mask (Approach C, honors Veilweaver writer intent) + climate-driven shape splines per archetype (Approach F, Minecraft 1.18+ canonical pattern, integrates with D.1-D.5 architecture). New campaign sketches 8 sub-phases (F.0-F.8); estimated 8-12 sessions including diagnostic+remediation pairs. Data-driven success criterion: cross-archetype Pearson drops from current 0.978-0.989 to target < 0.7 (distant pairs) / < 0.85 (adjacent pairs). Architectural commitment is real but bounded; D.1-D.5 work preserved as within-region machinery, new campaign adds upstream regional layer. Crimson Desert at ~150 km² with 5 archetype regions + GDC 2025 disclosure that "weather computes from biome type, temperature, elevation, wind patterns, time of day" confirms per-region climate parameter blocks at runtime — load-bearing AAA validation that AstraWeave's `WorldArchetype` is a structurally-aligned analog. Two follow-up sessions pre-scoped: (1) campaign-design session drafts new ground-truth doc; (2) F.4.B.3.D + F.5 closure session marks F.4.B.3.D as CLOSED VIA PIVOT (not COMPLETE/PARTIAL) and re-marks parent campaign Phase 1 / 1.5 COMPLETE.
    F.4.B.3.D.6 (Andrew-gate + closeout): SUPERSEDED — F.4.B.3.D will be CLOSED VIA PIVOT in a follow-up session referencing the regional archetype variation campaign. Original D.6 scope (Andrew-gate visual verification of D.5 archetypes) does not apply: diagnostic-3 + research pass established that the archetype-as-coloring outcome is structural, not tunable within F.4.B.3.D's scope.
...
F.5 — Editor UI wiring + integration tuning + closeout: CLOSED WITH REDUCED SCOPE 2026-04-29, commit d7c5be917. Original scope (apply_erosion handler wiring + 8-climate integration tuning + ARCHITECTURE_MAP update + parent campaign re-mark) reduced because (a) the eight-biome preset system was removed in F.4.B.3.D.3 (BiomeNoisePreset deleted; integration tuning at the preset level no longer applies); (b) integration tuning at the archetype level moves to the new Regional Archetype Variation campaign's F.7 (per-archetype tuning + Andrew-gate). F.5 retains: ARCHITECTURE_MAP.md terrain section update (executed in this closure session); parent campaign §7 re-mark of Phase 1 + Phase 1.5 COMPLETE (executed in this closure session); apply_erosion handler wiring deferred to standalone follow-up if still relevant after the new campaign lands (the new campaign's F.4 changes how erosion preset selection works; revisit then).
```
...
### 2026-04-28, Sub-phase F.4.B.3.D.5-fix (Path B amplitude reduction for elevation-overlay biomes), commit 2d7459a75

**Verdict:** PASS (one-round convergence; Andrew-gate ablation pending user run).

**Remediation summary:** D.5-diagnostic-2's recommended Path B applied verbatim. Per-biome `mountains_amplitude` reduced for the four elevation-overlay biomes:

| Biome | Pre-fix | Post-fix | Δ |
|-------|--------:|---------:|---:|
| Alpine | 2.5 | 1.4 | -44% |
| SnowCap | 2.5 | 1.4 | -44% |
| MountainRocky | 3.0 | 1.6 | -47% |
| Scree | 2.0 | 1.2 | -40% |

No other parameters changed. Other 15 biomes (11 terrestrial + 4 aquatic excluding the elevation overlays + River) retain D.3a values unchanged.

**Round 1 measurement (single iteration, converged)**:

Re-ran `phase_1_6_f4_b_3_d_5_diagnostic_2_real_heightmap` against the post-fix code:

| Metric | Pre-fix (D.5b) | Post-fix (D.5-fix Path B) | Δ |
|--------|---------------:|--------------------------:|---:|
| **Continental Temperate** | | | |
| MC% | 28.93% | 28.93% | unchanged |
| Max elevation | 1214m | **698.5m** | **-43%** |
| MC p50 elevation | 650.5m | 407.6m | -37% |
| Above 280m (% of MC) | 99.65% | 95.54% | -4.1pt |
| Spatial pattern | COHERENT | COHERENT | unchanged |
| **Equatorial Tropical** | | | |
| MC% | 28.76% | 28.76% | unchanged |
| Max elevation | 1214m | **698.5m** | **-43%** |
| MC p50 elevation | 641.7m | 398.3m | -38% |
| Above 280m (% of MC) | 99.61% | 93.42% | -6.2pt |
| Spatial pattern | COHERENT | COHERENT | unchanged |

Both archetypes converged to the same max (698.5m), at the upper edge of the 500-700m target range from §1.4 plan. MC% biome distribution unchanged (Path B affects HEIGHTS but not WHICH biome a vertex resolves to). Spatial pattern remains coherent — Path B did not introduce new boundary-thrashing artifacts.

**Path B converged in ONE iteration**; §1.4's 3-iteration bound was not needed.

The slight increase in below-280m MC fractions (0.35% → 4.46% CT, 0.39% → 6.58% ET) is the expected §2.5 invariant artifact: lower amplitude peaks erode to lower elevations, dropping more vertices below their biome's elevation threshold post-erosion while the PRE-erosion biome ID assignment remains correct. Not a regression.

**Test threshold tightening enabled by Path B:**

- D.4 `gradient_smoothness` threshold: 0.5 → 0.4. Maximum amplitude differential at any biome boundary post-Path-B is ≤0.7 (vs ≤1.7 pre-fix); per-step blended delta correspondingly reduced.
- Phase-2 continuity grassland tolerance: 140 → 80 WU. Measured 69.3 max at seed 12345 (vs D.5a's 124.3 max). Pre-existing 47.4 WU floor (F.4.B.3.G inheritance) still the underlying floor; 80 WU is ~70% headroom over floor.
- Phase-2 continuity mountain tolerance: 25 → 10 WU. Measured 4.4 max at seed 12345 (vs D.4's 20 max). Returns to roughly the F.3-phase-3 baseline of ~2.3 WU. The BorealForest 1.5 ↔ SnowCap differential dominated D.3b/D.4 divergence at 1.0 absolute amplitude; post-Path-B that differential is BorealForest 1.5 ↔ SnowCap 1.4 = 0.1, a 10× reduction.

**Test updates**:

- `phase_1_6_f4_b_3_d_3_alpine_biomes_have_dramatic_mountains` (D.3a) renamed to `phase_1_6_f4_b_3_d_3_alpine_biomes_have_elevated_mountains` and tightened to assert `mountains_amplitude` in `[1.0, 2.0]` range (instead of `>= 2.0`). The new range still distinguishes elevation-overlay biomes from grassland-class biomes (0.8×) while accommodating Path B's reduction. Documented in test's doc comment.

**Test scoreboard at F.4.B.3.D.5-fix close:**
- All 738 terrain crate lib tests pass (3 ignored): includes biome_parameters (8/8), biome_param_blending (10/10), biome_lookup (25/25), climate (18/18), world_archetypes (12/12), F.2 permanent regression (5/5), runevision (6/6), perlin_gradient (4/4), noise_gen (18/18), elevation_biome remaining tests, others.
- D.3b integration (6/6): pass.
- Phase-3 diagnostic (3/3): pass.
- Phase-2 continuity (4/4): pass with D.5-fix tightened thresholds.
- D.4 `gradient_smoothness` at tightened 0.4: pass.
- `cargo check` (terrain + aw_editor + render): clean.

**Pre-existing failures unchanged from D.4** (3 elevation_biome tests retired in D.3c; phase-2 grassland 47.4 WU floor still flagged for F.4.B.3.G).

**Deviations from §1 plan:**

- *§1.4 iteration bound (≤3 rounds)*: Path B converged in ONE round at the sketch values. No further iteration needed. The 3-round bound was a worst-case allowance; data converged on first attempt because the sketch was data-driven (510/1214 ratio + headroom from D.5-diagnostic-2).
- *§1.5 Andrew-gate*: deferred to user run (auto mode). The visual ablation (Continental Temperate + Equatorial Tropical + a third archetype at radius 10 seed 12345) is the gating decision; D.5-fix's code-level convergence + tightened test thresholds are the prerequisite. Andrew's PASS/PARTIAL/REGRESS verdict drives D.5-fix closeout.
- *§1.6 phase-2 continuity tightening*: §1.6 noted "Phase-2 continuity tolerances: revisit. D.4 tightened mountain to 25 WU." Done — both grassland (140→80) and mountain (25→10) tightened.

**What this remediation did NOT address (deferred to follow-up sessions):**

- Equatorial Tropical's archetype-specific tropical character (audit §6 #1): Path B reduced max elevation, but ET still produces ~29% mountain-character biomes due to warm temperatures keeping elevation overlays as Alpine instead of SnowCap. The visual character of "tropical world with ~30% Alpine peaks" may or may not feel right per Andrew's gate. If REGRESS or PARTIAL, archetype-specific tuning (latitude_temperature_drop_c bump, or per-archetype bootstrap-amplitude reduction) is the next session.
- Bootstrap noise pipeline elevation skew (audit §7 / D.5-diagnostic-2 §7 question): "26% of vertices land at elevation ≥ 450m" was identified as too high for "lowland with rare peaks" intent. Path B reduces the multiplier on top of bootstrap; doesn't fix the bootstrap distribution. If post-Path-B worlds still feel "all mountain" despite max elevation being correct, bootstrap is the next investigation.
- Other 4 archetypes' real-chunk measurements (Boreal/Subarctic, Mediterranean, Desert, Custom): not measured at real-chunk level. The strikingly similar 28-29% MC across CT and ET suggests these will land in the same range; Andrew's third-archetype validation in §1.5 + per-archetype follow-up sessions if needed.
- Dead-write bug at terrain_panel.rs line 943 (D.5-diagnostic Q2): small cleanup; tracked for follow-up.
- F.4.B.3.G inheritances (47.4 WU phase-2 grassland precision floor + Climate Preview overlay).

**Andrew-gate ablation (deferred)**: per §1.5 plan: Continental Temperate + Equatorial Tropical + one Andrew-chosen archetype (Boreal/Subarctic, Mediterranean, or Desert) at radius 10 seed 12345. Verdict drives:
- **PASS**: spike pattern eliminated, mountain-character regions plausible. Proceed to D.6.
- **PARTIAL**: amplitude reduction landed, but new artifacts emerge (e.g., terrain too flat, ET too mountain-dominated). One more iteration round per Andrew's feedback OR archetype-specific tuning.
- **REGRESS**: spike pattern persists. Path B premise needs reconsideration; alternatives are bootstrap pipeline review (audit §7) or Path C variant from D.5-diagnostic-2 §6.

**Scope held**: D.5-fix only modifies `astraweave-terrain/src/biome_parameters.rs` (4 amplitude values + test rename), `astraweave-terrain/src/biome_param_blending.rs` (gradient_smoothness threshold), and `astraweave-terrain/tests/phase_1_6_f3_phase_2_continuity.rs` (continuity threshold updates). No archetype changes, no threshold changes, no bootstrap pipeline changes, no D.4 blending algorithm changes — just per-biome amplitude tuning per Path B's bounded scope.
...
**Trigger:** D.5-fix's Path B successfully reduced max real-chunk elevation 1214m → 698.5m (-43%) across CT and ET, validating the amplitude-reduction premise. Andrew-gate visual ablation post-Path-B revealed a deeper issue: four archetypes (Continental Temperate, Equatorial Tropical, Desert, Mediterranean) produce visually identical worlds. Different surface coloring, different scatter density, but the terrain _shape itself_ — peak placement, valley cuts, mountain spacing — is approximately archetype-invariant. This diagnostic measures the contribution of archetype-aware vs archetype-agnostic code paths to actual terrain shape variance, to inform a campaign-level Path 1 / Path 2 decision.

**Single `#[ignore]`-marked test added**: `astraweave-terrain/tests/phase_1_6_f4_b_3_d_5_diagnostic_3_cross_archetype.rs` (~36s wall-clock). Three measurements + one code inspection; no production changes:

- §1.1 cross-archetype heightmap similarity (5 chunks per archetype pair).
- §1.2 per-source variance decomposition (chunk (0,0), 3 archetypes).
- §1.3 per-archetype amplitude distributions (1000 samples × 6 archetypes).
- §1.4 bootstrap parameter archetype-awareness audit (code inspection).

**Measurement summary** (full data in `docs/audits/f4b3d5_diagnostic_3_cross_archetype_2026-04-28.md`):

| Measurement | Result | Decision-matrix bin |
|-------------|--------|---------------------|
| §1.1 CT vs Desert Pearson | **0.989** | > 0.95 (nearly identical) |
| §1.1 CT vs ET Pearson | **0.978** | > 0.95 (nearly identical) |
| §1.1 Mean \|Δ\| (both pairs) | 22m | "meaningful but bounded" band |
| §1.2 Bootstrap variance contribution | **72-81%** | Bootstrap dominates |
| §1.2 Per-biome modulation contribution | 19-28% (CT 19.2%, Desert 25.1%, ET 28.1%) | Ratio 0.10-0.30 |
| §1.2 Identical p95/max across archetypes | 542.32m / 736.90m | Bootstrap fully owns high-elevation regime |
| §1.3 Amplitude distribution means | 1.097-1.379 (26% relative span) | Narrow band |
| §1.3 Mass in `[1.0, 1.5)` | **60-85%** across all 6 archetypes | Same dominant range |
| §1.4 Archetype-aware bootstrap parameters | **1 of 18** (per-vertex `mountain_amplitude_multiplier` only) | Bootstrap is structurally archetype-agnostic |

**Findings**:

1. Bootstrap dominates terrain shape (72-81%); per-biome modulation contributes 19-28%.
2. Bootstrap is 100% archetype-agnostic — all shape-determining parameters (mountain bulk amplitude, scale, octaves, noise type, continental scale, base elevation, etc.) are constants from `NoiseConfig::default()`. The only archetype-aware lever is the per-vertex mountain amplitude _multiplier_.
3. A scalar multiplier cannot change shape; it can only scale heights. Different archetypes scale the same underlying noise field by slightly different amounts.
4. The architecture is producing the same world with ~12% perturbation per archetype (22m mean |Δ| on top of 178m mean elevation). To the eye, that reads as "same world, slightly different lighting."
5. **None of these findings are bugs.** Each piece works as designed. The architecture chose to make biomes drive _per-biome shape character via amplitude_ (D.3a's design decision), not _per-archetype shape character via bootstrap parameters_. The visual variety problem is in the design boundary, not in implementation correctness.
6. D.5-fix's Path B did its job. Max elevation 1214m → 698m as intended. The post-Path-B amplitude range (1.0-1.5 dominant band) IS the bounded variance the audit measures. Path B didn't shrink archetype variety — that variety was always small.

**Two campaign-level paths surfaced (data does NOT decide; decision is Andrew's appetite + product priority):**

- **Path 1 (recommended for product impact)**: Branch a new sub-campaign for per-archetype bootstrap shaping. Scope sketch: research pass + F.0-F.5 (6-10 sessions). Per-archetype `NoiseConfig.mountains.{amplitude, scale, octaves, noise_type}`, per-archetype `continental_scale`, per-archetype `base_elevation.amplitude`, possibly per-archetype `erosion_preset`. Inherits everything D.1-D.5 built (climate field, biome lookup, per-biome parameters, blending) — adds an upstream archetype-driven parameter layer. Re-run diagnostic-3 between sub-phases to verify Pearson drops from ~0.98 toward 0.5-0.7. Risk: per-archetype noise tuning is iterative; some archetypes may need erosion-style or texture-palette differentiation in addition to bootstrap parameters.
- **Path 2 (recommended for campaign efficiency)**: Close current campaign as PARTIAL. Ship F.4.B.3.D with the D.1-D.5 deliverables (climate field, Whittaker biome lookup, per-biome parameters, scattered-convolution blending, world archetype catalog + UI, Path B amplitude reduction). Document deferred work: per-archetype world shape variety, Climate Preview overlay, ET archetype-specific tuning, bootstrap noise pipeline elevation skew, dead-write bug at terrain_panel.rs line 943, F.4.B.3.G inheritances. Accepts archetype-as-coloring + biome-classification as the current layer of variety. Per-archetype shape variety becomes a Phase 2/3 polish concern, not a Phase 1 blocker. Lets Veilweaver content work proceed against the current terrain.
...
| Risk | High (iterative tuning, may not converge per-archetype) | Low (documented limitation) |
...
**Scope held**: investigation-only sub-phase. Single `#[ignore]`-marked test file + audit doc + this §10 entry + §9 line. No `BiomeParameters` changes, no `WorldArchetype` changes, no bootstrap parameter changes, no test threshold changes, no documentation reorganization. The remediation prompt that follows (D.6 or a Path 1 launch prompt) is informed by this audit; this sub-phase does not preempt Andrew's path decision.

**Test disposition**: KEEP as `#[ignore]`-marked. Re-runnable as the principal "did per-archetype variety land?" signal if Path 1 is selected. If Path 2 is selected, KEEP as as-shipped baseline measurement for any future campaign that revisits per-archetype variety.

**Pre-existing inheritances unchanged**: phase-2 grassland 47.4 WU floor still flagged for F.4.B.3.G. Climate Preview overlay still deferred. Dead-write bug at terrain_panel.rs line 943 still tracked.

**Andrew-gate**: not applicable to a measurement audit. The decision is Path 1 vs Path 2. The diagnostic-3 prompt explicitly framed this as "the data informs but does NOT decide; Andrew's appetite + product priority decide."
...
**Trigger:** Andrew's 2026-04-29 chat clarified the actual product target after diagnostic-3's data: regional archetype variation with smooth blending, ~5-10 archetypes per world, one archetype per Tikva storyline region. This is architecturally larger than diagnostic-3's Path 1 sketch (which framed per-world archetype selection). The new target is "biome of biomes" — multiple archetypes coexist within a single world, organized regionally, with smooth transitions at flyover scale (Crimson Desert atmosphere-globe-view as visual reference). Closing F.4.B.3.D as PARTIAL would attempt to address regional variation inside a campaign drafted from biome-classification first principles; the cleaner path is a new campaign with explicit regional-archetype scope. This research pass is the new campaign's launch precondition.

**Single audit document deliverable**: `docs/audits/regional_archetype_variation_research_2026-04-29.md` (~750 lines). Investigation-only; no production code, no campaign-doc creation, no F.4.B.3.D closure. The new campaign's ground-truth document is drafted in a follow-up session; this audit's §7 is the load-bearing input.

**Research scope**:
- §3 **Crimson Desert (primary reference)**: Pearl Abyss BlackSpace engine. GDC 2025 closed-door talk (March 2025) yielded named streaming primitives (hierarchical level subdivision, hierarchical proxy load, imposter rendering), voxel SDF clipmaps for GI (~2km coverage, UE5 Lumen-analog), full path tracing on PC, FFT ocean + shallow water simulation, and **climate-per-region runtime calculation** confirmed at the data-model level. World ~150 km² with 5 archetype regions.
- §4 **Enshrouded (concrete-implementation reference)**: Keen Games Holistic engine. Published authoring pipeline at Graphics Programming Conference Breda 2024-2025 (NOT GDC; 6 talks with PDF slides + YouTube). World "handcrafted by the developers" (official AMA quote). Authoring pipeline: rough sculpt → sub-biome scene → POI placeholders → hand-placed voxel stamps → algorithmic detail pass. 0.5m voxel resolution; 24 km² EA → 64 km² target.
- §5 **Supplementary**: Minecraft 1.18+ MultiNoise biome architecture (6D NoiseHypercube nearest-neighbor + climate-driven splines mapping `(continentalness, erosion, PV) → (offset, factor)` for terrain shape; the canonical published pattern for climate-driven shape variation). NMS one-archetype-per-planet with 7-layer Uber Noise stack per archetype, deterministic seed cascade. Witcher 3 / Horizon / Skyrim / Hytale all show hand-authored macro + procedural detail.
- §6 **Six algorithmic approaches surveyed**: (A) Voronoi + blend zones; (B) climate-field-driven; (C) author-painted regions + falloff; (D) hierarchical noise pyramid; (E) hybrid C+B; (F) climate-driven splines à la Minecraft. Evaluation across 5 criteria: complexity, authoring UX, blending quality, performance, scalability to 5-10 archetypes, **honors author intent**. Only A (with author-placed seeds), C, and E satisfy "honors author intent" — the deciding criterion for Veilweaver.

**Recommendation (§7): Hybrid C + F.**

- **Painted archetype mask + falloff** (Approach C) for archetype assignment. Honors Tikva writer intent. Confirmed AAA pattern by Crimson Desert, Enshrouded, Skyrim, Hytale, Witcher 3 (5 of 6 references).
- **Climate-driven shape splines per archetype** (Approach F, Minecraft 1.18+ canonical) for per-region shape character. Each `WorldArchetype` carries `BootstrapSplineSet` with curves for `mountains_amplitude`, `mountains_scale`, `continental_scale`, `base_elevation_amplitude`. Splines read climate field (D.1's `ClimateMap::sample`, extended with `erosion` + `weirdness` fields). Per-archetype noise stack as data structure (NMS pattern).

The painted mask gives writer **what** the region is. The splines + per-archetype noise stack give each region **how** the terrain expresses it. Combination produces hand-authored macro + per-region procedural shape character + smooth transitions, convergent with all 6 AAA references in §3-§5.

**New campaign high-level sub-phase sketch (§7.3)**: F.0 plan + F.1 climate field extension + F.2 BootstrapSplineSet infrastructure + F.3 spline wiring (single archetype) + F.4 RegionalArchetypeMask + falloff sampler + F.5 editor UI for archetype painting + F.6 scattered-convolution at regional layer + F.7 per-archetype tuning + Andrew-gate + F.8 closeout. Estimated 8-12 sessions.

**Data-driven success criterion** (§7.5): cross-archetype Pearson drops from current 0.978-0.989 to **target < 0.7 (distant pairs) / < 0.85 (adjacent pairs)** — measurable via diagnostic-3-equivalent re-run after F.7's per-archetype tuning. Visual Andrew-gate complements but does not replace this objective signal.

**Deferred-work inheritance plan (§8)**:
- **Absorbed into new campaign**: per-archetype world shape variety (core); ET archetype-specific tuning (F.7); bootstrap noise pipeline elevation skew (F.7 spline tuning); Climate Preview overlay D.5c (F.5 — useful for archetype painting UI).
- **Stays as standalone follow-ups**: F.4.B.3.G 47.4 WU phase-2 grassland precision floor (unrelated f32 precision); dead-write bug at terrain_panel.rs line 943; stale `NoiseConfig` literals in `behavioral_correctness_tests.rs` (pre-existing test file failure); MountainRocky/River unproduced biomes (River → hydrology campaign); per-biome runevision tuning (architecturally orthogonal).

**Risk register (§9)** — top 3:
1. Writer authoring time. Painting UX must enable plausible-world authoring in <30 minutes; F.5 Andrew-gate validates.
2. Spline tuning is iterative. F.7 includes diagnostic-3-equivalent re-measurement as objective convergence signal.
3. Performance at radius 10 with 5-10 archetypes. Per-vertex archetype-blend + spline evaluation may push over budget; F.4 measures early; ≤+30% over D.5-fix baseline.

**Two follow-up sessions pre-scoped**:
1. **Campaign-design session**: drafts new ground-truth `.md` document (likely `docs/current/REGIONAL_ARCHETYPE_VARIATION_CAMPAIGN.md`) using this audit as input.
2. **F.4.B.3.D + F.5 closure session**: marks F.4.B.3.D as **CLOSED VIA PIVOT** (not COMPLETE, not PARTIAL); enumerates §8.1 vs §8.2 split; runs F.5's reduced closeout scope; updates parent `TERRAIN_MATERIAL_SYSTEM_CAMPAIGN.md` (Phase 1 + 1.5 re-mark COMPLETE).

**Scope held**: investigation-only sub-phase. Three deliverables: audit doc + this §10 entry + the §9 line. No campaign-branching commits, no new sub-phase entries beyond §9 line + §10 deviation entry, no structural changes to TERRAIN_GENERATION_QUALITY_CAMPAIGN.md beyond these two updates, no production code, no test changes. Andrew's appetite/priority decision (proceed with new campaign) drives the next prompt.

**Andrew-gate**: not applicable to a research pass. The audit's §7.1 recommendation is the load-bearing decision artifact; Andrew either commits to the new campaign or pushes back on the recommendation. The diagnostic-3 prompt explicitly framed this preceding-decision discipline ("the data informs but does NOT decide; Andrew's appetite + product priority decide").

**Next**: campaign-design session drafts new ground-truth doc.

### 2026-04-29, Sub-phase F.4.B.3.D.5-design (campaign-design pass for new campaign), commit 0e51763d4
...
### 2026-04-29, Sub-phase F.4.B.3.D + F.5 closure, commit d7c5be917

**Closure entry — not a deviation; captures the architectural-pivot rationale and the campaign-level lessons for future campaigns.**

**Closure decision**: F.4.B.3.D CLOSED VIA ARCHITECTURAL PIVOT (not COMPLETE, not PARTIAL). F.5 CLOSED WITH REDUCED SCOPE. Predecessor campaign as a whole CLOSED. New Regional Archetype Variation campaign launched at `docs/current/REGIONAL_ARCHETYPE_VARIATION_CAMPAIGN.md` (ground-truth doc landed in commit `0e51763d4`).

**Why CLOSED VIA PIVOT and not COMPLETE/PARTIAL**:

- COMPLETE would be dishonest. F.4.B.3.D's implicit promise (via the world archetype dropdown shipped at D.5b) was per-archetype world variety. Diagnostic-3 measured the gap quantitatively: cross-archetype Pearson 0.978-0.989; 1-of-18 bootstrap parameters archetype-aware; bootstrap dominates 72-81% of terrain shape variance and is structurally archetype-agnostic. Marking COMPLETE would set the precedent that "code-level deliverables shipped" equals campaign success, which is the failure mode §0 anti-drift discipline was designed to prevent.

- PARTIAL would imply "got most of the way and stopped." Inaccurate. What happened is that the architectural target shifted as the campaign learned more — the diagnostic-3 measurements + Andrew's 2026-04-29 chat clarification of Veilweaver's actual product requirement (regional archetype variation with smooth blending, not per-world archetype selection) revealed that the broader scope is upstream of where F.4.B.3.D operated. Not partial completion of original scope; an architectural pivot to a different, larger scope.

- CLOSED VIA PIVOT is the honest framing. This campaign's scope was bounded; D.1-D.5 delivered ship-quality work within that scope; the broader architectural target requires a different campaign with different scope. Avoids the trap of perpetually-extending a campaign as scope grows.

**What landed (F.4.B.3.D D.1-D.5 + D.5-fix + research pass)**:

- D.1 climate field architecture (`ClimateMap` + `WorldArchetype` + `ClimateSample` real-world units API).
- D.2 Whittaker biome lookup (`BiomeId` taxonomy of 19 fixed variants + `lookup_biome` deterministic function).
- D.3 per-biome parameter system replacing legacy `BiomeNoisePreset` (per-biome `mountains_amplitude`, `ridge_strength`, `runevision_config`, etc.).
- D.4 scattered-convolution biome blending (NoisePosti.ng-style organic boundary softening).
- D.5 world archetype catalog + UI (6 archetypes; editor dropdown).
- D.5-fix Path B amplitude reduction for elevation-overlay biomes (max real-chunk elevation 1214m → 698.5m).
- Diagnostic-2 + diagnostic-3 audits (real-chunk biome distribution, cross-archetype variance decomposition).
- Research audit `regional_archetype_variation_research_2026-04-29.md` (load-bearing input for the new campaign).

These are preserved as the within-region machinery the new campaign builds on top of. The new campaign's §1.4 explicitly enumerates which D.1-D.5 components stay unchanged vs which get extended.

**What did NOT land within F.4.B.3.D's scope (correctly, because architectural)**:

- Per-archetype world shape variety. Cross-archetype Pearson 0.978-0.989 at campaign close; new campaign's F.7 targets < 0.7 (distant) / < 0.85 (adjacent).
- Regional archetype variation (5-10 archetypes per world, paintable). New campaign's F.4-F.6 territory.
- Climate-driven shape splines per archetype. New campaign's F.2-F.3 territory.
- Crimson-Desert-class atmosphere-flyover visual quality. New campaign's F.7 Andrew-gate target.

**Campaign-level lessons for future campaigns** (captured here for explicit reference):

1. **Code-level PASS is not plan-level success without Andrew-gate.** F.4.B.3.D ran 6+ sub-phases that each declared code-level PASS while Andrew-gate caught REGRESSes three times: F.4.B.3.B (octave-emphasis Path 1), F.4.B.3.C (runevision filter), F.4.B.3.D.5 (archetype-as-coloring after Path B amplitude reduction). The pattern: numerical metrics passed; visual reality failed. New campaign's §0 bakes Andrew-gate authoritative discipline for visible-terrain sub-phases as non-negotiable structural commitment.

2. **First-principles drafting at F.0 produces architectural blind spots.** F.4.B.3.D drafted from biome-classification first principles without surveying canonical AAA implementations. The result: an architecture that worked at the biome layer but didn't deliver per-archetype variety because the bootstrap pipeline was never made archetype-aware. New campaign was preceded by a research pass (`regional_archetype_variation_research_2026-04-29.md`) that surveyed 6 AAA references + 6 algorithmic approaches before architectural commitment.

3. **Scope creep across sub-phases compounds.** F.4.B.3.D's scope grew across 6+ sub-phases (F.4.B.3.A research → F.4.B.3.B/C transform attempts → F.4.B.3.D climate reframe → D.1-D.5 within-region machinery → D.5-fix amplitude reduction → 3 diagnostic audits → research pass). Each expansion was data-justified at the time, but cumulative effect was a campaign whose final scope was unrecognizable from F.0 framing. New campaign's §0 includes scope-creep discipline: "if a sub-phase surfaces an architectural gap, treat that as evidence of insufficient research-pass depth and consider another research pass rather than continuing to expand the campaign in-flight."

4. **Diagnostic-then-remediate beats remediate-then-discover.** The pattern of diagnostic-2 → Path B → diagnostic-3 → research pass was load-bearing — each diagnostic gated the next remediation, preventing premature commitment to architecturally-insufficient fixes. New campaign should preserve this pattern: when a sub-phase Andrew-gate returns REGRESS or PARTIAL, run a diagnostic before remediation rather than guessing at fixes.

**F.4.B.3.D deferred-work split (mirrors new campaign's §10 launch entry; recorded here for predecessor-campaign navigation)**:

Absorbed into new campaign:

- Climate Preview overlay (D.5c deferred from D.5b): new campaign F.5 (archetype painting UI benefits from climate field debug overlay).
- Equatorial Tropical archetype-specific tuning: new campaign F.7 (per-archetype tuning pass).
- Bootstrap noise pipeline elevation skew (audit §7 of diagnostic-2: "26% of vertices land at elevation ≥ 450m"): new campaign F.7 (per-archetype spline tuning operates at the bootstrap layer).

Stays as standalone follow-ups (NOT absorbed by new campaign):

- F.4.B.3.G phase-2 grassland precision floor (47.4 WU): unrelated f32 precision issue at chunk shared edges; standalone follow-up.
- Dead-write bug at `terrain_panel.rs` line 943: small cleanup; standalone follow-up.
- Stale `NoiseConfig` literal constructors at `behavioral_correctness_tests.rs` lines 517, 911, 912: pre-existing test file failure unrelated to regional archetypes; standalone follow-up.
- MountainRocky never produced by `lookup_biome`: reserved for future slope-conditional expression; standalone follow-up.
- River never produced: deferred to Water System Rebuild campaign per `docs/audits/water_system_architecture_2026-04-20.md`.
- Per-biome runevision tuning (Alpine/MountainRocky/SnowCap/Scree): architecturally orthogonal to regional archetypes; standalone follow-up.
- F.5's `apply_erosion` handler wiring: revisit after new campaign's F.4 lands (changes how erosion preset selection works).

**F.5 reduced-scope rationale**:

F.5's original scope was: (a) wire `apply_erosion` handler, (b) 8-climate integration tuning, (c) ARCHITECTURE_MAP update, (d) parent campaign re-mark. With F.4.B.3.D.3 removing the eight-biome preset system, scope items (a) and (b) no longer apply at the preset level — preset-shaped integration tuning is structurally absent. Scope items (c) and (d) execute in this closure session. The `apply_erosion` handler wiring becomes a standalone follow-up to revisit after the new campaign's F.4 lands; the new campaign's data flow may change how erosion preset selection works, and wiring the handler against the current architecture would create rework.

**Parent campaign update**: `docs/current/TERRAIN_MATERIAL_SYSTEM_CAMPAIGN.md` §7 updated to reflect Phase 1.6-F closed via architectural pivot; Phase 1 + Phase 1.5 re-marked COMPLETE per F.5's reduced closeout scope. New Phase 1.X (Regional Archetype Variation) added as the in-progress phase.

**ARCHITECTURE_MAP update**: terrain section reflects the closed campaign's deliverables (climate field, Whittaker biome lookup, per-biome parameters, scattered-convolution blending, world archetype catalog) and the new campaign launch (Regional Archetype Variation in progress).

**Test scoreboard at closure (no new tests; verification only)**:

- All terrain crate lib tests (738/738 passing per F.4.B.3.D.5-fix close): pass.
- D.4 / D.5 / D.5-fix integration tests: pass.
- Phase-2 / phase-3 / phase-4 invariants: pass at F.4.B.3.D.5-fix tightened thresholds.
- F.2 permanent regression (5/5): pass.
- Pre-existing failures unchanged from D.5-fix close: F.4.B.3.G 47.4 WU phase-2 grassland precision floor; stale `NoiseConfig` literal constructors at `behavioral_correctness_tests.rs` lines 517+911+912.
