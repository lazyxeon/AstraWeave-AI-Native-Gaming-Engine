...
|---|---|---|---|---|---|
| #3 | Editor `OrbitCamera` | `tools/aw_editor/src/viewport/camera.rs:50-639` | §2.1 | FOV in degrees, field name `fov` (stores degrees, converts to radians at producer boundary in `to_render_view`) | **C.4 boundary conversion applied; structural field rename deferred to C.4.B** |
