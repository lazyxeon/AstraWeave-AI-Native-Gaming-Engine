...
| Audit precedent | `docs/audits/camera_system_architecture_audit_2026-05.md` (C.0, pre-campaign), `docs/audits/camera_system_phase_2_audit_2026-05.md` (C.5, mid-migration), and `docs/audits/cinematics_consolidation_audit_2026-05.md` (C.7.0, cinematics-chapter preliminary) |
...
- **C.7.0 closed (2026-05-24):** Cinematics consolidation preliminary audit committed at `docs/audits/cinematics_consolidation_audit_2026-05.md`. Third audit-shaped sub-phase in the campaign (after C.0 and C.5). Pure-audit (no source changes). Inventoried cinematics surface across 6 crates (astraweave-cinematics, astraweave-render, astraweave-gameplay, tools/aw_editor, examples/cutscene_render_demo, astraweave-ui) plus test infrastructure. Mapped cross-crate dependency graph; identified two new dependency edges required for direction A (astraweave-gameplay → astraweave-cinematics for C.7.A; tools/aw_editor → astraweave-cinematics for C.7.C). No circular-dependency risk (astraweave-cinematics has zero astraweave-* deps).
  
  **Load-bearing finding**: empirical roll dormancy investigation (audit §3) verified **DORMANT** (high confidence). `CameraKeyframe.roll` has zero downstream runtime consumers: writes via UI slider (`cinematics_panel.rs:1068`), reads only for UI label display (line 1296) and lerp producing another CameraKeyframe (line 1417). No conversion to view matrix, no flow to renderer, no serialization output to anything that reads roll. This **reframes the planning round's Q2.γ**: roll absorption into CameraKey is feature *addition*, not preservation. C.7.A planning round must decide between three scope variants (γ-as-feature-addition / α-drop / γ-shim-only); audit §6 Q1 surfaces the decision.
  
  5 new latent issues catalogued (L.7.1 tick_cinematics zero callers — closes via C.7.B; L.7.2 CutsceneState::tick return-type awkwardness; L.7.3 tuple-vs-Vec3 boundary; L.7.4 editor CinematicsPanel dormant runtime-flow; L.7.5 astraweave-ui parallel panel). 8 open questions for the C.7.A planning round. Per-sub-phase scope estimates: C.7.A ~3 files; C.7.B 1-2 files; C.7.C 2-7 files (scope depends on Q1 outcome); C.7.D ~3 files; C.7.E 1-4 files.
  
  §7.11 methodology candidates accreted: audit-shaped-sub-phase pattern (now three concrete instances at distinct phase boundaries — codification candidate); empirical-verification of feature-existence assumptions before locking execution scope (reinforces C.5 §7.2); cross-crate consolidation requires explicit dependency-direction mapping in preliminary audits. Codification deferred to C.9 / E-closeout.

- **C.6 closed (2026-05-24):** Migration queue from C.5 audit fully executed in a single commit with six sectioned deliverables (A–F). All five in-scope migration targets closed; FreeFly gains explicit `sanitize()` validation. No (α) full migrations were selected by the planning round — the campaign's tail consolidated to removal- and targeted-fix-shaped work where the audit revealed dormancy (gizmo `CameraController`, nanite_demo camera state) or intentional bespoke sandboxes (unified_showcase, fluids_demo, shadow_csm_demo) for which full migration was out-of-campaign scope.
  
  - **C.6.A — Gizmo `CameraController` deletion** (per L.5.1). The `CameraController` and `SceneViewport` types removed from `tools/aw_editor/src/gizmo/scene_viewport.rs`; the file retains only `Transform` (actively used by `transform_panel`, `scene_state`, and mutation tests — Phase 1 inventory finding the C.5 audit did not enumerate). `TransformPanel.camera` field deleted. Both bench files (`gizmo_benchmarks.rs`, `gizmo_benchmarks_simple.rs`) deleted; `[[bench]] name = "gizmo_benchmarks"` Cargo.toml entry removed. The `camera_controller_tests` module (~150 lines) in `tools/aw_editor/tests/mutation_resistant_comprehensive_v2.rs` removed alongside the type. L.5.2/L.5.3/L.5.4 vanish with the deletion.
  - **C.6.B — unified_showcase sandbox formalization** (per L.5.5 confirmed + L.5.6 + L.5.7). Added file-level sandbox-status comment documenting convention deviations (-Z forward, no aspect-zero guard at projection, FOV-as-degrees field). Added `fov_degrees: f32` field for discoverability (was hardcoded `45.0_f32.to_radians()` at the projection site). Added pitch clamp `±85°.to_radians()` symmetric in mouse-motion handler. Projection site now reads `self.fov_degrees.to_radians()`.
  - **C.6.C — shadow_csm_demo targeted fix** (per L.5.8 + L.5.9). Renamed `Camera.fov: f32` → `Camera.fovy: f32` (radians; name now matches §2.1; semantic-equivalent). Added `.max(0.01)` aspect guard at projection site.
  - **C.6.D — fluids_demo targeted fix** (per L.5.11 + L.5.12). Converted `Camera.fovy` storage from degrees to radians (the field name was already canonical post-§2.1; the unit now matches). Dropped `.to_radians()` boundary conversion at projection site. Added `.max(0.01)` aspect guards at both initialization (line ~349) and resize handler (line ~471).
  - **C.6.E — nanite_demo unused camera state deletion** (per L.5.14). Deleted `DemoState` struct (camera_pos, camera_yaw, camera_pitch, camera_speed, mouse_sensitivity, last_mouse_pos fields); deleted `get_view_matrix()` method; deleted `handle_input()` method (WASD + mouse-orbit + pitch clamp). Event loop simplified to handle window close + escape only. Redraw handler remains stub; example's actual purpose (meshlet hierarchy generation) is unaffected.
  - **C.6.F — `FreeFly::sanitize()` validation** (per L.5.16). Added explicit caller-invoked validation method (Shape A; no hot-path boundary clamping at `projection_matrix()`). Clamps `fovy ∈ [10°, 170°].to_radians()`, `znear > 0.0001`, `zfar > znear + 0.001`, `aspect ≥ 0.01`. Six contract tests added to `freefly::tests`: clamp below/above fovy bounds, znear positivity, zfar > znear, aspect minimum, idempotence on valid state.
  
  Closure proof: multi-shape composite (six per-section proofs verified together). Structural-deletion for A/E; structural addition + behavioral test for B; structural-rename + behavioral preservation for C/D; contract test family for F. All grep verifications return expected counts; `cargo check --workspace`, `cargo build --workspace`, `cargo test -p astraweave-camera --lib` all pass; rendered output byte-equivalent for C/D (unit-equivalent conversions).
  
  Migration table §3 rows: all five in-scope C.6 targets marked **CLOSED (C.6)** with sub-deliverable identifier; L.5.16 row CLOSED (C.6.F).
  
  **C.5 audit falsifications recorded** (post-hoc per Q9 planning round decision, "Standard" granularity — three per-finding entries with file:line citations):
  
  - **shadow_csm_demo pitch clamp** (C.0 finding #24): **FALSIFIED**. Clamp verified present at `examples/shadow_csm_demo/src/main.rs:589-593` (`±89°.to_radians()`). C.0's inspection focused on projection/view-construction sites without grepping input-handler sites for the same pattern.
  - **fluids_demo pitch clamp** (C.0 finding #25): **FALSIFIED**. Clamp verified present at `examples/fluids_demo/src/main.rs:502` (`.clamp(-1.4, 1.4)`). Same root cause as shadow_csm_demo.
  - **nanite_demo pitch clamp** (C.0 finding #26): **FALSIFIED**. Clamp verified present at `examples/nanite_demo/src/main.rs:121-124` (`±FRAC_PI_2 ± 0.1`) — though the code containing the clamp was itself deleted in C.6.E as part of removing the dormant `DemoState`.
  
  Methodology observation banked as §7.11 candidate pillar (falsification opportunity discipline): medium-confidence audit findings inferred from absence-of-evidence at one site warrant grep verification across all sites where the pattern could legitimately live before recording the finding. The three falsifications above share root cause — inspection focused on projection/view-construction without checking input-handler sites. Codification deferred to C.9 / E-closeout.
...
| #4 | ~~Gizmo `CameraController`~~ | ~~`tools/aw_editor/src/gizmo/scene_viewport.rs:19-173`~~ | ~~§2.4 (partial)~~ | ~~`set_view_top` switches `up` field to `Vec3::NEG_Z` (line 163) — field semantics change during use~~ | **CLOSED (C.6.A) — type deleted; L.5.3 (`up` field mutation) vanished** |
| #4 | ~~Gizmo `CameraController`~~ | ~~`tools/aw_editor/src/gizmo/scene_viewport.rs:76-92`~~ | ~~(correctness, not §2)~~ | ~~Quaternion orbit math has unguarded gimbal-lock NaN at top/bottom (`offset.cross(up)` → zero)~~ | **CLOSED (C.6.A) — type deleted; L.5.2 (gimbal-lock) vanished** |
| #4 | ~~Gizmo `CameraController`~~ | ~~`tools/aw_editor/src/gizmo/scene_viewport.rs:19-173`~~ | ~~(duplication, not §2)~~ | ~~Reimplements OrbitCamera-class functionality in editor subsystem~~ | **CLOSED (C.6.A) — type, `SceneViewport` wrapper, both bench files, `TransformPanel.camera` field, and `camera_controller_tests` module all deleted. `Transform` (also in `scene_viewport.rs`) retained per Phase 1 finding — it has active production callers** |
...
| #23 | ~~`unified_showcase` bespoke camera~~ | ~~`examples/unified_showcase/src/main.rs:148-150`~~ | ~~§2.1~~ | ~~No `fovy` field; hardcoded `45.0_f32.to_radians()` at projection site (line 2168)~~ | **CLOSED (C.6.B) — `fov_degrees: f32` field added for discoverability; projection site reads `self.fov_degrees.to_radians()`; pitch clamp `±85°.to_radians()` added to mouse handler (L.5.5 confirmed C.0 finding). The `-Z forward` convention (L.5.6) and bespoke-pipeline status (out of §2.9) remain by design; sandbox-status comment formalizes the deviations.** |
| #23 | ~~`unified_showcase` bespoke camera~~ | ~~`examples/unified_showcase/src/main.rs:2162-2164`~~ | ~~§2.8~~ | ~~-Z forward at yaw=0 (uses `Vec3::NEG_Z` rotated by yaw/pitch quats — Bevy/glTF convention)~~ | **CLOSED (C.6.B) — deviation formalized in file-level sandbox-status comment per planning round's β scope; migration to canonical +X forward would entail wholesale rewrite of the bespoke render pipeline, out of campaign scope.** |
| #23 | ~~`unified_showcase` bespoke camera~~ | ~~(general)~~ | ~~§2.9~~ | ~~Operates entirely outside `astraweave-render::Renderer` — bespoke wgpu pipeline with own `CameraUniforms` struct~~ | **CLOSED (C.6.B) — planning round chose β (sandbox formalization), not α (engine migration). Deviation formalized in file-level comment; full migration out of campaign scope.** |
| #24 | ~~`shadow_csm_demo::Camera`~~ | ~~`examples/shadow_csm_demo/src/main.rs:74-115`~~ | ~~§2.1~~ | ~~Field `fov` (not `fovy`); stores radians but uses non-canonical name~~ | **CLOSED (C.6.C) — field renamed `fov: f32` → `fovy: f32` (radians, semantic-equivalent). Plus `.max(0.01)` aspect guard added at projection site (line ~602).** |
| #24 | ~~`shadow_csm_demo::Camera`~~ | ~~(input layer)~~ | ~~(correctness, not §2)~~ | ~~Likely missing pitch clamping (only inspected fields; input handler not audited in C.0)~~ | **FALSIFIED (C.5) — clamp verified present at `examples/shadow_csm_demo/src/main.rs:589-593` (`±89°.to_radians()`). Recorded in C.6 status log as "C.5 audit falsifications recorded" entry per Q9 Standard granularity.** |
| #25 | ~~`fluids_demo::Camera`~~ | ~~`examples/fluids_demo/src/main.rs:26-46`~~ | ~~§2.1~~ | ~~Field `fovy` stores **degrees**, converted to radians at projection time inside `build_view_projection_matrix` (line 39) — name matches §2.1 but unit doesn't~~ | **CLOSED (C.6.D) — `fovy` storage converted degrees → radians; constructor default uses `45_f32.to_radians()`; projection site drops `.to_radians()` conversion (field is now canonical units).** |
| #25 | ~~`fluids_demo::Camera`~~ | ~~`examples/fluids_demo/src/main.rs:460`~~ | ~~§2.3~~ | ~~Aspect resize without `.max(1)` or `height > 0` guard~~ | **CLOSED (C.6.D) — `.max(0.01)` guard added at both initialization (line ~353) and resize handler (line ~471).** |
| #26 | ~~`nanite_demo` bare fields~~ | ~~`examples/nanite_demo/src/main.rs:23-30`~~ | ~~§2.9~~ | ~~No Camera type; bare `camera_pos`, `camera_yaw`, `camera_pitch` fields on `DemoState`~~ | **CLOSED (C.6.E) — entire `DemoState` struct deleted along with `get_view_matrix()` and `handle_input()` methods. Per L.5.14: the example's redraw handler is a stub; camera state had zero callers. Example continues to demonstrate meshlet hierarchy generation (its actual purpose).** |
| #26 | ~~`nanite_demo` bare fields~~ | ~~(input layer)~~ | ~~(correctness, not §2)~~ | ~~Likely missing pitch clamping~~ | **FALSIFIED (C.5) — clamp verified present at `examples/nanite_demo/src/main.rs:121-124` (`±FRAC_PI_2 ± 0.1`), though the containing code was deleted in C.6.E. Recorded in C.6 status log as "C.5 audit falsifications recorded" entry per Q9 Standard granularity.** |
...
- v1.4 (2026-05-24) — C.6 status log entry appended. Six sectioned deliverables (A–F) closed all five in-scope migration targets plus L.5.16 FreeFly validation. Migration table rows updated: #4 (gizmo, all three rows) marked CLOSED (C.6.A); #23 (unified_showcase, all three rows) marked CLOSED (C.6.B); #24 fov-rename row marked CLOSED (C.6.C); #24 pitch-clamp row marked FALSIFIED (C.5) post-hoc; #25 fovy-storage row marked CLOSED (C.6.D); #25 aspect-guard row marked CLOSED (C.6.D); #26 bare-fields row marked CLOSED (C.6.E); #26 pitch-clamp row marked FALSIFIED (C.5) post-hoc. Three C.5 falsifications recorded post-hoc with file:line citations per Q9 Standard granularity. Methodology observation banked as §7.11 candidate pillar (falsification opportunity discipline; codification deferred to C.9 / E-closeout). After C.6, only cinematics-related rows (#10 family) remain open in the migration table; C.7 will close those.
- v1.5 (2026-05-24) — C.7.0 status log entry appended. §0 metadata table updated with parallel reference to `docs/audits/cinematics_consolidation_audit_2026-05.md` alongside the C.0 and C.5 audits. C.7.0 is the third audit-shaped sub-phase in the campaign; its load-bearing roll dormancy finding (DORMANT, high confidence) reframes Q2.γ as feature-addition rather than feature-preservation. 5 new latent issues (L.7.1–L.7.5); 8 open questions for C.7.A planning round.
