...
| **FALSE-PRODUCTION-READY** | **(v1.1 split — narrowed to the hollow / over-claim kind.)** (1) A production-ready claim **attributable to the crate itself** — i.e. role `P` with `lifecycle=active` AND `integration∈{wired,mixed}`, **or** an **own-docs production claim** (defined below); AND (2) verification fails that claim **in a way that proves it hollow**: `ISLAND`/`ex-only` with `boundary=no` (Check B), **OR** the advertised core is a stub (Check C4), **OR** `feature-exercising=no` (Check C3 — tests don't exercise the claimed feature). *The production claim that orphaning / a stub / no-feature-coverage contradicts is the defining feature — the dangerous false-positive the audit exists to catch.* A **wired, non-stub** crate whose only failure is a failing test (Check C2) is **PRODUCTION-CAPABLE-FAILING-TESTS** (next row), NOT this. **First firing in R.0.B → second-agent confirmation (§4.5).** |
| **PRODUCTION-CAPABLE-FAILING-TESTS** | **(v1.1 split — added at R.0.B close, 2026-06-30.)** Check A PASS, AND Check B = `live` (or `bin`) — wired to real callers, AND the advertised core is **non-stub** (`stub_core=no`, `feature_exercising∈{yes,partial}`), BUT Check C2 fails (**≥1 failing test**). *A real, integrated product carrying test-rot or a regression — explicitly distinct from a hollow crate; the failing tests get fixed (test-rot) or the defect gets fixed (regression), not relabelled.* **Selector vs. FALSE-PRODUCTION-READY:** a production-claimed crate that fails verification → if `reachability∈{ISLAND,ex-only}` (boundary=no) **or** `stub_core=yes` **or** `feature_exercising=no` ⇒ FALSE-PRODUCTION-READY; else (wired + non-stub + failing test) ⇒ PRODUCTION-CAPABLE-FAILING-TESTS. |
...
# Verdict
verdict: VERIFIED-PRODUCTION|PRODUCTION-CAPABLE-FAILING-TESTS|WIRED-BUT-UNTESTED|COMPILES-BUT-ORPHANED|DORMANT-HONEST|FALSE-PRODUCTION-READY|UNTRACED-UNKNOWN|BROKEN
