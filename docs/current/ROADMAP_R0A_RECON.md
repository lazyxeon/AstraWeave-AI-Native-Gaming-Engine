...
| A2 | **GOAP + BT + LLM hybrid arbiter** — mode transitions deterministic & feature-tested | `astraweave-ai::arbiter` + `astraweave-behavior` VERIFIED-PRODUCTION (Check C3 must assert the GOAP↔LLM↔BT transition, not smoke) | behavior live (6 rdeps); arbiter is the load-bearing AI surface |
...
| A5 | **LLM integration honesty** — the documented model path is the real path | `astraweave-llm` VERIFIED-PRODUCTION; the `OLLAMA_MODEL` default vs Qwen3 doc-drift resolved | CLAUDE.md flags `phi3:medium` default vs documented Qwen3 — a claim-vs-reality item |
