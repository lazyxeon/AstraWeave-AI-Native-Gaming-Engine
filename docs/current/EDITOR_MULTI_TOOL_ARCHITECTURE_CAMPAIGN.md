...
Sub-phase 3 Mediator Brush Round-5-Closure.A — campaign doc codification of Round 5 outcome with three fix candidate designs for Andrew-gate: COMPLETE 2026-05-07, commit <Round-5-Closure.A-hash> (this commit). Outcome 1 per Round 5 prompt §3.6: Mechanism 1 (wrong texture, "different render target" sub-class) DEFINITIVELY CONFIRMED. T8.A=0.0 invariant + T8.B handle mismatch every frame + T8.C confirms adapter API has no depth attachment param + T8.D inside_viewport=true 650/650 + T8.E math consistent (depth=0 → ndc_z=0 → near-plane unprojection). 5 rounds of evidence-grounded narrowing converged on single mechanism in one capture session. New methodology lesson candidate: §7.7 wrapped-component resource identity trap (sibling to §7.6 derived-value reasoning trap).
Sub-phase 3 Mediator Brush Real-Fix — fix candidate (a)/(b)/(c) per Andrew-gate decision: NOT STARTED (Andrew-gate decision required; small-fix prompt drafted post-decision per two-session-minimum).
Sub-phase 3 Mediator Brush Fix 8f4668599 cleanup revert: NOT STARTED (gated on real-fix Andrew-gate PASS for clean recovery narrative per audit §5.1).
Sub-phase 3.C — closeout: NOT STARTED (gated on real-fix Andrew-gate PASS + 8f4668599 cleanup revert).
...
### 2026-05-07, Sub-phase 3 Mediator Brush Diagnostic Round 5 Closure, commit <Round-5-Closure.A-hash>
