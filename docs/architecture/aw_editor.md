...
- **The viewport shading-mode dropdown is inert.** `ShadingMode { Lit, Unlit, Wireframe }` (`viewport/toolbar.rs:455-464`) is converted at `viewport/widget.rs:880` and handed to `ViewportRenderer::render`, whose parameter is **`_shading_mode: u32`** (`viewport/renderer.rs:570`) — unused. No match, no branch, no uniform write; `has_lighting()` / `is_wireframe()` (`toolbar.rs:491,496`) have zero non-test callers. Selecting Unlit or Wireframe changes nothing rendered. Decisional — wire it, or remove the control.
- **A second, wider shading dropdown advertises two modes that do not exist.** The docked Viewport panel offers five entries (`tab_viewer/mod.rs:2082` — Shaded / Wireframe / Unlit / **Normals** / **UVs**) and `main.rs:5208-5214` maps Normals and UVs silently to `Lit`. Beyond the inert-parameter problem above, this promises debug visualisations the editor has never had — which is precisely what a renderer diagnosis wants first. Decisional; implementing Normals/UVs would be the single highest-value debug affordance for the terrain campaign.
- **The Terrain panel's "Base Amplitude" slider is inert.** Full trace in `docs/architecture/terrain.md` §11; proven empirically by `tools/aw_editor/tests/t2d_camera_light.rs` Experiment F rendering amplitudes 50/20/6 byte-identical.
