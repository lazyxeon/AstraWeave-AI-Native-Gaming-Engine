...
- Five parallel solver/manager surfaces (**not unified**): `FluidSystem` (lib.rs PBD GPU pipeline), `UnifiedSolver` (high-level coordinator), `ResearchFluidSystem` (research-grade umbrella), `PCISPHSystem` (standalone PCISPH), `WaterEffectsManager` (visual coordinator)
