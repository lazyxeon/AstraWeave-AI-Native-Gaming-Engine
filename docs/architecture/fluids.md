...
> **Recovery path is LOCAL-ONLY (2026-07-21 update).** The remote
> `w0-pre-deprecation` tag was **deleted from GitHub on 2026-07-20**
> (director-executed, board-state §3 option (a) — it anchored the full
> pre-rewrite history and blocked the D8 LFS reclamation; `git ls-remote --tags
> origin` now lists only `assets-v1`). The corpus remains recoverable from three
> LOCAL sources: the `w0-pre-deprecation` tag in the canonical checkout
> (`d:\AstraWeave`), the pristine pre-rewrite mirror at `C:\AD6_BACKUP`, and the
> bundle `d:\w0-pre-deprecation.bundle` (956 MiB, created 2026-07-20). A fresh
> clone does NOT carry it. Every "recoverable at tag" statement in this doc and
> its cross-references means these local paths.
...
| 1.9 | 2026-07-21 | **w0 recovery-path wording update (T.0 record reconciliation; no code re-verification — `last_verified_commit` unchanged).** The remote `w0-pre-deprecation` tag was deleted from GitHub on 2026-07-20 (director-executed, `BOARD_STATE_2026-07-19.md` §3 option (a) — it anchored full pre-rewrite history and blocked the D8 LFS reclamation). Added the §0.5 LOCAL-ONLY recovery note (local tag in the canonical checkout / `C:\AD6_BACKUP` / `d:\w0-pre-deprecation.bundle`) and re-pointed the doc's operative "recoverable at tag" statements (Owner notes, invariants 25–31 row) at it. Historical narrative untouched. Cross-referencing docs updated in the same commit (water.md, gameplay.md, CLAIMS_REGISTRY, staleness banners). |
