...
| **Last verified against commit** | branch `campaign/fluids-f3` (F.3 execution; see `docs/campaigns/fluids-integration/F3_EXECUTION_REPORT.md`) |
...
| `astraweave-fluids/src/volume_grid.rs` | 1,049 | `WaterVolumeGrid` + `WaterCell` + `MaterialType` (8 variants, with absorption rates) + `WaterGridStats` + `WaterSimConfig` + `CellFlags` | Active (module-level) | Voxel-water alternative to particle simulation. **F.3: gate-flag reads (`cell_flow_blocked`), conserving `flow_horizontal`, `simulate`→`simulate_substep` dt clamp (`MAX_STABLE_DT`), `apply_terrain_boundary(&[f32])` glue. Backs `astraweave-water`'s `voxel` WaterQuery.** |
