...
**Notes:** The surface is a discrete grid of `CHUNK_SIZE = 64` chunks; a `(2*GRID_RADIUS+1)² = 289`-chunk block around the camera is active each frame (`water.rs:31-35`). Each chunk picks an LOD by camera distance (`LOD_DISTANCES`/`LOD_SUBDIVS`); chunks of the same LOD draw in **one instanced draw call**. Wave displacement is a pure function of world XZ, so chunk meshes are world-stable and shared LOD-boundary vertices agree exactly; the only mismatch is curve-vs-chord on a coarse neighbour's edge (≤ ~1.65 unit total wave amplitude), covered by per-chunk skirts that drop `SKIRT_DEPTH = 8.0`. The vertex shader sums 4 Gerstner waves with a Profile-A `Q ≤ 1.0` steepness cap to prevent normal inversion (epsilon-guarded denominator since T.W.1.A so a zero styled amplitude cannot NaN). World Y comes from the `water_level` uniform (no baked mesh Y) — this is the real `set_water_level`. **T.W.1.A additions:** (a) a **horizon shell** — a flat 8-quad annulus at water level from one chunk *inside* the grid's outer boundary (overlap band, no T-junction seam class) out to ±`HORIZON_OUTER_HALF_EXTENT` = 100,000, anchored to the grid's snapped chunk center, drawn under the same dormant condition — so the visible water boundary is the far-plane horizon arc (or full fog), never a mesh edge; (b) a **camera-distance amplitude fade** (`WAVE_FADE_START/END` = 260/420, WGSL-mirrored) bringing Gerstner amplitude+steepness to exactly zero before the shell's inner edge (grid→shell seam is flat-against-flat by construction; near field ≤ LOD1 untouched); (c) **per-style wave params** via `set_wave_params(amplitude_scale ∈ [0,1], foam_threshold)` riding the former `_pad2` uniform slot (layout unchanged, 512 B).
...
| SPH / voxel fluid *simulation* | (removed) | Deleted (W.1) | 58,796 deletions; preserved only at the `w0-pre-deprecation` anchor @ `3a8296038` — **local-only since 2026-07-20** (remote tag deleted; recovery = local tag / `C:\AD6_BACKUP` / `d:\w0-pre-deprecation.bundle`). See [`fluids.md`](./fluids.md) §0.5 recovery note |
...
### Hot paths
- **Water surface pass** (`render()` per frame): near ≈ 0.24 ms, horizon ≈ 0.18 ms (`F4_3_EXECUTION_REPORT.md` §2). W.2a chunked surface added +0.04-0.06 ms over the single-plane baseline (`W2A_EXECUTION_REPORT.md` §2). T.W.1.A horizon shell + fade: +0.004 ms isolated-near median (0.2478→0.2521), horizon within run variance — `TW1A_OUTCOME.md` §3.
