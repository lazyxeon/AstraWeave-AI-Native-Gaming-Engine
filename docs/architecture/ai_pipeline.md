...
3. **The Arbiter is a tested-but-post-v1.0 hybrid path — NOT production-wired (R-series A2 recon / M1 trace-honesty, 2026-06-30).** `AIArbiter` is feature-gated on `llm_orchestrator` (OFF by default), requires a live Ollama LLM, bypasses the dormant ~15K-LoC LLM hardening surface (§13.7), and has **zero production constructors** outside tests. The **wired, deterministic v1.0 AI path is `RuleOrchestrator`** (the `ecs_ai_plugin` production path) **+ `GoapOrchestrator`** (the flagship `hello_companion` demo) — both VERIFIED-PRODUCTION. The "stable AI pipeline" v1.0 claim rests on those, not the arbiter; wiring the arbiter (enable the feature, accept the live-LLM dep, connect the dormant hardening, fix the BT-stub / dead `fast_executor` / one-way-terminal BT) is a **post-v1.0 capability build** (R.1 §6.1). When enabled it runs GOAP for instant tactical control + the LLM (runtime default phi3:medium; Qwen3 opt-in via `OLLAMA_MODEL`) for background strategic plans, with a 4-tier fallback. Dual-executor architecture per `ai_arbiter.rs:1-46` doc-comment.
...
5. **A5 honesty — the runtime LLM default is `phi3:medium`, NOT a wired Qwen3 path (R-series M1 trace-honesty, 2026-06-30).** Doc-comments and CLAUDE.md refer to "GOAP+Qwen3 Hybrid" (`ai_arbiter.rs:1`); older docs (e.g. `ARCHITECTURE_REFERENCE.md:41`) say "GOAP+Hermes Hybrid"; commit `2468b25f1` is "Replace Phi3 with Hermes…". **None of these is a wired model path:** the LLM is dormant / post-v1.0 (the arbiter is feature-gated, note 3), the runtime default is `phi3:medium` (`orchestrator.rs:488-490`), and the alternatives are opt-in via `OLLAMA_MODEL`. No v1.0 doc should claim a live LLM model path the engine does not run (the A5 honesty constraint; R.1 §4.1/§6.1).
...
8. **Per-crate v1.0 verified verdicts (R-series M1 trace-honesty, 2026-06-30) — the group `active/wired` front-matter syncs to the primary crate `astraweave-ai` only.** Individually verified (R.0.B): `astraweave-ai`, `astraweave-behavior`, `astraweave-llm`, `astraweave-director` = **VERIFIED-PRODUCTION**; `astraweave-memory`, `astraweave-embeddings`, `astraweave-rag`, `astraweave-persona` = **VERIFIED-PRODUCTION** (their wired status holds in substance — the CLAIM-MISMATCH flag was inherited-not-individually-asserted, now asserted; the dormant *composites* — Memory pipeline, RAG pipeline, Advanced GOAP, LLM hardening, Dialogue LLM layer — remain in-design per §13); **`astraweave-coordination` and `astraweave-npc` = COMPILES-BUT-ORPHANED** (in-design / dormant, zero workspace consumers — consistent with §13.5 / §13.3).
