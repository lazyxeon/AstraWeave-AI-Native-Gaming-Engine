...
[Stage R3: Shadow cascades (CSM)]
    files: astraweave-render/src/renderer.rs (the LIVE machinery: 2048²×2-layer
           Depth32Float array, update_cascade_splits PSSM λ-blend, sphere-fitted
           per-cascade matrices, per-cascade UBOs, static `shadow_pipeline` +
    ...
    role: Render directional-light depth into a texture_2d_array (one cascade per layer);
          ...
          L.3: in `draw_into`, terrain-forward chunks ALSO draw into both cascade layers
          (depth-only, per-cascade AABB culling); in `render()` terrain does not draw at all
          (the terrain forward path itself is draw_into-only — recorded parity gap).
    │
    ▼
